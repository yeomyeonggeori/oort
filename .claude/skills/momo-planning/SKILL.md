...
name: momo-planning
description: >
  oort 기획/오케스트레이션 워크플로우 스킬. momo 레포에서 ADR 기안·승인, 티켓 발급(MOMO-NNN),
  핸드오프 패킷 작성, 병렬 worker 오케스트레이션(상한은 docs/planning/PIPELINE.md), 코드리뷰·순차 머지, 계획 이탈(deviation)
  판정·로드맵 환류 작업을 할 때 사용한다. 트리거: "ADR 기안/결정", "티켓 발급/끊어줘", "핸드오프",
  "워커에게 넘겨", "머지 사이클/리뷰 돌려", "이탈 판정", "로드맵 반영", "기획 세션 시작".
---
