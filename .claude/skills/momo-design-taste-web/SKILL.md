...
- **Buttons are filled pills, not outlines** (ADR-0189 D6). `<Button>` primary = ink, secondary/outline = `--surface-muted` fill, destructive = `--danger-fill`, ghost = none; `size="icon"` is a 34px circle (send, floating round buttons); header icon controls follow the mockup `.a-ibtn` (34px, `rounded-md`, no border, `text-ink-muted`) — see `channelHeaderControlClass`. On a sheet or a grey band a secondary pill disappears (1.01); use the mockup's sheet grammar there: `variant="ghost"` + `bg-surface shadow-sm`. A 3:1 `border-line-strong` outline is for **text input vessels only** (`Input`, `Select`, composer).
...
- **Focus ring is mandatory and visible.** The house pattern is `focus-visible:focus-ring`: a 2px signal outline inset by its width (`outline-offset: -2px`). Filled controls draw the ring in their label colour: ink fills (`bg-primary`) add `focus-ring-on-primary`, signal and destructive fills add `focus-ring-on-fill`. If you set `outline-none`, you must add a `focus-visible:` ring in the same class list; the pre-flight fails a naked `outline-none`.
