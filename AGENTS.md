...
- 배포는 [M7 게이트](docs/cicd/03-store-readiness-gate.md) 등급을 따른다. **내부 등급(M7-I)** PASS 기록과 **배포 건마다의** owner 승인 없이 팀 대상 TestFlight internal·직접 전달 공증 DMG를 배포하지 않는다. 데스크탑 업데이터 매니페스트는 배포 채널이 정해지기 전까지 게시하지 않는다. **스토어 등급(M7-S)** PASS 기록과 owner 승인 없이 App Store 제출·external TestFlight·공개 공증 배포를 하지 않는다. release·유료 macOS workflow와 로컬 배포 스크립트(`publish_next_build.sh` 등)는 owner 승인 경계를 유지한다. 구현 권한이 배포 권한으로 확대되지 않는다.
