...
- 배포는 [M7 게이트](docs/cicd/03-store-readiness-gate.md) 등급을 따른다. **내부 등급(M7-I)** PASS 기록과 **배포 건마다의** owner 승인 없이 팀 대상 TestFlight·공증 DMG(직접 전달·next 채널)를 배포하지 않는다. PASS 전 증거 빌드(owner 1인 TestFlight 그룹·owner 기기)도 빌드마다 owner 승인 뒤에만 한다. 데스크탑 next 채널(팀 채널, 공개 URL) 게시도 이 규칙을 따른다. **스토어 등급(M7-S)** PASS 기록과 **배포 건마다의** owner 승인 없이 App Store 제출·external TestFlight·공개 공증 배포를 하지 않는다. PASS는 기록된 커밋·빌드에만 유효하다. release·유료 macOS workflow, 로컬 배포 스크립트(`publish_next_build.sh` 등), Xcode Cloud의 TestFlight 액션은 owner 승인 경계를 유지한다. 구현 권한이 배포 권한으로 확대되지 않는다.
