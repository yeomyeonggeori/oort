...
function AddChannelMemberPanel({
  ...
  if (roster.pending && groups.total === 0) {
    ...
    //
    // 이 버튼의 「멤버 초대하기」는 의도적으로 남는다 (#1573): 초대는 워크스페이스에
    // 새 사람을 부르는 행위의 낱말이고, DirectoryRoute의 같은 행위와 같은 이름이다.
    // 개명된 쪽은 여기로 들어오는 문(빈 채널의 「멤버 추가하기」)이라, 첫 실행
    // 경로에서 같은 라벨이 두 번 나타나던 이중 의미는 그쪽에서 갈라졌다.
    body = (
      <EmptyInvite
