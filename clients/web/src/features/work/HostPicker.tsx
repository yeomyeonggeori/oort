...
//
// 두 표면이 같은 act를 제안한다: 작업 세션 패널의 인수와 작업 흐름 상세의
// 인수. 둘의 문장은 이미 한 글자까지 같게 쓰기로 한 것이었는데 컨트롤은 두
// 벌이었고, 그래서 어휘만 호출자가 주고 기하·상태·키보드는 여기 한 곳에 둔다 —
// FilterTabs가 인박스와 작업 흐름 사이에서 세운 형태 그대로, 두 번째 구현이
// 아니라 두 번째 호출자여야 한다.
//
// 그 규율을 이 파일 한 칸에서 멈추면 픽커를 **감싸는 문장들**이 다시 두 벌이
// 된다 — 실제로 그랬고 둘 다 틀린 채로 같이 늙었다("Git 계보만 새 호스트로
// 이어집니다"는 서버가 이어 붙이는 것이 스레드라는 사실과 정반대였다). 그래서
// 지금은 이 픽커의 유일한 호출자가 `TakeoverBlock` 이고, 문장은 거기 있다
// (ADR-0154 D3, #1137).
//
...
//    `인수` -> `인수하는 중`으로 바꾸면 누른 버튼의 폭이 바뀌고, 아직 포인터가
