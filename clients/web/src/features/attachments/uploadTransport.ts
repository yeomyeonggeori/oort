...
// http: https: ws: wss:`라 어디로든 나간다. 브라우저 배포의 정책은 훨씬 좁고,
// 이 레포가 내려보내는 것(`infra/prod/Caddyfile`)에는 **이슈 #1207 이후**
// `https://www.googleapis.com`이 들어 있다 — 선언을 실제 데이터 경로에 맞춘
// 것이다. `clients/web/gates/gate-csp-deploy.mjs`가 그 정책 문자열을 파일에서
// 그대로 읽어 이 함수를 태우고, 토큰 하나를 빼면 아래 갈래가 되살아난다.
//
// 그래도 이 감지기는 남는다. 이 앱이 우리 Caddyfile 뒤에서만 도는 것은 아니고
// (자체 호스팅·역프록시·기업 정책), 그런 배포에서는 여전히 XHR이 status 0으로
// 실패한다 — 네트워크가 끊긴 것과 구별되지 않는 모양으로. 구별하는 방법은 이
// 레포가 이미 한 번 골랐다: 문서의 `securitypolicyviolation`을 듣고 막힌 주소가
// 내가 부른 그 주소인지 본다(`observerStream.cspBlockedHost`가 관전 소켓에 대고
// 같은 일을 한다). 그래야 화면이 호스트를 탓하거나 영원히 도는 대신 **배포
// 설정이 막았다**는 참말을 한다.
// =============================================================================
