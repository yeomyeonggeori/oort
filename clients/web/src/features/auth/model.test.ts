...
import { toDeepLinkUrls, toMdnsRecords } from "@/lib/tauri";
...
describe("LAN discovery decisions", () => {
  it("offers a sighting whose TXT base is a usable http(s) URL", () => {
    expect(
      discoveredServers([
        { name: "momo on macbook", txt: { base: "http://macbook.local:28000" } },
      ])
    ).toEqual([
  ...
  it("dedupes by base and preserves discovery order", () => {
    expect(
      discoveredServers([
        { txt: { base: "http://a.local:28000" } },
        { txt: { base: "http://b.local:28000" } },
        { txt: { base: "http://a.local:28000" } },
  ...
  it("stays silent rather than guessing an address it was not told", () => {
    expect(
      discoveredServers([
        { name: "momo on macbook", host: "macbook.local", port: 28000 },
        { txt: { base: "ftp://macbook.local" } },
        { txt: { base: "   " } },
        { txt: {} },
      ])
    ).toEqual([]);
  ...
  it("drops the port from the label when it is the scheme default", () => {
    expect(
      discoveredServers([{ base: "https://momo.example.com" }])[0].displayHost
    ).toBe("momo.example.com");
  });
});

describe("shell payload normalisation", () => {
  it("reads the deep-link plugin's array, a bare string and { urls }", () => {
    expect(toDeepLinkUrls(["momo://join?code=a"])).toEqual([
      "momo://join?code=a",
    ]);
    expect(toDeepLinkUrls("momo://join?code=a")).toEqual([
      "momo://join?code=a",
    ]);
    expect(toDeepLinkUrls({ urls: ["momo://join?code=a", 7] })).toEqual([
      "momo://join?code=a",
    ]);
    expect(toDeepLinkUrls(null)).toEqual([]);
  });

  it("reads mDNS records as an array or under { servers }", () => {
    const record = { name: "momo", txt: { base: "http://a.local:28000" } };
    expect(toMdnsRecords([record])).toEqual([
      { name: "momo", host: undefined, port: undefined, base: undefined, txt: { base: "http://a.local:28000" } },
    ]);
    expect(toMdnsRecords({ servers: [record] })).toHaveLength(1);
    expect(toMdnsRecords({ nope: true })).toEqual([]);
    expect(toMdnsRecords(undefined)).toEqual([]);
  });
});
