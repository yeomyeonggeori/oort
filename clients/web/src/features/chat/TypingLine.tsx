...
// =============================================================================
// 컴포저 공유 메타 행의 「작성 중」 판 (ADR-0149 · goal B3 W2 · U-8).
//
// ## 힌트와 한 자리를 쓰되, 작업 중 줄과의 인접성은 지킨다
//
// 이 줄과 `AgentActivityBar`(「김인턴이 작업 중」)는 같은 종류의 정보다: **지금 이
// 채널에서 누가 무엇을 하고 있나.** 사람이 작성 중이면 넓은 화면의 키 힌트를 같은
// 26px 행에서 밀어내고, 바로 아래 `AgentActivityBar`와 인접해 사람을 위에 둔다.
// 아무도 작성하지 않을 때는 그 자리를 힌트가 쓰거나, 힌트도 없는 판에서는 이
// 컴포넌트의 zero-width-space 판이 예약한다. 따라서 wide-only 키 힌트와 작성 중 줄이
// 두 행으로 동시에 쌓이지 않으면서도 어느 전이에서도 컴포저 높이는 바뀌지 않는다.
// 폰 DM의 상시 안내는 이 공유 대상이 아니며 기존처럼 이 행 바로 위에 따로 남는다.
//
...
// 그대로 한다. 기본에는 힌트가 같은 글자 상자를 채우고, 힌트가 없으면 **같은 높이의
// 빈 자리**를 남기며, 작성 중에는 이 문장이 그 상자를 교체한다. 종전처럼 힌트 26px과
// 예약 26px을 동시에 쌓지 않으므로 빈 채널 하단에서 한 행(26px)을 회수한다.
//
...
//   * **2줄 고정** — 흔들림을 막으려면 공유 메타 행 자체를 늘 52px로 예약해야 한다.
//     그러면 아무도 안 치는 기본 시각의 한 줄 힌트 아래에 26px이 다시 죽고, 작업 중
//     줄까지 서는 판은 78px가 된다. 화면 아래쪽 78px는 폰에서 메시지 두 줄이다.
//
 ...
 *
 * `px-6`은 이 자리를 기본에 쓰는 힌트 판과 같은 값이다 (`Composer.tsx`의 그 주석:
 * 폼의 `p-3` +
 * 텍스트에어리어의 `px-3` = 24px, 그래서 첫 글자가 플레이스홀더의 첫 글자와 같은
 * 세로선에 선다). 1차는 `px-4`였고, 당시 입력창 아래 12px 회색 3행이 왼쪽 모서리를
 * 두 개 갖고 있었다 — 그 판정이 이미 같은 파일 주석에 적혀 있었다
