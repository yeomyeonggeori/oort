      ...
      <span className="line-clamp-2 min-w-0 flex-1">
        {block.kind === "deleted" ? (
          ...
          <>
            ...
            {block.quotesAnother && (
              <span data-testid="quote-nested"> · {QUOTE_NESTED_MARK}</span>
            )}
          </>
        )}
      </span>
...
export function QuoteBlock({
  ...
  return (
    <button
      ...
      className={cn(
        RAIL,
        "mb-1 w-full rounded-sm hover:border-accent hover:text-ink focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-accent"
      )}
    >
      <QuoteContent block={block} directory={directory} />
