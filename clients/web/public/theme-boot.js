// =============================================================================
// 첫 페인트보다 먼저 스킴을 못 박는다 (U2 · 웹 테마 토글).
//
...
(function () {
  ...
  try {
    ...
    // 프라이빗 모드 · 임베디드 웹뷰의 저장 금지. 읽을 선택이 없으면 시스템을
    // 따르고, 그것이 이 앱의 기본값이므로 아무 일도 하지 않는 것이 정답이다.
    stored = null;
  }
  // "system"과 미지의 값은 같은 답을 받는다: 스탬프를 붙이지 않는 것.
  ...
  for (i = 0; i < metas.length; i++) {
    if ((metas[i].getAttribute("media") || "").indexOf(stored) >= 0) {
