...
//
// 시트를 열어 본 것이 권한 요청이 되면 안 된다. 요청은 사람이 누른 버튼에서만
// 한다: M3 알림 미리 안내의 [계속], 그리고 이 시트의 「알림 켜기」 줄(아직 안
// 물었을 때). 둘 다 `PushProvider` 의 `usePushPrompt().ask` 를 부른다(#2820,
// ADR-0193 D8). 여기서는 지금 서 있는 답만 읽는다.
//
