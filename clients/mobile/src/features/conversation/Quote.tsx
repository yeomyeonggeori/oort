...
const buildStyles = (color: Palette) => StyleSheet.create({
  ...
  // **1.100:1** 이다 — 범위를 닫기는커녕 눈에 보이지 않는다. 그래서 블록을
  ...
  // `textFaint` 는 웹의 `--line-strong` 과 **사실상 같은 자리**였고, #1164 이후로는
  // 두 스킴 다 바이트로 같다(다크 #6f6e73 · 라이트 #84817d). 타임라인 배경 대비
  // **3.562:1** 로 웹이 컨트롤 테두리에 요구하는 ≥3:1 을 넘는다 — 중성이면서 실제로
  // 보이는 값이다. (`border`(#34323b)는 1.428:1 이라 「중성」이라는 말만 지키고
  // 선을 지우게 된다.)
  //
  ...
  // 데만 쓴다 — `textFaint` 는 배경 대비 3.562:1 로 본문 AA 를 못 지났고,
