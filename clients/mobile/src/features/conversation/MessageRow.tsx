 ...
 * (`#d9a441`)으로 그린다. 두 값이 그때도 닮아 보였지만 **계약이 아니었다** — 폰의
 * `accent` 는 그 시점에 파랑이라 이름으로는 대응조차 되지 않았고, 어느 한쪽
 * 팔레트를 손대는 날 경계가 조용히 갈라질 자리였다.
 *
 * 그리고 실제로 손댔다: #1155 가 폰 다크의 accent 가족을 웹 다크 항(#f0a850)으로
 * 정렬했다. 이름으로 대응했더라면 이 줄이 그날 파랑에서 호박으로 **뜻 없이**
 * 옮겨갔을 것이다. 역할로 대응했으므로 이 줄은 움직이지 않았다.
 *
...
const buildStyles = (color: Palette) => StyleSheet.create({
   ...
   * `warnSurface`(#241d0f)는 이제 OKLab 거리 0.047 · 색상각 20도 차이의 이웃이다
   * (정렬 전 0.086 · 179도). 남은 논거는 첫째 겹 하나이고, 그것으로 충분하다 —
   * 이 자리의 질문은 「무슨 색인가」가 아니라 「누구의 뜻인가」였다. 두 값이
   * 갈라져 있는지는 `conversationHygiene.test.tsx` 가 계속 잰다.
   *
