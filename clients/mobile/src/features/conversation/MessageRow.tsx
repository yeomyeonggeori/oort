 ...
 * `accent` 는 그 시점에 파랑이라 이름으로는 대응조차 되지 않았고, 어느 한쪽
 * 팔레트를 손대는 날 경계가 조용히 갈라질 자리였다.
 *
 * 그리고 실제로 두 번 손댔다: #1155 가 폰 다크의 accent 가족을 웹 다크 항(#f0a850)
 * 으로, #1164 가 `warn` 을 웹 다크 항(#d4a72c)으로 정렬했다. 이름으로 대응했더라면
 * 이 줄이 그날 파랑에서 호박으로 **뜻 없이** 옮겨갔을 것이다. 역할로 대응했으므로
 * 이 줄은 두 번 다 자기 역할을 지킨 채 값만 따라갔다.
 *
...
const buildStyles = (color: Palette) => StyleSheet.create({
   ...
   *
   * ## 왜 `accentSurface` 가 아닌가 (색 선택 근거)
   *
   * 웹의 대응물은 `bg-accent-soft` 이고, 그래서 첫 번째 유혹은 폰의
   * `accentSurface` 를 쓰는 것이다. **그것은 이름을 옮긴 것이지 뜻을 옮긴 것이
   * 아니다.**
   *
   ...
   *
   * 폰의 accent 가 맡고 있는 뜻은 **다른 가족**이다 — 보내기 버튼과 **내 반응 칩**,
   * 즉 **「내가 한 것」**. u44 직전 리뷰가 인용 레일에 대해 이미 같은 판정을 내렸고
   * 그 단정이 테스트로 남아 있다(`conversationVisual` H-2: *"accent 는 이미 보내기
   * 버튼과 내 반응 칩의 뜻이고, 세 번째 뜻을 갖게 두지 않는다"*). 착지 틴트에
   * `accentSurface` 를 쓰면 방금 점프해 온 행이 **내가 반응한 행**의 옷을 입는다.
   *
   * 이 판정이 처음 쓰였을 때 그 논거는 두 겹이었다 — 뜻이 다르고, 색도 달랐다
   * (폰 accent 는 파랑 #3b6fd4). #1155 가 폰 다크의 accent 를 웹의 호박(#f0a850)
   * 으로 정렬하면서 **둘째 겹은 사라졌다**: `accentSurface`(#33261a)와
   * `warnSurface` 는 그때 OKLab 거리 0.047 · 색상각 20도 차이의 이웃이 됐다
   * (정렬 전 0.086 · 179도). #1164 가 `warn` 가족까지 웹으로 옮기면서 둘은 **더**
   * 가까워졌다 — 거리 0.023 · 색상각 27.8도(`warnSurface` #292415). 남은 논거는
   * 첫째 겹 하나이고, 그것으로 충분하다 — 이 자리의 질문은 「무슨 색인가」가 아니라
   * 「누구의 뜻인가」였다. 두 값이 갈라져 있는지는 `conversationHygiene.test.tsx` 가
   * 계속 잰다.
   *
   ...
   * ## 값이 옳은지 (실측)
   *
   * `warnSurface`(#292415) 는 앱 배경(#17161a) 위에서 **1.16:1**. 같은 팔레트의
   * 다른 부드러운 단들과 한 계단이고(`dangerSurface` 1.19 · `okSurface` 1.21),
   * 웹의 착지 틴트가 자기 표면(`--surface`) 위에서 갖는 값(1.23:1)과 같은
   * 자릿수다. 즉 이것은
   * **띠가 아니라 물듦**이다 — 고도(`surface`, 1.10:1)로 오해되지 않을 만큼
   * 색을 갖되, 행을 카드로 만들지 않는다.
   *
   * 그 「같은 자릿수」가 우연이 아닌 것도 #1164 부터다. 배경이 웹 `--surface` 로
   * 옮겨가면서 여섯 상태 채움·테두리도 **배경 대비 같은 걸음**을 들고 따라갔고,
   * 그래서 이 세 값의 배경 위 세기는 정렬 전과 같다. 안 따라갔다면 배경만 밝아져
   * 물듦이 고도보다 조용해졌을 것이다(1.08 대 1.10 — 뜻이 뒤집힌다).
   *
