...
final class MomoChannelChromeTests: XCTestCase {
    ...
    func testSearchAndDownloadCopyDescribeCurrentCapabilities() {
        ...
        XCTAssertTrue(korean.downloadsScopeNote.contains("첨부파일"))
        XCTAssertTrue(english.downloadsScopeNote.contains("Chat attachments are saved"))
        XCTAssertTrue(korean.downloadsSubtitle.contains("최근 이력"))
        XCTAssertTrue(english.downloadsSubtitle.contains("recent history"))
