...
// ---------------------------------------------------------------------------
// Local harness detection (ADR-0190 D3-a·D3-b, #2813)
// ---------------------------------------------------------------------------

/// Every production source file of the shell: `src/*.rs` minus this test-only
/// file, each cut at its unit-test module (`#[cfg(test)]` + `mod tests`;
/// `lib.rs` also carries `#[cfg(test)] mod shell_contract;` mid-file). Read from disk so a module
...
fn only_the_command_table_reaches_the_harness_status_probe() {
    for (name, src) in production_sources() {
        ...
        for needle in [
            ...
            let allowed = usize::from(name == "lib.rs" && needle == "detect_local_harnesses");
            assert_eq!(hits, allowed, "{name} mentions {needle}");
        }
    }
    let status = include_str!("harness_status.rs");
    assert!(status.contains("pub async fn detect_local_harnesses() -> Vec<LocalHarnessProbe>"));
