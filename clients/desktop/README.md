...
| `detect_local_harnesses` | — | `{ id: "claude" \| "codex", installed: boolean, auth: "logged_in" \| "needs_login" \| "unknown" }[]` | Finds `claude`/`codex` on the inherited PATH plus fixed install folders (no login-shell probe) and runs only `claude auth status` / `codex login status` by absolute path, no shell, fixed arguments, 6 s limit each (in parallel). Exit code only: 0 = logged in, non-zero = needs login, missing/failed/timed out = unknown. stdout/stderr are discarded, credential files are never opened (ADR-0190 D3-a·D3-b, #2813). Desktop only. |
