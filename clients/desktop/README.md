...
| `detect_local_harnesses` | — | `{ id: "claude" \| "codex", installed: boolean, auth: "logged_in" \| "needs_login" \| "unknown" }[]` | Finds `claude`/`codex` on the inherited PATH plus fixed install folders (no login-shell probe) and runs only `claude auth status` / `codex login status` by absolute path, no shell, fixed arguments, 6 s limit each (in parallel). Exit code only: 0 = logged in, the CLI ran and exited non-zero = needs login, missing/failed to start/126·127 (did not run)/signal/timed out = unknown. stdout/stderr are discarded, credential files are never opened (ADR-0190 D3-a·D3-b, #2813). Desktop only. |
...
| `pty_spawn` | `{ request: { program: {kind:"shell"} \| {kind:"harness", id:"claude"\|"codex"\|"grok"} \| {kind:"login", id:"claude"\|"codex", method:"browser"\|"device"}, cwd?: string, cols, rows }, onOutput: Channel<ArrayBuffer>, onExit: Channel<PtyExit> }` | `number` (session id) \| error | Local terminal lane (ADR-0190 D1·D2, #2772). `shell` = `$SHELL -l` (must be in /etc/shells); `login` = one row of `pty.rs` `LOGIN_COMMANDS` (ADR-0190 D3-f, #2816): `claude auth login --claudeai`, `codex login`, `codex login --device-auth` — no other method, no `cwd` (home only); the page renders the output only when the person opens 「터미널로 보기」 and never parses, stores or logs it; `harness` = that CLI resolved by `harness_path.rs` (inherited PATH + fixed install folders; nothing is run to find it, #2813) — the same lookup `detect_local_harnesses` uses. No path or argv from the webview (unknown fields are refused). `cwd` must be a directory inside home (default home). `cols` 2–1000, `rows` 1–500, at most 32 sessions. The environment drops the account/endpoint variables in `harness_path.rs` `STRIPPED_ENV` (ADR-0191 D2's four plus the #2824 review's additions), sets `TERM=xterm-256color`, and `LANG=en_US.UTF-8` only when no locale is set. Output bytes arrive raw on `onOutput` and **must be acknowledged** with `pty_ack`: after 1 MiB unacknowledged the shell stops reading the PTY and the child blocks. `onExit` fires once with `{ id, code: number\|null, signal: string\|null }`. When the shell exits, whatever it left in its session is ended too — unlike Terminal.app, `nohup cmd & exit` does not survive; a process that starts its own session (`setsid`, a tmux server) does. Every `pty_*` command except `pty_write` is async (never on the main thread); `pty_write` is sync on purpose so input keeps its order (it only enqueues) and granted only by `capabilities/pty.json`. A (re)load of the main page ends every session. Desktop only. |
| `pty_write` | raw bytes (invoke body) + header `x-oort-pty-id` | `void` \| error | Keystrokes/pastes, ≤ 1 MiB per call. Calls reach the child in the order they were invoked (no need to await one before the next). Queued for the session's writer thread, never written inline. When 4 MiB are already waiting (the child is not reading its input) the write is refused whole with an error starting `busy` and nothing of it is sent. Rejects a JSON body. Needs the `ipc:` transport (same CSP note as `open_pdf_attachment`). |
