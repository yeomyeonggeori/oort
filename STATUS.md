...
- migration 009는 workspace root에 `ENABLE/FORCE RLS`와 exact `app.workspace_id` policy를 추가했다. public join의 invite hash→workspace UUID lookup은 `momo_join_private` locked schema의 fixed-search-path `SECURITY DEFINER` 함수 하나로 제한하며 PUBLIC/worker/relay/platform 호출을 거부하고 app 역할만 USAGE+EXECUTE를 받는다. migration이 runtime role보다 먼저 실행되는 fresh production 순서에서도 `bootstrap_roles.sql`이 app grant와 relay/worker denial을 재확정하며, ephemeral PG18 verifier가 roles absent→migrate→bootstrap→app allow/relay·worker deny를 고정한다. 일반 public-schema function grant 이후에도 ACL은 재확장되지 않는다.
- 설정은 1-80자 validation, owner/admin 전용 이유, conflict/permission/connection별 한국어·영어 오류를 제공한다. no-cache load 실패도 sidebar에 keyboard(`⇧⌘R`)/VoiceOver 가능한 retry를 노출하고, counter/validation/save는 같은 trimmed 문자열을 사용한다. `Workspace`의 이전 cache 형식은 `updatedAtMs=0`으로 호환 디코딩한다. 전체 Swift 테스트는 Core 24·Server 79·Relay 2·Worker 29·macOS 226, 총 360건 0 failure로 통과했다.
...
- review-fix worker gate: focused race/cache/copy/draft 5 tests + server contract 1 test + increased-contrast/large-text raster 2 tests PASS, fresh-order role bootstrap verifier PASS, 최종 full `runtime-db` 30/30 PASS(`20260714T221124Z-…-r584776886194`), launch 포함 `macos-ui` PASS(`20260714T215608Z-…-r2c4b9fcb68c3`). PR #389 draft의 momo-main final rereview/merge만 남았다.
