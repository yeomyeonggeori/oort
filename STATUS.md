...
- 코어·웹 전체 스위트, tsc, lint, build, 디자인 프리플라이트는 green. `runtime-unverified(real server/outbox round-trip and browser visual)`: #1717 실서버 왕복은 오케스트레이터 몫이고, 로컬 캡처는 Chromium Mach-port 거절과 인앱 localhost 권한 거절로 시작하지 못했다.
