...
- 설정은 1-80자 validation, owner/admin 전용 이유, conflict/permission/connection별 한국어·영어 오류를 제공한다. no-cache load 실패도 sidebar에 keyboard(`⇧⌘R`)/VoiceOver 가능한 retry를 노출하고 semantic primary text로 고대비를 보장한다. settings는 streaming `ChatViewModel` 대신 좁은 projection만 관찰하며 counter/validation/save는 같은 trimmed 문자열을 사용한다. 전체 Swift 테스트는 Core 24·Server 80·Relay 2·Worker 29·macOS 232, 총 367건 0 failure로 통과했다.
...
- final review fix는 delayed login→clear 및 overlapping A/B connect generation, normal/error realtime resubscribe cleanup, workspace identity+channels guarded parallel bootstrap, one-query workspace membership read, narrow settings invalidation을 회귀 테스트로 고정했다. focused 신규 macOS 6 + server 1, high-contrast/large-text raster 2 PASS. full `runtime-db` 30/30 PASS(`20260714T224621Z-…-r7f86c3c71502`), 실제 window launch 포함 `macos-ui` 20/20 PASS(`20260714T224918Z-…-rfd90ac91063d`). PR #389 draft의 momo-main final rereview/merge만 남았다.
