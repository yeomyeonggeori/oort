...
- migration 009는 workspace root에 `ENABLE/FORCE RLS`와 exact `app.workspace_id` policy를 추가했다. public join의 invite hash→workspace UUID lookup은 `momo_join_private` locked schema의 fixed-search-path `SECURITY DEFINER` 함수 하나로 제한하며 PUBLIC/worker/relay/platform 호출을 거부하고 app 역할만 USAGE+EXECUTE를 받는다. 일반 public-schema function grant 이후에도 이 ACL이 재확장되지 않음을 runtime verifier로 고정했다.
...
- review-fix worker gate: focused race/cache/copy/draft 5 tests + server contract 1 test + increased-contrast/large-text raster 2 tests PASS, `verify_rls.sh`/`verify_channel_management.sh` PASS, full `runtime-db` PASS(`20260714T215241Z-…-r237a4a7b4af9`), launch 포함 `macos-ui` PASS(`20260714T215608Z-…-r2c4b9fcb68c3`). PR #389 draft의 momo-main final rereview/merge만 남았다.
