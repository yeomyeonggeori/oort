...
- MOMO-001 이전에는 런타임 e2e가 미검증이었으나, 현재는 아래 Runtime Gate에서 compose/migrate/server health/seq gapless, relay→Centrifugo publish 왕복, RLS 테넌트 격리까지 검증됨. hermes SSE는 후속 M1 goal에서 확인 필요.
...
- 남은 runtime-unverified: MOMO-004(AgentWorker↔hermes SSE + 비용 reserve/reconcile).
...
- 남은 runtime-unverified: MOMO-004(AgentWorker↔hermes SSE + 비용 reserve/reconcile), WebSocket live subscribe/presence/recovery 세부 UX.
...
- 남은 runtime-unverified: MOMO-004(AgentWorker↔hermes SSE + 비용 reserve/reconcile), WebSocket live subscribe/presence/recovery 세부 UX, APNs.
...
> **남은 runtime-unverified:** hermes SSE, 비용 회계, WebSocket live subscribe/presence/recovery, APNs.
