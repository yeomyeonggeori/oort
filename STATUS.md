...
- 별건 보고(이 PR 범위 밖): `clients/web-legacy/src/api/schema.d.ts`가 스펙과 이미 어긋나 있다 — 생성물에 `/v1` 경로 64개뿐인데 스펙은 101개이고 provider 경로는 아예 없다. `local_gate.sh:855`의 "web generated API types in sync" 단계는 이 변경 이전부터 red다.
