...
- **정직 라벨 `runtime-unverified(input delivery)`.** 서버 절반(발급·창·게이트·`input_enabled`·3경로 닫힘)은 실제 PG18 + 실제 Axum router로 증명했다. 화면 절반은 아니다 — `input_enabled: true`를 읽고 datachannel을 열어 키를 넣어 본 producer도, 반환 시 그 채널을 닫아 본 producer도 없다. 특히 **revoke 절반**은 템플릿 사양의 선언이자 서버의 진술일 뿐이고 이 레포는 producer가 그것을 지키는지 증명할 수 없다. `unverified.inputDelivery`로 명문화했다. TURN 이후 실 E2E는 여전히 미측정.
