...
- **광고한 스키마가 곧 집행되는 스키마다.** `tools/call` 인자는 카탈로그가 게시한 input schema로 `momo-mcp`에서 검증된다 — `additionalProperties:false`(오타 `rootID`가 조용히 unthreaded 게시되던 결함), required/nullability, `minimum`/`maximum`, `minLength`/`maxLength`, `format:uuid`, `enum`. 모든 위반은 adapter의 invalid-arguments와 **byte 동일**한 하나의 응답이라 새 열거 oracle이 생기지 않는다. 음수 토큰 수는 스키마(`minimum:0`)와 기록 경계(`bounded_token_count`) 양쪽에서 거절되어 workspace 사용량 총계를 깎을 수 없다(landed `usage_ledger`의 CHECK 추가는 #1375 hygiene 후보로 남긴다).
