...
- 본문 메시지/대기 행·설정 토글 행·`<summary>`·메뉴 행은 fill-only (`hover:bg-surface-hover active:bg-surface-pressed` 또는 `press-instant-fill`, 변형 없음). 사유: `.press` 의 `:active { scale(0.98) }` 가 본문 드래그 선택을 접고(#1743 B-4) 전폭 행의 채움을 테두리에서 떼기 때문이다. `--surface-pressed` 는 hover 에서 한 단 더 어두운 값이며, 같은 재료 여부는 §2.2 의 **한 쌍**(대비 ≥ 1.05 **그리고** OKLab dE ≥ 0.02)이다 — 색상 가족 면제는 정본에 없다.
...
- 선택 분기는 공유 base 에 `press`(패턴 C). 가드는 하드코드 사본이 아니라 **제품 소스**의 class 목록을 읽어 컴파일한다. `WorkPanel.ScopeButton` 을 R2(선택 분기에 press 없음)로 되돌리면 그 가드가 붉다.
- 설정 토글 행은 `<label>` 이 행 전체이고 릴리스 어디서나 토글된다. `checked` 분기도 같은 hover/press 쌍. 캡처: 체크박스 부모가 LABEL, 행 오른쪽 8px 히트가 LABEL, deadRight ≤ 2px (R3 는 73px 비타깃).
- 텍스트 링크(정본 §2.6: `<a>`/`<button>` 의 렌더가 **글자뿐** — 밑줄 또는 `hover:text-`, **채움과 상자(테두리·배경 상자) 없음**. 패딩·`rounded-*`·터치 타깃만으로는 상자가 아니다)는 `press` 를 안 든다. 밑줄만 있는 8곳은 R3 에서 받은 `press` 를 되돌렸다. 상자 있는 밑줄 컨트롤은 `press` 를 유지.
- Ledger 전수 인구는 **상호작용 요소**(태그/role)이지 `hover:` 키가 아니다. cmdk 행·checked 토글 분기처럼 hover 도 press 도 없는 분기도 센다. N0=**448** · N1=**11**(텍스트 링크 잔량) · interactive-without-press **0** · 천장 **11**. JSX + `createElement` · `.ts`/`.tsx` · 같은 파일·import 클래스 상수 · 조건식 **분기별** 클래스 · `tokens.css` `@utility` hover-without-active. 소비자의 `press` 와 유틸리티 `:active` 는 한 임자. 천장보다 많으면 `hover-only control added at <file:line>`, 적으면 `lower the ceiling to N`.
- 3짝 표면 핀은 `PRESS_TRIPLET_GALLERY`/`INSITU` 와 갤러리 `data-testid` 의 **양쪽 집합 동등**. `settings-row` 를 빼거나 하나를 더하면 붉다(부분문자열 핀이 아님).
- 이관 표면 compiled CSS: `transition-property` 에 transform 포함, outline-color 제외. 스케일은 `.press` 의 `scale(0.98)`. `duration-*`/`scale-*` 리터럴 0. 메뉴 행은 `press-instant-fill`(스케일 없음, `:active { transform: none }`) + `active:bg-surface-pressed`. `DraftRow` 는 링크 하나가 채움+press+전이 목록을 진다.
- 캡처: 갤러리 6 + in-situ 4 × rest/hover/active × 두 스킴, 390 은 라이트·다크 둘 다 in-situ+사이드바 행. hover≠active 는 픽셀 수가 아니라 픽셀당 OKLab dE ≥ 0.01. 레인은 시작과 **중단**에 `press-triplet*`+카탈로그를 지운다 — 중단된 런이 지난 3짝을 증거로 남기지 않는다. 중단 사유가 intro/scroll/timeout 이면 NOTES 에 선재 intro-scroll 플레이크(#2057 N-4)를 적는다.
- NOTES(DS-1): 전폭 행은 채움만이라 `press` 의 `scale(0.98)` 1224px ±12.5px 이동이 없다. 초안·리마인더·세션 목록의 **안쪽** 링크/버튼은 여전히 `press` 를 진다.
- R4 가 고친 R3 거짓 문장: 라이트 pressed 가 hover 색상 가족이라 accent-soft dE 0.0185 를 통과한다 · 모서리 프로브가 AABB 꼭짓점만 재도 clip 을 보증한다 · M-1 가드가 하드코드 사본으로도 산다 · 텍스트 링크가 `rounded-*`/`px-*`/`touch-target` 이면 상자다 · N1=1 · 천장 11 이 hover-only 인구다 · in-situ 핀이 부분문자열이다.
