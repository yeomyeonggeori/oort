...
- 검증(이번 라운드): 격리 PG18(로컬 19432)에서 `linked_devices_conformance_pg` 6 green — 실제 행 잠금 barrier(`pg_stat_activity.wait_event_type='Lock'`로 대기 확인, sleep 순서 추측 없음)로 refresh 선행·revoke 선행·동시 중복 refresh·잠금 admit 시점의 binding 변경 거절(부분 기록 0)을 재현. `device_link_conformance_pg` 8 green. 게이트: `cargo fmt --all --check` · `cargo clippy --workspace --all-targets -D warnings` · `cargo test --workspace` 1346 pass/0 fail.
...
- R3(독립 검수 C 수리): H1 — `logout`도 두 `revoke_token` UPDATE 전에 linked refresh와 같은 `lock_member_session_tokens_by_ids`(id 순 `FOR UPDATE`)를 먼저 잡는다. **검수의 전제는 코드로 반증됨**: `token.id`는 `uuid DEFAULT uuidv7()`(`001_init.sql:335`)이고 access 반쪽이 항상 먼저 INSERT되므로 모든 pair가 `access_id < refresh_id`다 — `ORDER BY id`와 logout의 access→refresh는 같은 방향이라 40P01 순환은 이 스키마에서 형성되지 않는다. 격리 PG18.4 실측 `SELECT bool_and(a < b) FROM (SELECT uuidv7() a, uuidv7() b FROM generate_series(1,5000)) t` → `t`. 수리는 라이브 버그 수정이 아니라 「id 생성기의 우연」을 코드 불변식으로 승격하는 하드닝으로 기록한다. 순서 무보장 sweep(`revoke_member_session_tokens`·`revoke_privileged_session_tokens`)은 범위 밖 — 후속 #2539.
- R3 시험: `refresh_and_logout_race_never_deadlocks` — access 행(=낮은 id) su barrier 위에서 linked refresh 1 + 같은 기기 logout 1을 두 순서로 대기 확인(`pg_stat_activity.wait_event_type='Lock'` + `FROM token`/`UPDATE token`) 후 release. refresh 선행 → refresh 200·logout 200 `alreadyRevoked`·새 pair 2 live·새 access 200; logout 선행 → logout 200(두 반쪽 revoke)·refresh 401 `refresh token already used or revoked`·live 0. 두 런 모두 `pg_stat_database.deadlocks` 델타 0(stdout grep 대신 수치). 사보타주: logout의 잠금 호출 삭제 → `logout must park on the id-ordered session-row lock before revoking` RED, 그때 logout이 실제로 걸려 있던 문장은 `UPDATE token SET revoked_at = now() WHERE token_hash = digest($1::text, 'sha256') AND revoked_at IS NULL RETURNING id`. 같은 사보타주 트리에서 그 단정 2개만 빼면 **나머지 전부 green(데드락 0)** — 40P01 사보타주는 만들 수 없다는 증거이며 꾸며내지 않았다.
- R3 M2(중간 실패 rollback 런타임 증거): 스크래치 `git worktree add --detach`에서 두 번째 `record_session_token_with_device` 직후 `return Err(DbError::Sqlx(Protocol("injected")))` 주입. 정상 트리 `[#2498 M2] status=200 recorded=2->4 live=2->2 presented_refresh_consumed=true binding_moved=true`, 주입 트리 `[#2498 M2] status=500 recorded=2->2 live=2->2 presented_refresh_consumed=false binding_moved=false`. `with_tenant_tx`(`momo-db/src/tenant.rs:77-95`)가 클로저 Err에 rollback 후 원오류를 그대로 올린다(검수 미확인 1 해소). 주입은 커밋하지 않고 시험은 정상 트리 기대값으로 커밋, 스크래치 워크트리는 제거.
- R3 L3·L4: `SELECT_LINKED_DEVICE_PAIR_SQL`에 「실효 가드는 `auth_routes.rs`의 `locked.refresh_id != old_refresh_id` 비교」 주석. L4 — linked refresh의 `device_label` 출처가 `token` 행(`session_device_label`)에서 `device_link_token.device_label`(`locked.device_label`)로 바뀌었다. `consume_device_link_in_tx`가 양쪽에 같은 `device_name`을 쓰므로 현재 값은 동일하고, 라벨 변경 API가 생기면 갈라질 수 있다.
- R3 검증: `cargo fmt --all --check` OK · `cargo clippy --workspace --all-targets -- -D warnings` clean · `cargo test --workspace` 1346 passed / 0 failed / 511 ignored · 격리 PG18.4(19432) `--ignored --test-threads=1`: `linked_devices_conformance_pg` 8 passed, `device_link_conformance_pg` 8 passed, `http_smoke_pg` 7 passed, `client_rewire_smoke_pg` 4 passed.
...
- runtime-unverified: 성재 계정 `terraform apply` → doctor 27/27 · 5432 거부 · 재부팅 보존은 승인 지점 1 대기(이 런에서 apply 없음).
