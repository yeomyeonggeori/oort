...
## ADR-0158 서버 축 — `runId` 서비스 개시 · refine 공지 · 어댑터 스트림의 서버측 닫기 (#1130 W-N, 2026-08-08)
...
- **red proof 3종 전부 실주행 반전 확인.** ① 소유권 체크 제거 → 남의 run에 대한 POST가 403 대신 **201**. ② 검증 블록 제거 → 타 워크스페이스 run이 404 대신 **201**로 테넌트 타임라인에 들어간다(`message.run_id` FK는 워크스페이스 쌍이 없는 전역 FK이고, uuid를 컬럼에 넣는 것은 그 행을 읽는 게 아니라 RLS가 안 잡는다 — 스위트가 검증 없는 경로를 **실제로 실행해** 커밋을 단정한다). ③ 파생 키 검증 제거 → 한 refinement의 재시도가 **두 줄**이 된다.
- **발견된 블로커(반경 밖 · 결정 필요).** `PATCH …/messages/{id}`가 `momo_auth::required_agent_scope` 표에 **없다** — agent bearer는 403이다. Swift 원본에도 없었고 #1152/#1173 conformance는 **사람 로그인**으로 증명해서 아무도 이 질문을 안 했다. 결과적으로 ADR-0158 D6의 "stream 계약 소비"는 어댑터 자기 자격증명으로는 **불가능**하다. 보안 경계 확대라 워커가 임의로 넓히지 않았고, 신규 conformance에 403을 **이름 있는 단정으로 고정**했다(결정이 랜딩하면 그 줄이 먼저 빨개진다).
- **검증.** `cargo fmt --check` green · `cargo clippy --workspace --all-targets -D warnings` 0 · `cargo test --workspace` 실패 0 · 신규 `run_binding_refine_conformance_pg` 3/3(실 PG18+`momo_app` NOBYPASSRLS) · 인접 실DB 스위트 무회귀(`stream_edit` 9 · `stream_message` 6 · `mention_routing` 13 · `agent_run_cancel` 4 · `run_terminal_backfill` 6 · `http_smoke` 3 · `client_rewire` 4 · `gateway_mode` 2) · `verify_openapi_contract_rust.sh` **PASS 55/55** · `verify_merge_tree.sh` 7레인.
