...
## 0b1. MOMO-317 Local Gate Wave 2 — BuildKit cache mount + worktree 공유 Swift 빌드 캐시 (2026-07-07)

- **Docker 빌드 캐시**(`infra/prod/docker/swift-service.Dockerfile`): 빌드 스테이지를 재구성해 (1) 매니페스트만 먼저 복사(`COPY <pkg>/Package.*` — `Package.resolved`는 리포에 커밋 안 될 수 있어 glob으로 optional) → `swift package resolve`를 자체 레이어로 분리(Sources/Tests만 바뀌면 resolve 레이어 캐시 히트), (2) resolve/build RUN에 `--mount=type=cache,target=/src/<pkg>/.build,sharing=locked` BuildKit 빌드 캐시 mount(빌드 간 SwiftPM 산출물 재사용, 동시 같은-패키지 빌드 직렬화로 `.build` 오염 방지), (3) build+산출물 cp를 **한 RUN**으로 합쳤다 — `.build`가 cache mount라 바이너리가 mount 안에 생기므로 같은 RUN에서 `--show-bin-path` 결과를 `/artifact`(일반 레이어)로 cp해야 런타임 스테이지가 본다. `# syntax=docker/dockerfile:1`은 기존 유지.
- `scripts/verify_internal_host_runtime.sh`의 이미지 5개 `docker build` 호출에 `DOCKER_BUILDKIT=1`을 명시(계약 고정 — cache mount는 BuildKit 없이는 파싱조차 실패).
- **worktree 간 호스트 Swift 빌드 캐시 공유**(`Makefile` + `.conductor/setup.sh`): `build`/`test`가 `momo_swift` 헬퍼를 통해 `SWIFT_SCRATCH_ROOT` env가 있을 때만 패키지별 `--scratch-path "$SWIFT_SCRATCH_ROOT/<pkg>"`로 `.build`를 공유(worktree들이 같은 캐시 재사용). **미설정이면 패키지 로컬 `.build`로 기존 동작 그대로 — 순수 opt-in(맨 `make build`는 회귀 없음)**. macOS엔 `flock`이 없어 패키지별 `mkdir` 락 디렉터리로 동시 빌드를 직렬화하고, `SWIFT_SCRATCH_LOCK_WAIT`초(기본 300)까지 대기 후 안 풀리면 패키지 로컬 `.build`로 폴백(교착 없이 항상 진행), `trap`으로 EXIT/INT/TERM 시 락 해제. `.conductor/setup.sh`가 `SWIFT_SCRATCH_ROOT=$HOME/.cache/momo-build`를 `.env.worktree`에 기록 + 디렉터리 생성(공유 캐시 켜려면 `set -a; . .env.worktree; set +a; make build`). 브랜치별 서브키 없이 패키지별 공유 — SwiftPM 증분 빌드가 파일-해시 기반이라 안전(아래 실측).
- **검증(퀵 범위 — 전부 실측 PASS)**: ① Dockerfile `docker build --check`(BuildKit lint) "Check complete, no warnings found"(cache mount + `Package.*` glob 파싱 OK). ② 셸: `bash -n`/`/bin/bash -n`(3.2.57)/`sh -n` — setup.sh·verify script 전부 OK. ③ Makefile: `make -n build` 확장 확인 + `momo_swift` 스텁-swift 유닛(opt-in OFF=로컬 .build / ON=`--scratch-path` 사용+락 정리 / 동시 2빌드 SERIALIZED+락 정리 / 선점 락 3초 초과 시 로컬 .build 폴백). ④ 실 툴체인(Swift 6.3.2, DEVELOPER_DIR=Xcode) `make build/test SWIFT_PKGS="clients/Core"`: OFF 정상, ON 콜드→웜 2회차 `Build complete! (0.09s)` 재컴파일 0(증분 캐시 재사용), `make test` 18 tests green, **ON 모드에서 worktree `clients/Core/.build` 미생성(산출물은 공유 scratch에 격리 — 누출 없음)**. ⑤ dirty-state 누출 없음 실증: 공유 scratch에 build 후 소스 1파일 변경(브랜치 전환 모사) → 재빌드 시 **변경 파일만 재컴파일**(stale 재사용 없음), 원복 후 clean.
- **정직 표기(honest gaps)**: (a) Docker 2회차 캐시-히트 **시간 단축 실측**은 콜드 release 컴파일(`swift build -c release --static-swift-stdlib`, OutboxRelay)이 퀵검증 창(≤10분/명령) 안에 완주하지 못해 **SKIPPED(runtime-unverified: 2x-build 시간 측정)** — 대신 Dockerfile 정합성은 `docker build --check` BuildKit lint PASS로, 캐시 재사용 원리는 호스트 SwiftPM 증분(웜 0.09s·재컴파일 0)으로 간접 입증했다. 콜드→웜 실측은 host-runtime 풀게이트(이미지 5개를 반복 빌드)에서 자연히 잡히므로 (b)와 함께 Fable 후속에서 닫는다. (b) 수용기준 ③ `local_gate.sh --profile host-runtime` PASS(웜/콜드 각 1회)는 이번 웨이브 **역할 분리상 Fable(momo-main)이 풀게이트 배비싯** — 여기선 **SKIPPED(runtime-unverified: host-runtime 풀게이트는 Fable 후속)**. (c) 이 worktree는 실제로 `64eb23f`(재설계 머지 전)에서 분기돼 있어, 대상 파일들을 `main`(6e01142) 버전으로 맞춘 뒤 편집했다(Wave 1/MOMO-316 위에 clean 3-way 머지 — reviewer note 참조).
