...
| Day-2: replace the image (Update / new digest) | `scripts/oort upgrade` (`--to <ref@sha256:…>` or `--manifest URL` or `--local-build`). Backs up first, refuses missing env/volumes, waits for `IDEMPOTENCY_OK`, then doctor PASS. Prints a rollback command on failure; never auto-rolls back; never `down -v`. |
| Day-2: take or restore a dump | `scripts/oort backup` (`--out DIR`) and `scripts/oort restore <dump>`. Restore refuses a non-empty stack. Calls `scripts/self_host_pg_dump.sh` / `self_host_pg_restore.sh`. |
