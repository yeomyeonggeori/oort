...
**아직 로그인은 안 된다.** 패키징된 앱은 `tauri://localhost` 에서 REST 를 교차 오리진으로 부른다. 서버 쪽 해법(CORS 오리진 allowlist, MOMO-605 / #768)은 `main` 에 이미 있지만 이 브랜치의 베이스(`track/uxui` P2)에는 없다. 그래서 위 `0.1.0-next.*` 빌드들은 설치와 자기 갱신은 깨끗하게 되지만 momowebqa 로그인은 아직 못 한다. **테스터 초대는 그 머지 뒤에** 하는 것이 맞고, 업데이터는 그것과 무관하게 이미 동작한다.
