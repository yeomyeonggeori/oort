...
runner는 기본 검증 경로에서 headless `MomoMacSmoke`를 실행한다. 사용자가 스택을 직접 띄울 때는
기본 고정 포트 flow인 `scripts/momo start`를 쓴다. runner가 생성한 `summary.md`의
`MOMO_SERVER_BASE_URL`/`MOMO_CENTRIFUGO_WS_URL`/`MOMO_LOGIN_*` 환경변수 예시로 macOS 앱 창을
열던 경로는 **삭제됐다**(W-S1 / #1215 — 6장). 창을 봐야 하면 웹 표면
(`npm --prefix clients/web run dev`)이 그 자리를 대신한다.
...
웹 서빙만 검증할 때는 아래 infra profile을 사용한다. 실제 `f399e417:clients/web-legacy` build(ADR-0119 v0, #2166에서 삭제)를 `web-init`이 named volume에 복사하고 LinkShort와 prod Caddyfile을 HTTP로 구동하며, 호스트 curl로 `/join` SPA 폴백과 `/i/*` 프록시를 포함한 8개 단정을 수행한다. 포트는 28070~28074만 사용한다. 로컬 내부 CA를 만들지 않기 위해 HTTPS는 의도적으로 제외하며 공인 DNS·ACME·production TLS와 초대→가입→메시지 실왕복은 별도 orchestrator gate다.
...
## 6. macOS 클라이언트 (데모 surface: D / B / C) — ⛔ **삭제됨**

> **이 장의 트리는 삭제 대기가 아니라 이미 삭제됐다** — `clients/macOS`·`clients/iOS`·`clients/Core`는
> W-S1(#1215)에서 사라졌고, 그것들을 띄우던 `scripts/macos_dev_run.sh`와 게이트 레인 <!-- docs-cmd-ignore: 삭제된 실행체를 "삭제됐다"고 말하기 위해 이름을 부르는 문장 -->
> `local_gate.sh --profile macos-ui`도 함께 사라졌다. 아래에서 그 명령들을 지운 이유가 그것이다: <!-- docs-cmd-ignore: 삭제된 게이트 레인을 "사라졌다"고 말하기 위해 이름을 부르는 문장 -->
> 존재하지 않는 실행체를 코드블록에 남겨두면 다음 사람이 그대로 붙여넣고 `command not found`를 만난다
> (#1525 — 게이트 `scripts/check_docs_commands.py`가 이제 이 문서를 그 기준으로 검사한다).
> 현행 제품 표면은 `clients/web`(React/Vite) · `clients/desktop`(Tauri 2, 같은 웹 번들) · `clients/mobile`(React Native)이며,
> 개발 실행은 `npm --prefix clients/web ci && npm --prefix clients/web run dev`다.
> 삭제된 절차의 원본이 필요하면 `git log --diff-filter=D -- clients/macOS scripts/macos_dev_run.sh`가 가리킨다. <!-- docs-cmd-ignore: 삭제된 경로를 인자로 받는 것이 목적인 git log — 경로가 없어야 정상이다 -->

macOS 패키지(`clients/macOS`)는 v0에서 **SwiftUI 라이브러리 + 빌드검증 smoke 실행파일 +
SwiftPM 개발용 window 앱 + 릴리스용 Xcode thin host app**으로 구성됐다. 데모 경험
**D(Live Tool-Call) / B(비용 호흡) / C(승인 인박스)**는 `MomoMacDevApp`과 Xcode host
`MomoMac.app` 모두에서 같은 `MomoMacRootView`로 확인할 수 있었다. 아래 설명은 그 시절의
구조 기록이고, **오늘 실행 가능한 명령은 이 블록이 전부다:**

```sh
# 로컬 dogfood 한 줄 실행: 서버가 없으면 mock Hermes 스택을 올린다.
# (macOS 앱이 삭제된 뒤로 이 명령이 여는 것은 스택까지다.)
scripts/momo start
...
- ⛔ **dev run loop는 삭제됐다.** W-S1(#1215) 전까지는 SwiftPM GUI workflow에 맞춘 dev-only
  run loop 스크립트가 `dist/MomoMacDevApp.app`을 만들어 `/usr/bin/open -n`으로 띄우고,
  process/window smoke·unified log·bundle subsystem log·evidence 수집 종료를 플래그로 갈랐다.
  그 스크립트도 그것이 빌드하던 `clients/macOS`도 지금 이 트리에 없다 — 원본은 git 이력에 있다. <!-- docs-cmd-ignore: 삭제된 dev run loop의 형상 기록. 실행체가 없으므로 명령 형태로 적지 않는다 -->
...
- Codex app Run action(`.codex/environments/environment.toml`)이 가리키던 dev run loop도 같은
  W-S1(#1215) 삭제에 포함됐다 — 그 파일의 주석이 이미 그렇게 적고 있다.
...
- 서비스 실행(`MomoServer`/`OutboxRelay`/`AgentWorker`)은 Makefile 타깃이 아니라
  **5장의 `swift run … <Executable>`로 직접** 띄운다. macOS GUI 개발 앱을 띄우는 타깃은
  없다 — 그 트리와 실행 스크립트는 W-S1(#1215)에서 삭제됐다(6장).
...
# --- 선택적 수동 UI 확인 ---
# macOS window 경로는 삭제됐다(W-S1 / #1215 — 6장). 웹 표면은:
