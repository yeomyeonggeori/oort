...
> **범위**: 이 문서는 **제품·UX 원칙**이다(알림 예산, unread가 곧 제품, 부팅 예산, 키보드 우선…). 시각 규칙 — 색·간격·타이포·위계·상태·강제 기제 — 의 정본은 [`docs/design-system/README.md` 「오르트 구름」](../design-system/README.md)이다(ADR-0159). 시각 축의 조항이 여기 없는 것은 누락이 아니라 분업이다.
...
| P5 | 온보딩은 제품이 제품 안에서 스스로를 가르치는 것 | 첫 실행 = 에이전트와의 첫 대화(Slackbot 패턴). 활성화 매직넘버 1개를 정해 소유. 셀프호스트에서 첫 대화 상대가 아직 없으면 『첫 에이전트 연결』이 첫 대화의 문이다 | B1, B2, A2, A10 |
