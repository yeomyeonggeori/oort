...
| MOMO-319 | Gate/verifier hardening | Add verifier port/process cleanup, watchdog defaults, and targeted reuse options so repeated dogfood gates do not leak child processes or fail from stale state. | `runtime-db`, `runtime-agent`, `local-alpha` repeated runs |
