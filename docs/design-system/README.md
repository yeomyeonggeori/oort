...
- **크기:** 기본은 16px(`size-4`) **하나다** — 실측 165곳 중 직접 지정 102곳이
  `size-4`, 55곳은 버튼 프리미티브처럼 부모가 SVG 크기를 정하는 계약 위임이고,
  20px(`size-5`)는 레포에 **0건**이다(헤더 상위 액션 — 허들·핀·터미널 — 도 16px).
  두 번째 크기를 들이는 것은 관례 추가가 아니라 이 절의 개정이다. 교체하면서 컨트롤
  상자·패딩·행 높이를 바꾸지 않는다. 12px(`size-3`)처럼 기존의 측정된 기하를 보존하는
  예외는 해당 컴포넌트가 이유를 진다 — **잔량: 오늘 `size-3` 7곳 중 사유 주석 0곳**
  (WorkConsoleRoute·AgentWorkPanel·ObserverTerminal·MentionRoutingBar), 아무 기계도
  이를 재지 않는다(§5.3).
...
| **아이콘** | `iconSystem.test.ts` — 로컬 SVG **예외 목록 전수**(브랜드 3파일 밖 신규 SVG=빨강)와 **정적 named import 형태**만 | ❌ (ADR-0172가 명시적으로 범위 밖) | — |
...
| — | **아이콘 규칙의 나머지 전부**(§2.8) | `iconSystem.test.ts`는 예외 목록과 import 형태 **둘만** 잰다. 16px 단일 기본·`strokeWidth` 재지정 금지·`currentColor` 상속·`aria-hidden`(잔량 57곳/인접 레이블 없는 36곳)·CSS 도형/아이콘 폰트/기능 이모지 회피·`src/**` 아래 SVG 자산 유입 — 전부 무검사다. §2.8이 잔량을 세어 둔 이유다 |
