...
> ADR-0145 B안 Phase 0 산출물 D2. **재작성이 이 표를 위반하면 실패다.** D1(`server-rust.md`) crate 배선 위에서 하드 불변식 각각의 강제 지점 + DB 백스톱 + "깨는 red 테스트"를 확정. red 테스트 규율(oort 전통): **모든 불변식은, 되돌리면 이름 붙은 단정이 실패하는 형태로 증명한다.**
