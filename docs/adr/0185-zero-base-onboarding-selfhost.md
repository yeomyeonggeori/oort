...
- Status: **Accepted** (2026-09-09 기안 Fable → **2026-09-09 성재 결재**: D-A **(3) 절충** · D-B **(b) 시드 유지+덮어쓰기(v0.1.5)**, (a)는 후속 ADR(SH-12z) · D-C **(iii)** — 권고안 그대로 채택. 결재 기록 §10. §8 봉인 파라미터는 SH-12b~e 티켓에서 확정)
- Date: 2026-09-09
- Deciders: 성재
- 증보: 2026-09-26 온보딩 2.0(ADR-0193 Q6·Q8) — §5-2 진행 점 해석, §6 폰 알림 미리 안내 화면 해석. 파일 끝 「증보 2026-09-26 — 온보딩 2.0」 절
...
## 10. 결재 기록
- **2026-09-09 성재**: 「권고안 그대로 Accept」 — D-A: ☐(1) ☐(2) **☑(3)** · D-B: ☐(a) **☑(b, v0.1.5)** · D-C: ☐(i) **☑(iii)**. 질의 문면: 「D-A 절충(필수 『내 워크스페이스·내 이름』 1화면 + 선택 『팀원 초대』 skip 탈출구, 팀 규모는 질문 대신 초대 수로 추론) · D-B v0.1.5는 시드 유지+온보딩에서 이름·프로필 덮어쓰기(시드 제거는 후속 ADR) · D-C 에이전트 0명이면 『첫 에이전트 연결』이 첫 대화 자리를 잇고 첫 에이전트 활성 시 킥오프 재생」. 같은 자리에서 「v0.1.5는 SH-12b·c 랜딩 뒤 발행 준비」 확인.
- 집행: SH-12a #2301 **랜딩 완료**(PR #2308, 09-09) · SH-12b~e 패킷은 결재 직후 발급(이슈 번호는 `docs/planning/CURRENT_STATE.md` 스냅샷 97에 기록) · §8 봉인 파라미터의 planner 기본값: E1 = `PATCH /v1/workspaces/{ws}` 본문 `{name, updatedAtMs}` · E2 = `PATCH …/members/me`에 `handle` 확장(온보딩 창구 + 설정 상시, 쿨다운 없음, 감사 행) · S1 핸들 제안값 = `fallback_handle` 동형 · S2 초대 링크 TTL 24h·개수 1(재발급 가능) · D-C (c2) 훅 = 네이티브 생성 tx **와** hosted `active` 전이 둘 다, 대상 오너 1인 · `no-active-agent` 판정 = 클라 디렉터리 · 완료 마커 키 `oort.onboarding.v1` — 티켓 검토에서 바뀌면 여기 갱신.

---

## 증보 2026-09-26 — 온보딩 2.0: 진행 점과 폰 알림 미리 안내의 해석

- Status: **Accepted** (2026-09-26 성재 결재). 온보딩 2.0 제안서 Q6·Q8의 권장안이다. 결재 인용과 전체 결정은 ADR-0193에 있다.
- 기안: Opus 5.5 worker(#2805)

### §5-2 해석 — 보이는 점은 AI 연결까지 잇는다 (Q8)
- §5-2의 두 단정은 **그대로다.** claim 비밀번호 화면을 뺀 필수 화면은 ≤ 1이고, 온보딩 스텝은 = 2다(`OWNER_ONBOARDING_STAGES` 2칸, `totalSteps_is_2` 동형 단정). 이 시험은 바꾸지 않는다.
- 바뀌는 것은 **보이는 진행 표시**뿐이다. 숫자 카운터(`1/2`·`2/2`)를 점으로 바꾸고, 점 줄은 first-run의 「AI 연결」(기존 「첫 에이전트 연결」 자리)까지 잇는다. claim 경로의 점은 claim·S1·S2·AI 연결 네 개다. 첫 화면과 첫 대화(킥오프)에서는 점을 숨긴다.
- 「AI 연결」은 D-C의 「첫 에이전트 연결」 자리를 바꾸는 것이다. 스텝 수 단정의 대상이 아니다(first-run은 §5-2의 2스텝 밖).
- 구현: #2807(OB2-1 점 컴포넌트), #2811(OB2-5 claim·S1·S2 겉 교체).

### §6 해석 — 폰에는 킥오프·첫 에이전트 단계를 만들지 않는다 (Q6)
- §6 「폰·데스크탑 별도 온보딩 — 하지 않는다」는 **폰에 프로필·킥오프·첫 에이전트 단계를 새로 만들지 않는다**는 뜻으로 읽는다. 폰의 연결은 ADR-0180 phone-link 그대로다.
- 이 해석 아래 폰에 **알림 미리 안내 화면 한 장**을 둔다. 기기 연결(확인 번호) 뒤, iOS 시스템 알림 창 앞이다. 목표 A는 실기기 푸시가 필수이고(ADR-0187), iOS는 이 창을 한 번만 띄운다. 버튼과 문구는 ADR-0193 D8(Apple HIG에 맞춘 [계속] 한 버튼)을 따른다.
- 폰 첫 화면을 QR 주인공 화면과 보조 경로(초대 링크·주소 로그인)로 나누는 것은 같은 연결 경로의 겉 교체다. 새 단계가 아니다.
- 구현: #2819(OB2-13 폰 첫 화면), #2820(OB2-14 알림 미리 안내).
