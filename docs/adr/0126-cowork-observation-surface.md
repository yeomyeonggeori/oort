...
1. oort의 기존 설계는 이미 절반을 달성했다: **세션=채널 스레드**(0114 D2)라 spawn 승인·중간보고·개입 입력·결과 발췌가 전부 팀 공개 원장에 남는다. "에이전트 작업 과정이 개인 블랙박스"가 되는 구조적 문제는 없다.
...
- B — 채널 공개 스트림 프록시(서버가 stdout을 채널로 중계): oort 서버 raw 비경유 불변식 위반. **기각.**
