...
경계: **외부 런타임(MOMO-536 A2A 카드 에이전트 등)은 oort가 프롬프트를 통제하지 못하므로 D4가 닿지 않는다** — 그들에 대한 방어는 D3의 구조 가드(depth·G2)가 유일하다(buzz 교훈 "한 런타임의 좋은 행동은 운"의 oort 대응이 정확히 이 지점).
