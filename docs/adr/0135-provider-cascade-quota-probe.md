...
- Status: **Proposed** (2026-07-26, Fable 초안 — 방향은 성재 2026-07-25 결정 큐 승인)
...
3. OAuth 잔여량은 ADR-0004와 정면 충돌해 보이지만, **조회 주체를 옮기면 충돌하지 않는다**: 자격증명이 이미 있는 곳(hermes adapter/호스트)이 프로브하고 momo에는 숫자만 들어온다.
...
- B — 어댑터 내부에서만 폴백(momo 몰래): 감사 공백. **기각.**

### D2. 잔여량 프로브 — 경계 보존형
- **A (권고)**: 프로브 실행 주체 = hermes adapter(자격증명 보유 측). momo에 신규 ingest: `POST /v1/provider/quota-snapshots` `{provider_ref, window(short|weekly), remaining_ratio, resets_at, probed_at}` — **숫자만, 토큰·헤더 원문 비유입**(ADR-0004 유지). 저장은 최신 스냅샷만(테이블 1개, upsert).
- 대시보드(사용량 섹션 확장): 창별 2게이지 + `{요일 HH:mm} 리셋` 절대 시각 + 스냅샷 나이 표기, 스냅샷 부재/부실 시 **"마지막 확인값" 폴백**(616 기구현 문법 재사용).
- B — momo 서버가 provider API 직접 조회: 자격증명 유입. **ADR-0004 위반, 기각.**

### D3. hermes adapter 다형화
- adapter 런타임에 provider 어댑터 인터페이스(chat/probe/health) 도입, 체인 구성은 momo가 내려준다(base_url+mode 목록). adapter는 자격증명을 자기 설정에서만 읽는다(기존 경계 그대로).
