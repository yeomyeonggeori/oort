...
- 증보: 2026-09-26 — 하네스 CLI 계정은 ADR-0191(파일 끝 증보 절)
...
## 증보 2026-09-26 — 하네스 CLI 계정
- 하네스 CLI 계정(Claude Code·Codex·Grok 구독)의 전환과 잔여량은 ADR-0191이 정한다. 개념(순서 있는 전환, 한도류만 넘김, 전환 기록, 조용한 전환 금지, 자격 보유 측이 숫자만 보냄)은 이 ADR과 같다. 저장 위치가 다르다. 프로필은 호스트 로컬 폴더이고, 서버에는 프로필 라벨과 쿼터 숫자(host 서명 스냅샷)만 간다(성재 2026-09-26 「전부 권장대로 가자」).
