...
## 증보 1 — 기본 allow-list의 의미 (2026-08-04 집행, 오케스트레이터 판정·성재 luna 지시가 직접 근거)

`allowed_agent_models`는 워크스페이스 설정에 allow-list **키가 아예 없고** `agent.model`이 **실측 카탈로그 id**일 때에 한해, 그 모델의 형제(같은 카탈로그) 모델을 함께 답한다(PR #1004). 설정이 존재하면(빈 배열 포함) 종전 그대로 설정이 전부이고, `hermes-*` 게이트웨이 핸들은 절대 확장하지 않는다. 되돌림은 `mention.rs`의 분기 한 줄. 근거: 설정 쓰기 표면이 없는 서버에서 기본값이 `[agent.model]` 단일이면 모델 선택 UI가 영구 공집합이 된다 — 성재의 "루나=luna 모델" 지시가 이 확장을 직접 요구했다.
