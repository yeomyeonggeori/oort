...
- **권한 모드는 host가 스스로 고정한다(불변식).**
  ...
  - 원격 세션은 프로젝트의 hooks·MCP 서버·allow 규칙을 읽지 않는다. 권한 다리를 우회하는 길이기 때문이다.
    - 예외는 ADR-0192 D3의 「설정 묶음」 하나뿐이다(2026-09-26). 소유자가 데스크탑에서 고른 항목만, 해시 고정, hooks·allow 규칙이 든 항목은 거부, MCP 도구 호출도 권한 다리 경유, 시크릿이 든 항목은 거부. 이 조건 밖은 여전히 읽지 않는다.
...
- **R1 전에는 팀 배포가 없다.** owner 기기 증거 빌드만 있다(§7, M7 I-8). R1 뒤 R2 전까지 폰에서 새 작업을 시키는 일은 Tailscale 임시 경로로 할 수 있다.
...
- **2026-09-23 성재.** 결재 질문 네 개에 한 번에 답했다.
  ...
  - W1: 코드 레인 착수를 승인했다.
- **기안자의 착수 판단.** 팀 배포가 R1을 기다리므로 원격 작업이 가장 긴 경로다. 그래서 R0(#2570)와 R1 첫 조각 `momo-workd`(#2571)를 같은 날 W1 레인과 함께 착수했다.
  - #2571은 뼈대 착수다. **R0 랜딩 뒤에만 머지**한다.
...
**§8.5 증보(2026-09-26, 성재 「전부 권장대로 가자」) — 위 조건의 좁은 완화.** 아래에 적은 완화만 있고, 각 ADR의 red proof가 있어야 연다. 여기 적지 않은 조건 1·2·8·F5와 D6 문장은 그대로 유효하다.
- ADR-0191 D1(A 레인 계정 프로필): 조건 1 「`CODEX_HOME` 하나」 → 「프로필마다 고정 경로 하나, 각각 조건 1 전부 만족」. 조건 8 Claude 목록에 host가 값을 정하는 `CLAUDE_CONFIG_DIR` 추가(상속 금지, 폴더 내용 제한).
- ADR-0192 D3(설정 묶음): 소유자 기기 키로 서명된 목록의 항목만, host 소유 폴더에서. **host가 넣은 서명 항목에 한해** 네 가지를 푼다. (1) Codex: F5의 `.agents/skills` 거부, (2) Codex: 조건 1의 `CODEX_HOME/AGENTS.md` 거부, (3) Claude: 조건 8에 딸린 프로필 폴더 내용 제한(ADR-0191 D1)에서 묶음 링크(skills·`CLAUDE.md`·플러그인)와 `settings.json`의 묶음 MCP 항목, (4) 양쪽: D6 「원격 세션은 MCP 서버를 읽지 않는다」를 묶음 MCP 항목에 한해. hooks·`permissions`·`allowed-tools`·리터럴 env가 든 항목과 MCP·`agents/`·`commands/`가 딸린 플러그인은 거부한다. Codex `plugins` feature 끄기(조건 2)는 그대로다.
