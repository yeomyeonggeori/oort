...
- **권한 모드는 host가 스스로 고정한다(불변식).**
  ...
  - 원격 세션은 프로젝트의 hooks·MCP 서버·allow 규칙을 읽지 않는다. 권한 다리를 우회하는 길이기 때문이다.
    - 예외는 ADR-0192 D3의 「설정 묶음」 하나뿐이다(2026-09-26). 소유자가 데스크탑에서 고른 항목만, 해시 고정, hooks·allow 규칙 제외, MCP 도구 호출도 권한 다리 경유, 시크릿 제외. 이 조건 밖은 여전히 읽지 않는다.
...
**§8.5 증보(2026-09-26, 성재 「전부 권장대로 가자」) — 위 조건의 좁은 완화.** 아래 두 곳만 바뀌고, 각 ADR의 red proof가 있어야 연다. 그 밖의 조건 1·2·8·F5 문장은 그대로 유효하다.
...
- ADR-0192 D3(설정 묶음): 소유자 기기 키로 서명된 목록의 항목만, host 소유 폴더에서. F5의 `.agents/skills` 거부와 조건 1의 `AGENTS.md` 거부를 **host가 넣은 서명 항목**에 한해 푼다. hooks·`permissions`·`allowed-tools`·리터럴 env가 든 항목은 거부한다. Codex `plugins` feature 끄기(조건 2)는 그대로다.
