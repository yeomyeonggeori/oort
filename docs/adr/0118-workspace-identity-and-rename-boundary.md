...
- Public invite join cannot know that context before resolving its bearer code. The sole exception is `momo_join_private.invite_workspace_id(text)`, owned by the migration/database owner in a locked schema: fixed `pg_catalog` search path, static SQL, schema/function PUBLIC revoked, and explicit schema USAGE + function EXECUTE only for the app role. Broad grants on `public` therefore cannot reopen it to relay, worker, or platform roles. Only the exact normalized invite hash may return one active workspace UUID. It returns no workspace or invite row data; all status reads and writes resume under that exact tenant context. This is not a workspace identity API and grants no enumeration or `BYPASSRLS` role.
