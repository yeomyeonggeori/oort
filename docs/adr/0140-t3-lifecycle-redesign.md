...
### D2. 전역 잠금 사다리 — `cloud_host → usage → session → host`

- **A (권고)**: T3 관련 트랜잭션은 **반드시** 이 순서로만 행 잠금을 잡는다. 필요 없는 단계는 건너뛸 수 있으나 **거슬러 올라갈 수 없다.**
  1. `work_cloud_host` 2. `work_host_usage`(+`work_host_usage_interval`) 3. `work_session` 4. `work_host`
- **왜 cloud host가 먼저인가**: 외부 호출에 묶인 것은 **intent**이고 intent는 cloud host 행에 산다. 이 순서에서만 "intent를 재검증한 뒤 종속 상태를 만진다"는 D4의 규칙과 잠금 순서가 **같은 방향**을 본다. 세션 우선 순서에서는 reconciler가 정산을 먼저 하고 intent가 낡았음을 나중에 발견해 abort하게 되며, 3차 리뷰가 우려한 **기아**를 abort 폭풍으로 재현한다.
- **비용은 T3에만 발생한다**: REST 종료 경로가 세션 잠금 전에 cloud host를 조회·잠그는 단계 하나를 얻는다(인덱스 조회 1회). **T1/T2 세션은 `work_cloud_host` 행이 없어 사다리가 세션 단계로 축약되므로 순서가 공허하게 성립한다** — 무료 티어 종료 경로에 추가 비용이 없다.
- B — `session → usage → cloud_host`(현행 다수결, reconciler만 고침): 고칠 코드가 가장 적다. 그러나 위의 abort 비대칭이 남고, "intent 먼저"라는 D4 규칙과 잠금 방향이 반대라 다음 사람이 또 헷갈린다. **기각(비용 근거로만 재론 가능).**
- **규약을 코드가 강제하게 한다**: 잠금은 각 경로가 직접 잡지 않고 **D3의 단일 진입 함수**가 잡는다. 주석에 순서를 적어두는 것으로는 다음 경로가 또 어긴다 — 3라운드가 증명했다.
...
- (−) reconciler를 사다리 순서로 다시 쓰는 비용. 세 경로 중 가장 많이 바뀐다.
