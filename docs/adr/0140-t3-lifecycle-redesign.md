...
- Status: **Accepted** (2026-07-28, 성재 — "전체 승인할게. 너의 추천대로 가자". 기안 Fable, 이슈 #888)
- 파생: #886(선행 — 049 탈출구) → **T-2 안전망**(sweep 분할 + advisory 직렬화 + 동시성 하니스) → **T-3 정본화**(단일 문 + 전이표 + 사다리 A 정렬) → **T-4 수렴**(D4 + reconciler 재작성). 즉시 호스트 인계는 별건(제품 결정 선행).
...
### D2. 경합 직렬화 — **C + A 동시 채택** (성재 결정, 2026-07-28)

> **결정**: 호스트 단위 advisory 직렬화(C)를 **강제 수단**으로 두고, 전역 사다리(A)를 **정본 순서**로 선언한다. 둘은 배타적이지 않다 — C가 교착을 실제로 막고, A가 "다음 사람이 새 경로를 어떻게 쓰는지"를 정한다. **단, A를 선언만 하고 기존 경로를 남겨두지 않는다**(§이행 2) — 선언과 코드가 어긋난 상태가 정확히 3라운드를 만든 원인이다.
>
> **근거(성재)**: "랩탑이 켜져 있는 동안은 로컬 자원, 폰·원격은 T3 — 그 수요가 점차 늘어난다." T3가 예외 경로가 아니라 **주 경로**가 되므로, provider 출발 경로가 깔끔한 쪽(A)의 값이 시간이 갈수록 커진다. C만으로는 아래 §"워크스페이스 축"을 덮지 못한다.

**정본 사다리 (coarse → fine)**

0. `pg_advisory_xact_lock('momo.t3', cloud_host_id)` — **첫 문장**. 둘 이상이면 host id 오름차순.
1. **워크스페이스 축** — `work_pool`, `workspace_credit`
2. `work_cloud_host` 3. `work_host_usage`(+`work_host_usage_interval`) 4. `work_session` 5. `work_host`

**워크스페이스 축을 사다리에 넣는 이유 (실측 — 리뷰 3라운드가 지목하지 않은 축)**

- `CloudUsageLedger.reserveProvisioningSlot`(`CloudUsageLedger.swift:31,45`)은 `work_pool` → `workspace_credit`을 **가장 먼저** `FOR UPDATE`로 잠근다.
- 정산은 `credit_entry` INSERT → `apply_credit_entry` 트리거(045:122-129)로 **가장 마지막에** 같은 `workspace_credit` 행을 잠근다.
- **오늘은 교착이 아니다** — 슬롯 카운트가 잠금 없는 SELECT라 MVCC로 통과한다. 그러나 방향이 정반대인 두 경로가 같은 워크스페이스 행을 쥔다. 잠금 하나만 추가되면 사이클이 성립하고, **이 축은 호스트 단위 advisory(C)로 덮이지 않는다**(서로 다른 호스트, 같은 워크스페이스).
- 워크스페이스당 동시 T3 세션이 늘수록 이 축의 경합 빈도가 올라간다 — **성재가 예상한 수요 증가가 곧 이 축의 압력 증가다.**
...
- **C (채택 — 강제 수단)**: 모든 T3 수명주기 트랜잭션이 **`pg_advisory_xact_lock('momo.t3', cloud_host_id)`를 첫 문장으로** 잡는다. 한 호스트에 T3 트랜잭션이 하나뿐이므로 **행 잠금 순서 논쟁 자체가 사라지고**, 각 경로는 지금 순서를 그대로 둔다.
...
- **A (채택 — 정본 순서)**: "intent를 재검증한 뒤 종속 상태를 만진다"는 D4 규칙과 잠금 방향이 같아 **provider 출발 경로가 가장 깔끔하다**. 비용은 T1/T2가 함께 지나는 REST 종료·sweep을 고치는 것인데, T3가 주 경로가 되는 전제에서는 이 비용을 **한 번** 치르는 편이 맞다. 회귀 위험은 C가 이미 교착을 막고 있는 상태에서 옮기므로 **안전망 위에서의 재작업**이다.
- **B (기각)**: 고칠 코드가 가장 적고 변경이 T3 전용 워커에 갇힌다. 잠금 획득은 쓰기가 아니므로 reconciler는 `session→usage`를 먼저 잠근 뒤 cloud host에서 intent를 재검증해도 **쓰기 전에 abort**할 수 있다(초안이 주장한 "정산 후 abort 폭풍"은 성립하지 않는다 — 정정). 대신 provider 출발 경로가 "무잠금 조회 → 세션 잠금 → 재확인"을 지게 된다.
...
## T3 상시화 전제가 바꾸는 것 (성재 방향, 2026-07-28)

"랩탑이 켜져 있으면 로컬(T2), 폰·원격이면 T3"가 상시 동선이 되면 **지금 예외로 취급되는 경로가 주 경로가 된다.** D1~D5 밖에서 함께 봐야 할 것 셋을 기록한다.

1. **호스트 전환이 예외가 아니라 일상이 된다.** 랩탑을 닫고 폰에서 이어받는 동선은 지금 **offline grace(기본 90초 — `Config.swift:72`) → sweep의 `orphaned` 판정 → resume offer → 계보 재개**를 지난다. 즉 **사용자는 최소 90초를 기다린 뒤에야 폰에서 이어받을 수 있다.** 이건 장애 복구 절차를 일상 동선에 그대로 쓰고 있는 것이다. → **"이 호스트 그만 쓰고 클라우드로 옮긴다"는 명시적 사용자 행동**이 별도로 필요하다(즉시 인계). **제품 결정이라 본 ADR에서 정하지 않고 후속 티켓으로 분리**한다.
2. **정산 트리거가 `orphaned`에 묶여 있다는 사실의 무게가 커진다.** D3가 `reason='orphaned'`를 정본으로 승격하는데, 위 1번대로면 이건 **가장 흔한 과금 종료 사유**가 된다. 드문 장애 경로가 아니라 상시 경로이므로, D5 검증기는 orphan 정산을 **주 시나리오로** 다뤄야 한다(현재는 예외 케이스 취급).
3. **"호스트당 미정산 세션 1건"(049:71-73)이 곧 병목이 된다.** C의 "동시성 손실 없음" 논거는 **이 v0 규칙에 의존**한다. 한 샌드박스에서 여러 세션을 돌리게 되면 호스트 단위 직렬화가 실제 비용이 되기 시작한다(세션 A의 정산이 세션 B의 pause를 막는다). → **다중 세션 허용은 본 ADR 범위 밖**이며, 열 때 **C의 전제를 함께 재검토**해야 한다. 이 결합을 여기 명시해 둔다.
...
- (−) C+A 동시 채택은 **재작성 범위가 가장 넓다** — sweep 트랜잭션 경계(C) + REST 종료·sweep 잠금 순서(A). T1/T2가 지나는 코드를 건드리므로 무료 티어 회귀 검증이 함께 필요하다. 대신 T3가 주 경로가 된 뒤에 다시 하지 않아도 된다.
- (−) T3 비활성 기간이 이행 완료까지 연장된다. **T1/T2 사용자 경험에는 영향 없다.**

## 이행 (Accepted 시)

1. **#886 선행 랜딩**(3단 마이그레이션 + 런북) — 업그레이드 차단 해제.
2. **T-2 안전망** — sweep을 세션 단위 트랜잭션으로 분할 + advisory 직렬화(C) + **동시성 하니스**(두 연결이 동시에 잠금을 쥔 것을 `pg_locks`로 단정). 교착이 실제로 사라지는 지점이고, 이후 재작업이 **안전망 위에서** 일어난다. T1/T2 무회귀 증명 동반.
3. **T-3 정본화** — `t3_terminate` 단일 문 + `settled_at` 트리거 봉인(D3) + 전이표 트리거(D1) + **사다리 A 정렬**(REST 종료·sweep·reconciler, 워크스페이스 축 포함). 잠금 사다리 0~2단은 단일 진입 함수가 소유하고 호출자는 세션 단계만 잡는다 — 그래야 정렬 지점이 한 곳이다. red proof = `deadlock detected`(40P01) + 워크스페이스 축 시나리오.
4. **T-4 수렴** — D4 규칙 + reconciler 재작성 + deadline 강제 + mock provider 정직성(pause 후 실패).
5. **쪼개는 근거**: 초안은 "선언과 코드가 어긋난 창"을 우려해 한 배치를 요구했으나, **T3가 기본 비활성이라 그 창은 사용자에게 닿지 않는다.** 대신 아래 7번을 게이트로 삼아 어긋난 상태로 켜지는 것을 막는다. 한 워커에게 이 전부를 넘기면 검증 못 하는 계층(PG 동시성)에 결함이 몰린다 — 이번 배치 실측 교훈.
6. **T1/T2 무회귀 증명**: REST 종료·sweep을 건드리므로 무료 티어 종료·orphan·재개 경로의 기존 검증기가 **변경 전후 동일하게 green**임을 T-2·T-3 각각에서 보인다.
7. `MOMO_T3_ENABLED` 기본 활성화는 **T-2~T-4 전부 통과 + 실 E2B smoke** 이후 별건 판단. **어느 하나라도 미완이면 켜지 않는다.**
8. **후속(본 ADR 범위 밖)**: 즉시 호스트 인계 동선(§T3 상시화 1 — 제품 결정 선행, 같은 수명주기 코드를 만지므로 **T-3 랜딩 후 착수**) · 호스트당 다중 세션(§T3 상시화 3).
