...
2. **C 먼저, A 그 다음 — 단 같은 배치 안에서.** ① sweep을 세션 단위 트랜잭션으로 분할 + advisory 직렬화 도입(교착이 실제로 사라지는 지점) → ② 사다리 A로 REST 종료·sweep·reconciler 정렬. C가 선다 = ②를 **안전망 위에서** 한다는 뜻이고, 같은 배치에 두는 이유는 **선언(A)과 코드가 어긋난 채로 머무는 창을 만들지 않기 위해서**다.
3. **D1 전이표 + D3 단일 문**을 2와 함께(쪼개면 중간 상태에서 두 규약이 공존해 더 위험하다). 기존 4개 호출자를 전부 `t3_terminate`로 대체.
4. **D4 수렴 규칙** — reconciler 재작성 + deadline 강제.
5. **D5 검증기** — 동시성 하니스는 2·3과 **같은 티켓**에서 나온다(나중에 붙이면 그 사이 green이 무의미하다). **워크스페이스 축 교착**(서로 다른 호스트·같은 워크스페이스)을 별도 시나리오로 포함한다 — advisory가 덮지 못하는 유일한 축이다.
6. **T1/T2 무회귀 증명**: REST 종료·sweep을 건드리므로 무료 티어 종료·orphan·재개 경로의 기존 검증기가 **변경 전후 동일하게 green**임을 같은 배치에서 보인다.
7. `MOMO_T3_ENABLED` 기본 활성화는 **위 전부 통과 + 실 E2B smoke** 이후 별건 판단.
8. **후속(본 ADR 범위 밖)**: 즉시 호스트 인계 동선(§T3 상시화 1) · 호스트당 다중 세션(§T3 상시화 3).
