...
**아직 하지 않는 것:** 데스크탑 업데이터 매니페스트 게시.
- 지금 next 채널(`momo-alpha`)은 누구나 받을 수 있는 공개 URL이다.
- 팀 전용 채널을 둘지, 공개 채널을 M7-S로 볼지는 성재가 정한다(ADR-0187 §4).
- 게시는 그 결정 뒤에 한다.
...
- [ ] **I-3 데스크탑.**
  ...
  - **스테이징 매니페스트**로 업데이터가 한 번 갱신된다.
    ...
    - 스테이징 위치는 배포 채널 결정(ADR-0187 §4)과 함께 정해 이 줄에 적는다.
  - OS 알림을 한 번 받는다.
- [ ] **I-4 iOS.**
  - 실기기에 설치된다.
  - 로그인 또는 QR 기기 연결이 된다.
...
- [ ] **I-5 안정성.** 30분 동안 [ITO-3 시나리오 표](../planning/research/2026-08-20-oss-launch-readiness-and-internal-test-plan.md)(§4)의 I1·I2·I3·I6·I7 행을 웹·데스크탑·폰에서 돈다.
  - 나머지 행의 처리: I4(VM 실화면 관전)는 ADR-0188 전에는 건너뛴다. I5(next 재발행)는 I-3으로, I8(시뮬레이터)은 I-4로 대신한다.
  - 치명 크래시 0.
...
- **owner 본인 기기에만** 설치한다.
  ...
  - 데스크탑: `publish_next_build.sh --public`로 공증하되 업로드는 하지 않고, owner 기기에 직접 전달

**배포 건마다 owner 승인:** PASS가 기록된 뒤에도 팀 대상 TestFlight 업로드와 공증 DMG 전달은 **건마다** 성재 승인을 받는다.
