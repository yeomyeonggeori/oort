...
| `ADR-0119` | 웹 클라이언트 트랙 — 서버 URL=웹 주소, 브라우저 인증/서빙/계약 경계 | Fable | **`drafting`** | 성재 | Proposed draft + option 승인 요청 (웹 우선 — 성재 방향 확정) |
| `ADR-0120` | 푸시 알림 경계 — Dawn 운영 push relay + 서버 notifier | Fable | **`drafting`** | 성재 | Proposed draft + option 승인 요청 |
| `ADR-0121` | 셀프호스팅 배포판·온보딩 — install/upgrade, universal link 초대, BM 경계 | Fable | **`drafting`** | 성재 | Proposed draft + option 승인 요청 |
