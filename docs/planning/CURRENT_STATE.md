...
| `PLN-20260716-01` | 플러그인 플랫폼 제품화 + Slack/MM 호환 표면 | Fable (engine planner) | **`adr-drafted`** | 성재 | 실검증(16-03: **Google 공식 Workspace MCP 존재 확인**·GitHub/Notion/Linear remote 위임 가능)+Slack/MM 호환(16-04: incoming webhook 호환=가성비 1위 검증) 완료 → **ADR-0113 Proposed 기안**(custody A안·GitHub-first+Drive 경로C·Slack-호환 webhook·delegation 4-튜플·egress manifest). 성재 option 승인 대기 — 승인 전 builder issue 금지 |
