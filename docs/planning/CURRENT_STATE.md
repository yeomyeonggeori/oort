...
> 기준일: 2026-07-11 · 기준선: main `0b2c94a` (MOMO-344 merge, verifier 격리 체인 342→344 완료) · 통합 책임: `momo-main`
...
- 현재 구현 체인: MOMO-337(`#307`)/MOMO-338(`#308`) bearer 단일화와 verifier 격리 체인 MOMO-342~344(`#314`/`#316`/`#318`)가 완료됐다. 병렬 착수 가능한 ready goal은 MOMO-339(`#309`, macOS pairing credential UI)와 MOMO-345(`#320`, live channel verifier 격리)다.
...
| verifier 격리 체인 | issue 본문이 패킷 역할 (`#318` 패턴 승계) | MOMO-345 `#320` | `ready` | 339와 병렬 가능 |
...
3. MOMO-345(`#320`)로 live channel verifier를 격리해 root main runtime-agent full gate를 green으로 복구한다 (339와 병렬 가능).
