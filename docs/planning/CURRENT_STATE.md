...
> 기준일: 2026-07-10 · 기준 커밋: `8d97c82` · GitHub 상태 확인: 2026-07-10 09:30 KST · 통합 책임: `momo-main`
...
- 현재 구현 체인: MOMO-337(`#307`)은 PR #310 / `8d97c82`로 완료됐다. MOMO-338(`#308`, adapter)과 MOMO-339(`#309`, macOS)은 선행 의존이 풀려 각각 M1/M3의 `ready`이며 병렬 구현 가능하다.
- 주의: 기준 시점의 root `main` worktree에는 이전 Hermes/local-dogfood 구현 변경이 남아 있다. 기획 작업에서 함께 stage/revert하지 말고 `git status --short`로 매번 확인한다.
...
| ADR-0101 Phase 1 | 같은 패킷 | MOMO-338 `#308` | `ready` (M1) | 2 (339와 병렬 구현 가능) |
| ADR-0101 Phase 1 | 같은 패킷 | MOMO-339 `#309` | `ready` (M3) | 2 (338과 병렬 구현 가능) |
...
2. MOMO-338/339를 서로 다른 worktree에서 병렬로 푼다. runtime 임계경로는 operator login을 제거하는 MOMO-338이므로 먼저 claim해도 된다.
3. MOMO-338은 realtime-first를 유지하고 `/gateway/jobs/pending`을 reconnect/recovery fallback으로만 사용한다. bearer 사용마다 audit row가 생기므로 고빈도 idle polling은 금지한다.
4. MOMO-337 계획 이탈(`/gateway/jobs/pending`이 실제 코드에 없어 actor-bound fallback을 신설)은 accepted implementation deviation으로 기록한다. 각 후속 merge의 `## 계획 이탈`도 `DEVIATION_LOG.md`로 환류한다.
