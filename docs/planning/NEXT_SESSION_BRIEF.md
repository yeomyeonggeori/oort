...
8/9 저녁~8/10 저녁 누계: 구 전선 4 정리 → buzz 진단 5축 → **관문①② 엔지니어링 완결** → **레포 개명(`yeomyeonggeori/oort`)+경량 PR CI 개통** → 배포 창 2(`momo-rust:5671f15d` — 064 발효·웹 임베드·`momo-build` 스탬프 라이브) → **Swift 클라이언트 퇴역(#1253 — 407파일/−10.9만줄)**. 이틀 누계 PR 27 머지. 남은 것은 거의 전부 성재 결정이다.

## 0.5 지금 갈림길 (성재 결정 큐 — 의존 순서)

1. **S10: engine→main 머지 승인** — 관문①② 산출물 전부가 engine에 있고 공개 대상은 main. README Self-host 갱신도 이것에 매달림(SELF_HOST.md가 main에 와야 링크 성립).
2. 공개 시점·형태(Actions 과금 해소 연동) · 공개 범위(planning 266+research 112=378파일) · 레포/org명.
3. **데스크톱 앱 재배포** — 기배포 알파의 자동업데이트가 org 개명 404로 단절(W-O1 적발·레포는 수리됨). `oort.app` 검수 빌드 준비돼 있음.
4. **구 org명 `Dawn-kim-official` 방어 선점**(5분 — 무주공산·제3자 선점 시 옛 링크가 남의 레포로).
5. 기존 잔여: dependabot 13 방침 · TestFlight 전환 신호 · #1254(design-review/taste 스킬 재조준 — .claude/ 접촉) · 5단계(서버 Swift — #1222 선행) 시점. (W-S1·#1234는 8/10 집행 완료)
