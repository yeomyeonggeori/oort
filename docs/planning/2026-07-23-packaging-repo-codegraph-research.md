...
> 발단: 성재 질문 — ① buzz의 "단일 이미지 셀프호스팅" 대비 oort 6이미지의 호스팅 부담 축소 계획 유무와 장단점 ② buzz 인프라/스택/철학/동작 ③ oort 모노레포 유지 vs 오가니제이션 분리 ④ code graph류 도구의 실효성과 적용 계획.
...
- 따라서 oort와의 실격차는 컨테이너 수(oort 상시 ~10 vs buzz 5~6)보다 **"커스텀 이미지 1개 vs oort 6개(api·relay·worker·migrate·web·linkshort)"** 와 owner 부트스트랩/day-2 표면이다. 후자는 Wave H3(560·561✅·562·563)가 이미 대응 중.
...
| buzz | 앱 이미지 1+PG+Redis+MinIO | 5~6 | oort와 동형 스펙트럼 |
...
- 반대 근거 병기: "Is Grep All You Need?"(arXiv 2605.15184) — Claude Code급 하네스에서 그래프 검색 이득 ~2pp. 그래프는 능력이 아니라 **토큰/왕복 경제** 개선. oort의 CLAUDE.md+planning 층이 이미 최상급 "큐레이션된 repo map"이므로 도구는 보조.
...
- **oort 편입 판정(제안): 현시점 비편입 — 관찰 + 얇은 호환 경로만.**
  - 궁합 사실: ADR-0004와 구조적 호환(키 자체가 없음, OpenAI-호환 `localhost:9337/v1`), 핵심 불변식(PG SoT/outbox/Centrifugo)과 무충돌 — 별도 compute plane.
  - 비편입 근거: ①oort 에이전트는 하네스가 지능을 가져오는 모델(Claude/Codex) — 오픈모델 전용 mesh는 현 타깃에 무가치 ②"지루하고 검증된 스택" 포지셔닝과 v0.x 실험 의존성 동봉이 정면 충돌 ③로컬 모델 수요가 실증되면 ollama 지원이 선행 수순(mesh의 가치=팀 공유 풀은 알파 팀 규모에 과잉) ④peer 평문 가시성은 oort의 RLS/채널 격리 서사와 긴장 — buzz 커뮤니티 최대 쟁점(교차 정보 누출)과 동계열.
