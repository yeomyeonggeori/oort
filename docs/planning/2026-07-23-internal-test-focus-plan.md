...
## §1b. 버전 규칙 (성재 2026-07-23)

- 내부 빌드는 **0.0.1부터 시작, patch만 증가**(0.0.1 → 0.0.2 → 0.0.3 …). minor를 올리지 않는다.
- **오픈 베타 전환 시에만 0.1.0.** 이후 오픈 베타 patch는 0.1.x.
- pre-release suffix(-alpha 등) 없이 0.0.x 자체가 내부 단계를 뜻한다. `scripts/publish_alpha_build.sh --version 0.0.N`.
- 폐기: 이전 0.5.0-alpha.1/.2 태그는 이 규칙과 역전이라 삭제됨.
