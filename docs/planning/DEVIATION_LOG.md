...
| 2026-07-17 | MOMO-411/412 게이트 (선재 발견) | runtime-db 게이트의 `MomoWindowChromeSnapshotTests`(dark/narrow/overlay light) 4건이 origin/main HEAD에서 이미 FAIL — UX 트랙 window chrome 최근 머지분의 환경 의존 스냅샷 drift. 411/412(server·tooling만 변경)와 무관. | origin/main HEAD 격리 재현으로 선재 확정. 엔진 트랙 게이트가 이 때문에 무한 대기하지 않도록 서버 표면 단독 verifier로 머지 판정. | `pending` | UX 트랙(GPT/Codex)에 통보 — 스냅샷 레퍼런스 재생성 또는 flake 원인 규명 필요. 엔진 트랙 조치 불요. |
