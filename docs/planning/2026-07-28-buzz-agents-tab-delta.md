...
| memory | **MemoryRoutes 13 endpoints 완비**(list/search/create/update/invalidate, grant 3종, delete-all, policy 2종, external-provider-consent 2종; 027+ADR-0129) | **소비자 0건** — 서버만 있고 UI가 없다 |
| 최근 작업 이력 | `agent-runs`가 **채널 단위** list + run detail. **에이전트별 전역 목록 없음**(CURRENT_STATE에 후속 후보로 기록돼 있던 그 항목) | 인박스·작업 패널(세션 축)뿐 |
| 현재 수행중 | agent.status/partial outbox 투영 완비 | `AgentWorkingRail`→`agentWorkingSignal.ts` 실배선 완료(MOMO-613/#789, PR #794). Sidebar·Composer가 같은 store를 소비하며, WorkPanel은 별도 session-event rail을 사용. 에이전트별 허브 집계 표면은 아직 없음 |
