...
| 3 | **mcp** (inbound·drive) | **없음.** 라우터에 `/mcp` 0. 언급은 스코프 문자열뿐(`momo-auth/src/agent_scope.rs:17` `/v1/mcp/drive`) | **없음(직접 호출 0).** 클라의 `mcp`는 전부 **플러그인 매니페스트의 필드**일 뿐 — `packages/momo-core/src/lib/api.ts:1227`·`:1229`, `clients/web/src/features/plugins/model.ts` | 사용자 눈에 직접 보이는 표면 없음. 다만 plugins(#1)의 매니페스트가 `mcp.tools`/`mcp.url`을 서술하므로, plugins를 살릴 경우 이 패밀리는 그 계약의 뒷면으로 따라온다 | |
