...
> 상태: **계획 확정(2026-09-02 성재 승인 — "권고 전부 승인")·ADR-0179~0182 **Accepted**(2026-09-02 성재 결재 — 정정 2026-09-04: 이 줄이 「Proposed, 결재 대기」로 남아 있었다).** 다음 행동은 §10. 워커 발사는 별도 go 신호.

---


> **정정 2026-09-23 (ADR-0187):** 아래 상자의 G1'·G2 정의는 ADR-0187 D1(목표 A — 팀이 데스크탑·iOS로 매일 쓰는 것 먼저)이 대체한다. 실기기 푸시 보류는 해제됐고(D4), 워커 레인 D-7은 PIPELINE 현행값을 따른다(D7). 셀프호스팅 두 케이스와 외부 셀프호스터 3은 외부 출시 단계의 목표로 남는다.
...
| D-7 | 워커 레인 = **grok 4.6(grok build CLI, grok-fleet 스킬 spawn 계약)** — 2026-09-02 성재 개정(Opus 5 Agent → grok 4.6) · 병렬 상한 2(동시 2기 조기 종료 전례 시 1로 보수) · 리뷰 = design-review(fresh) 폐곡선. 모델명은 레인 뒤에 숨긴다(PIPELINE.md) | Q7 + 2026-09-02 지시 |
