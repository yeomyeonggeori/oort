...
## 4. 성재 결정 큐 — **전부 확정 (2026-08-17 전권 위임 인터뷰 `interview_20260816_200427` 종결)**
1. **발사 확정** — 정지 해제(성재 재개 발화) 후 즉시 착수. 분해=**5a→5b 순차, 5c는 5b 후**.
2. **per-session ephemeral TURN 자격을 5a 범위에 포함**(정적 자격 은퇴가 5a 수용기준). 방식 세부(coturn use-auth-secret HMAC 권장 — 무상태·coturn 네이티브)는 5a 패킷이 권고, 워커 실측 확정.
3. **진입점=LIVE-4 로그인 핸드오프 카드 딥링크 우선**. 관전 블록 오버레이 버튼은 **5b에서 설계만**(구현 여부는 5b 리뷰에서).
4. 재개 게이트: 성재 자유 형태 발화(채팅 충분)·자동 조건 진행 불가.

→ 5a 실행 정본: `handoffs/2026-08-18-live-5a-engine-packet.md`
