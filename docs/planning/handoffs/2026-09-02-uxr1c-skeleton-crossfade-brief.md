...
## 구현 계약
1. `@utility skel`(tokens.css): 스켈레톤과 콘텐츠를 같은 grid cell에 겹치고 콘텐츠 도착 시 `filter: blur(var(--motion-blur-arrival))→0`·opacity 크로스페이드 `--motion-standard`·`--motion-ease-standard`. 제자리 `ready` true→false(`is-resetting`)는 전이 0. 재마운트는 새 인스턴스라 크로스페이드한다. reduced-motion: 즉시 교체. 페이드가 끝나면 막대는 레이아웃에서 빠진다(정착 높이 = 콘텐츠).
2. `SkeletonRows` → 래퍼 `Skeleton`(children=콘텐츠, `ready` boolean)로 승격, 기존 호출부 전량 이관(grep `SkeletonRows`). 로딩 중 높이 보존, 정착 후 콘텐츠 높이.
3. 펄스 없음. 높이 보존 중립 막대는 정지. 크로스페이드가 전부다.
