...
| A-4 | `ready` | **스레드 실전송 완성** | X-3 main 랜딩으로 개방: `Message.thread` 롤업으로 배지 실데이터화, `GET .../messages/:root/replies` cursor로 과거 답글 로드, `thread.updated` 소비(ChatViewModel 기연결)로 열린 패널/배지 갱신. 로드 범위 기반 임시 카운트 제거 | Core `Message.thread`/`ThreadRollupDelta`, replies REST, `verify_thread_projection.sh`가 계약 예시 |
| A-5 | `done` | **허들 UI 폴리시** | 비활성 사유를 포인터·키보드에서 동일하게 설명하고 참가 전/참가 중 상태의 아이콘·레이블·동작을 분리 | MomoHuddle*.swift |
| A-6 | `ready` | **파일 첨부 실업로드 완성** | X-4 main 랜딩으로 개방: 컴포저 실업로드(세션 발급→클라 직송 PUT→complete→`attachmentIds` 전송) + 수신 측 `Message.attachments` 카드/다운로드(기존 content proxy). `DraftMessage.attachmentIds` 가산됨. 업로드 URL은 로그·영속 금지 | AttachmentRoutes.swift, Core `MessageAttachment.swift`, `verify_attachment_upload.sh`가 계약 예시 |
...
| A-10 | `ready` | **Interactive Work Console session card UI (MOMO-485)** | create/active-list/owner-end REST를 host-owned 실행 표면에 연결하고 `kind=work_session` system card의 running/ended props 및 기존 thread를 렌더. Core `WorkSessionDelta`로 같은 card seq의 lifecycle을 갱신하되 command/PTY/cwd/provider credential을 momo 앱·서버에 영속하지 않음 | MOMO-483 (`#516`) track/engine 랜딩: WorkSessionRoutes.swift, Core WorkSession.swift, openapi, verify_work_session.sh |
