...
| A-17 | `ready` | **ACP 세션 카드·도구 프로파일 소비 (MOMO-532)** | MOMO-531의 `MomoLocalACPSession` event를 기존 thread card에 연결하고 plan/진행/immutable 승인 상태를 렌더한다. 승인 카드 결정은 `resolvePermission`으로 왕복하며, 앱 launch spec 하드코딩을 MOMO-533 profile 투영으로 교체한다. | `MomoACPHost`, `MomoLocalACPSession.swift`, MOMO-533 work-tool-profiles, `verify_acp_host.sh` |
