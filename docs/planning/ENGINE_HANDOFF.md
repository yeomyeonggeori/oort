...
| A-11 | `ready` | **로컬 Mac을 work_host로 자기등록 (MOMO-492)** | A-10은 hostId를 로컬 랜덤 UUID로 생성만 하고 서버에 등록하지 않는다. 그러나 MOMO-487이 "control 대상은 등록된 work_host여야 함"을 서버 강제(미등록→404)하므로 **현재 A-10 단독으로는 에이전트 spawn이 라우팅되지 않는다.** 필요 작업: 세션 연결 시 앱이 로컬 Ed25519 신원(생성·0600 저장)으로 `POST /v1/workspaces/:ws/work-hosts`(type=app, scope=member) 자기등록 → 반환 host_id를 hostId로 사용 → (선택) 서명 heartbeat로 online 표시 → host_id를 설정 화면에 노출해 AgentWorker `MOMO_WORK_HOST_ID`와 조율. X-6(auto-approve snapshot) 랜딩 시 함께 소비 | WorkHostRoutes(register/heartbeat·Ed25519 pubkey 필수), MomoWorkConsoleController.MomoWorkHostIdentity, relay/PushRelay(Ed25519 로컬 키 선례) |
