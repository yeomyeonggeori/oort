...
| X-5 | `in-progress`(MOMO-480 track 랜딩, MOMO-481 #512 PR 검토 대기) | A-9 잔여: 수정/삭제/반응 이벤트의 broker version 드랍과 Core 구-seq replay 드랍, history 재진입 시 편집 표시·tombstone 소실 | MOMO-480은 상호작용 4종의 Centrifugo version을 제거해 relay 전달을 복구했다. MOMO-481은 Core에서 4종을 cursor 비전진 projection으로 전달하고 history 세 변형에 durable state/timestamps+tombstone을 투영한다. Core/server tests PASS; Docker verifier는 오케스트레이터, 실 2-client WebSocket E2E는 C-4 후속 | `MessageRoutes.swift`, `RealtimeSubscriptionDriver.swift`, `verify_message_interaction.sh`, `docs/api/openapi.yaml` (2026-07-19) |
