...
| A-16 | `ready`(MOMO-528 #598 track/engine PR) | **불변 Context Packet 서빙 인스펙터 (MOMO-529)** | run/응답 카드에서 저장 packet의 히스토리·memory refs·tool grants·budget·redactions를 읽기 전용 상세로 표시하고 expired 상태를 구분한다. 클라에서 현재 정책으로 재조립하지 않는다. | `GET /v1/workspaces/:ws/context-packets/:packet`, `docs/specs/04-context-packet-v0.md`, OpenAPI |
| A-16 | `ready` | **ACP 세션 카드·도구 프로파일 소비 (MOMO-532)** | MOMO-531의 `MomoLocalACPSession` event를 기존 thread card에 연결하고 plan/진행/immutable 승인 상태를 렌더한다. 승인 카드 결정은 `resolvePermission`으로 왕복하며, 앱 launch spec 하드코딩을 MOMO-533 profile 투영으로 교체한다. | `MomoACPHost`, `MomoLocalACPSession.swift`, MOMO-533 work-tool-profiles, `verify_acp_host.sh` |
