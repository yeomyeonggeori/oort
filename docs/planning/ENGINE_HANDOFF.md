...
| A-21 | `done`(#684 MOMO-568, track/uxui) | **에이전트 작업신호 3종 (buzz Wave U″-1)** | 사이드바 working 배지+경과시간, 컴포저 활동 헤드라인, 턴 liveness. 서버 이벤트 계약 필요 — ADR-0104(presence/typing/streaming) draft를 UXUI 세션이 제안. buzz agentWorkingSignal 단일 모듈 패턴. | 2026-07-22-buzz-actions-plan.md Wave U″ |
| A-22 | `done`(#685 MOMO-569, track/uxui — who-can-talk 서버계약은 X-13 역등재) | **managed-by 표기+수신 게이트 (buzz Wave U″-2)** | 에이전트 카드/인스펙터에 관리 주체 표기, who-can-talk owner-only 기본. ADR-0131 profile 소비 후속. | agent_profile(036), buzz 계획 Wave U″ |
| A-23 | `done`(#686 MOMO-570, track/uxui) | **빈 채널 인트로 'Create agent'='Add people' 동급 (buzz Wave U″-3, 소형)** | 빈 채널 온보딩에 에이전트 생성 진입 동급 배치. | 537 생성 API |
