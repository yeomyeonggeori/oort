...
| A-12 | `done`(MOMO-511-U #555 → main 랜딩 2026-07-21) | **원격 Work 터미널 direct attach (MOMO-511 UXUI 절반)** | exact grant 메모리 소비, capability header, SwiftTerm direct stdout/stdin/resize/kill, 오류별 인라인 재시도, ended read-only, 카드→서랍 진입, 소켓 정리를 구현했다. UI는 remote binding이 명시된 세션만 fail-closed로 연다. 서버 list projection에 식별자가 없어 `remoteAttachAvailable` 또는 `ptyId` read projection(X-8) 랜딩 전 실데이터 액션 노출은 runtime-unverified | ADR-0125 D10, `MomoRemoteTerminalSession.swift`, `MomoWorkConsoleDrawer.swift`, macOS focused tests |
| A-13 | `ready` | **iOS 알림 UX v2 소비 (MOMO-504)** | APNs `thread-id`와 `momo.message|mention|approval|work` category를 등록해 빠른 답장·승인/거부·Work 딥링크를 바인딩한다. `momo.push.notification.v2`의 승인 전용 `approval_id`와 서버 badge를 소비하고, NSE fetch 기반 id-only 본문 경계는 유지한다. | MOMO-503 `PushDispatch.swift`, `docs/PUSH_RELAY_RUNBOOK.md`, `verify_push_notifier.sh` (Goal #550, main 랜딩 2026-07-21) |
...
| X-8 | `in-progress`(#558) | 511-U(맥 원격 attach)가 세션 목록에서 원격 PTY 결속 여부를 사전 판별 불가 — GET work-sessions 응답에 remote PTY 결속 투영이 없어 액션 노출이 `ptyId` 명시 세션에만 fail-closed | #558이 credential-free `remoteAttachAvailable` boolean과 observer projection을 WorkSession 응답에 가산했다. `attachEndpoint`/capability는 계속 비투영(발급 REST 전용). track/engine 랜딩 후 `ready`로 전환한다 | UXUI 발견(2026-07-21, MOMO-511-U #548/#555), MOMO-516 #558, OpenAPI `WorkSession`/`issueTerminalAttachCapability`, TerminalAttachRoutes 결속 원장 재사용 |
