...
| A-12 | `ready` | **원격 Work 터미널 direct attach (MOMO-511 UXUI 절반)** | running remote session owner가 `POST .../work-sessions/:id/terminal-attach`의 exact `{attach_endpoint,capability_token,pty_id}`를 메모리에서만 소비하고 SwiftTerm을 endpoint에 직결. 같은 pty의 connect/send_stdin/resize/kill을 구현하며 만료·revoke·네트워크 실패를 fail-closed UI로 표시. raw PTY와 capability는 MomoServer/relay REST, 로그, UI 상태, 영속 저장소에 넣지 않음. 실제 host PTY adapter는 별도 후속과 조율 | ADR-0125 D10, `TerminalAttachRoutes.swift`, OpenAPI `issueTerminalAttachCapability`, `verify_terminal_attach.sh` (track/engine 랜딩 시 ready) |
