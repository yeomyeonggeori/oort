...
| X-8 | `needs-engine-contract` | 511-U(맥 원격 attach)가 세션 목록에서 원격 PTY 결속 여부를 사전 판별 불가 — GET work-sessions 응답에 remote PTY 결속 투영이 없어 액션 노출이 `ptyId` 명시 세션에만 fail-closed | credential-free boolean `remoteAttachAvailable` 또는 `ptyId`만 WorkSession read projection에 가산. `attachEndpoint`/capability는 계속 비투영(발급 REST 전용). UXUI는 미결속 세션에 액션을 추정 노출하지 않고 fail-closed 유지 | UXUI 발견(2026-07-21, MOMO-511-U #548/#555), OpenAPI `WorkSession`/`issueTerminalAttachCapability`, TerminalAttachRoutes 결속 원장 재사용 |
