...
| X-8 | `needs-engine-contract` | work-session list/read projection에 remote PTY 결속 여부가 없어 UI가 attach 가능 세션을 사전에 정확히 구분할 수 없음 | credential-free boolean `remoteAttachAvailable` 또는 `ptyId`만 WorkSession read projection에 가산. `attachEndpoint`와 capability는 투영 금지. UXUI는 미결속 세션에 액션을 추정 노출하지 않고 fail-closed | MOMO-511-U #548, OpenAPI `WorkSession`/`issueTerminalAttachCapability` |
