...
| A-4 | `ready`(엔진 완료 — B-3 done) | **스레드 실전송 연동** | 컴포저 "스레드 초안"=로컬 초안만(`localDraft` 카피). 서버 `message.root_id`+`thread` 테이블 001부터 기존재 | 로컬 초안 → 실제 root_id 전송/스레드 뷰. 전송 REST의 root_id 개방 여부 선확인(§B-3와 짝) | 001_init.sql:174-205 |
...
| A-7 | `ready`(track/engine) | **검색 서버 승격** | MOMO-475 랜딩 — GET /v1/workspaces/:ws/search/messages(멤버십 필터·seq 커서·snippet+matchOffset) | 로컬 MomoWorkspaceSearchIndex의 메시지 소스를 paged server FTS로 교체(handoff 심: MomoWorkspaceSearchDestination 유지) | SearchRoutes.swift, openapi |
...
| B-3 | `done` → track/engine (main 대기) | **전송 REST root_id 개방 확인/개방** | A-4의 전제. V-1 노트: "필요 시 동티켓 개방" | 메시지 전송 DTO의 root_id 수용 확인, 미개방 시 소형 티켓 |
