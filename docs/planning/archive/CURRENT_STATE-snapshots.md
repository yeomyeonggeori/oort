# 이전 현재 상태 원문 — 2026-09-12 #2501에서 이동

이하 내용은 역사 기록이며 현행 지시가 아니다. 현재 진입점은 ../CURRENT_STATE.md.

# oort 기획 현재 상태 (Planning Current State)

> **2026-09-11 스냅샷 98 (Fable · momo-main — ★v0.1.5 릴리스 완료(태그 803ae7d5·Release·attestation PASS·E2E-B′ 발행 이미지+--claim 12/12) · 밤사이 랜딩: #2433 notifier 롤·#2452 --claim 1급·#2436 E2E-B 문서 이탈·#2443 PITR 시험·#2444/#2460 게이트 정책 배치 1·2·#2461 GRANT 트림 · 워커 1(#2029 → PR #2468 리뷰 중) · 체인 1(v0.1.5 매니페스트 #2469).)**
>
...
> 이하 스냅샷 96:
> **2026-09-09 스냅샷 96 (Fable · momo-main — ★E2E-A 그록봇 폐곡선 PASS(설치→doctor→터널/claim→Agent Port active→멘션 답장→disconnect 잔여 0). ADR-0184 Accepted. ADR-0185 Proposed main 도달. SH-12a #2308 리뷰 PASS·랜딩 중, SH-11f #2307·SH-11g #2310 검토+감사 중. 워커 0 · 검토 2 · 체인 1.)** 컴팩트 복원 진입점.
>
> **★ 정본 헤드**: main `4361cba7`(ADR-0185 Proposed 승격 bj) · uxui `4345ac3c` · engine `43192b1e`. 정합 PASS. **진행 중 체인**: `land-2308.sh`(SH-12a → uxui → main, 배치 bk) → 이 스냅샷 PR → (검토·감사 PASS 시) `land-2307.sh`(SH-11f, bl) → `land-2310.sh`(SH-11g, bm).
...
> **★ 다음**: 체인 완주 확인(`$S/land-2308.log`·`land-2307.log`·`land-2310.log` DONE/ABORT) → SH-11f/g 검토 결과에 따라 랜딩 → SH-11a~e 이슈화 → SH-12b~e(결재 뒤) → **v0.1.5 발행 준비**(CHANGELOG `[0.1.5]`·`releases/latest.json`, 성재 dispatch) → 제로베이스 온보딩 E2E 재실측 → ITO → G2(iOS v0·Railway 최종·APNs).
...
>
> **★ 종합 실테스트(성재 일괄 수행 방침 — #1825 코멘트 정본)**: 패킷 `handoffs/2026-08-28-comprehensive-test-packet.md`. **갭=최신 발행 v0.1.2(8/24)가 테스트 대상 전부 미포함**(VM=발행 이미지 기반). 준비 체인: ①[성재] v0.1.3 발행 창 승인(dispatch+release env, 체인 개시 게이트) ②[grok] **#1837** 문면 현행화(§2-B digest 3자 정합+§6 "읽기 403" 정정 — #1820으로 거짓 판명) ③[Fable] VM 갱신 릴레이(**배선 5종 보존 — CSP connect-src 소실 최고 위험**) ④검수 앱 재빌드 ⑤자격 발급(owner 로그인=성재 몫). 시나리오 S1(허들 2대+soak)~S5(UXUI)·PASS 기준 성문.
>
> **★ 성재 대기**: ①v0.1.3 발행 창 승인 ②owner 로그인+자격 발급(messages:read 명시) ③S1-b 폰 LTE 3분 ④종합 검수 세션(피드백 전량 티켓화) ⑤도어벨 재시험 포함 여부(별도 선결·기본 미포함).
>
> 이하 스냅샷 68:
> **2026-08-26 스냅샷 68 (Opus5 대행 · momo-main — ★승격 창 완결·삼자 정렬 복구 / RA-7·TC-2 착지 / 출시 결함 2건 적발).** 컴팩트 복원 진입점.
>
> **★ 승격 완결**: main=**dafe81b1**, track/engine=52855849, track/uxui=a3f22dfe. 잔여 0, `main ⊂ 두 트랙`. 순서=engine승격(#1771)→uxui sync(#1773)→engine sync(#1775)→uxui승격(#1772, 74커밋/9티켓)→docs(#1776)→sync 짝(#1779·#1780). **#850 웹 허들 착수 가능해짐**(선행 조건 해소).
>
> **★ 출시 결함 2건(TC-2 리서치 부산물, 최우선 후보)**: **#1777** host-signed 세션 변이 미이식 → `remote_attach_available` 항상 false → **방금 랜딩한 TC-1 관전 도크가 붙을 PTY가 원리적으로 없음**(리뷰 4회전이 픽스처 모킹 탓에 못 봄). **#1778** 소유자 관전차단 토글 400(동의 모델 열린 채 잠김). 둘 다 ADR 불요·이식.
>
> **★ RA-7 판정**(`research/2026-08-26-ra7-…md`): state 영속=URL 불변+인증서 재발급 0(성재 직관 확인) · URL 파괴는 state 소실 하나이고 **복구 불가** · **"계정 0개"는 불가**(oort tailnet 수용=ToS 소지+전 고객 서버 접근권) → 사용자 자기 tailnet(터미널 0회·클릭 4~5회) · **무계정+고정URL 부재는 구조** → A트랙=무계정 tier의 유일 구현(TLS 종단 프록시, CNAME 불가) · **진짜 blocker=Funnel WS 1001(#18827), soak 미실측**.
>
> **★ 신규 티켓**: AC-1~4(#1767 패스워드초기화·#1768 멤버 라이프사이클 10경로·#1769 초대revoke·#1770 role 표시명 커스텀) · #1774(정책 감사 3중 요구 마찰) · #1777·#1778.
>
> **★ 성재 결정 대기**: ①**TC-2 원격조작 동기**(부재중 대체/동시작업/지시주입 — 이 답 없이 ADR 기안 불가) ②WS soak 환경(**D8 Funnel 서버 응답 없음**·로컬 tailscale 없음) ③RA-7 D-1(계정 진입장벽 수용=A트랙 강제 여부) ④검수 앱 실물 QA(앱=264bb1dc 빌드 배포됨) ⑤#1777·#1778 우선순위 ⑥도어벨 재시험(벤더+루틴 key) ⑦#1774 정책 게이트 개선 여부.
>
> **★ 가동**: T-2 플레이북 §2 재작성(Funnel 기본·조용한 실패 2종 절차화) 워커 진행 중.
>
> 이하 스냅샷 67:
...
>
> **★ 트랙**: main=engine 승격+uxui 승격 완료(스냅샷 61) 이후 트랙에 추가 랜딩분 — uxui: U-8·#1703·#1719 / engine: #1696·#1717. **다음 승격 창 후보**(성재 게이트). 배포/이미지 재발행 결정도 병행(T-9·보관소·언퍼얼이 셀프호스트 이미지에 실리려면 publish 필요, MOMO_UNFURL_ENABLED 기본 0).
>
> **★ 성재 대기**: ①합류 릴레이 킷 2-paste ②iOS 시뮬레이터/실기 세션 ③다음 승격+배포 창 결정 ④신규 티켓 발사 순번(#1716·#1720·U-8 폰 TypingBar 각주).
>
> 이하 스냅샷 62:
> **2026-08-23 스냅샷 62 (Fable · momo-main — ★일괄 승인 창 마감: 승격 완결·3차 파도 3/3·언퍼얼 1/2 가동).** 컴팩트 복원 진입점.
>
> **★ main 승격 완결**: uxui(#1705)+engine(#1710)+docs(#1704)+sync 짝 4본 — 삼자 정렬. main에 UXUI 파도 전체+T-9+ADR 0166~0170 정본 반영.
>
> **★ 3차 파도 3/3 랜딩**: #1712(로컬 보관소=첨부 503 원천 해소)·#1711(U-8 26px 실측 회수)·#1715(M-2 후속). 신규 적립: #1716(전송 후 0 B — 서버 실측 크기 결정 필요).
>
> **★ 가동 중**: #1698 언퍼얼 서버 절반=grok(feat/1698-adr, base=track/engine). 완료 시 검수→랜딩→ENGINE_HANDOFF ready 행→클라 렌더 uxui 티켓(2/2). 검수 앱 재빌드(콜드) 진행 중 → 완료 시 ~/Desktop/oort-uxui-review.app 교체.
>
> **★ 성재 대기(수동 QA만)**: ①합류 릴레이 킷 2-paste ②iOS 시뮬레이터/실기 세션(M-1·M-2·#1703 indeterminate·U-8 폰 확인 — runtime-unverified 목록은 각 PR 리뷰) ③다음 배포 창 결정(main 랜딩분의 라이브/이미지 재발행 — T-9·보관소가 셀프호스트 이미지에 실리려면 publish 필요).
>
> 이하 스냅샷 61:
