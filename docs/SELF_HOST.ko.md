...
끝 네 자리만 돌아온다. 엔드포인트는 오늘 **외부 `https://`** 주소여야 한다. 셀프호스트
env에서 로컬 provider 허용(`--allow-local-provider`, SH-6a-e)을 켜기 전에는
노트북의 로컬 모델(`http://127.0.0.1:...`)은 거절된다. 거절되면 **설정 › AI 연결**이
그 문장을 자리에 보여 준다. 이 플래그는 클라 스위치가 아니다.
