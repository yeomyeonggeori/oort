# oort 셀프호스트 — clone에서 로그인까지 (SELF_HOST.ko.md)

> **정본은 영문 [`SELF_HOST.md`](SELF_HOST.md) 이다.** 이 파일은 같은 절 번호의 번역이다.
>
...
(도메인을 붙이는 것은 [§공개 오리진으로 열기](#공개-오리진으로-열기)이고, 별도 절차다).
...
끝 네 자리만 돌아온다. 엔드포인트는 보통 **외부 `https://`** 주소다. 서버가 같은 컴퓨터의 주소
(`http://127.0.0.1:...`, localhost)를 거절하면 **설정 › AI 연결**이 무슨 일이
일어났는지와 다음에 할 일을 자리에 보여 준다. 로컬 provider 허용은 운영자의
셀프호스트 설정(다음 절)이지 클라 스위치가 아니다.
...
아니다. hosted pairing 자격은 **설정 › 연결 › 에이전트 자격**에서도 발급한다.
...
역방향 프록시 뒤라면 **`MOMO_PUBLIC_BASE_URL` 도 사람이 실제로 여는 절대 https
오리진**(`https://<host>`, 뒤에 경로 없이)으로 설정한다. API 가 이 배포를 사람에게
이름 붙여야 할 때 이 값을 읽는다 — 지금은 승인 결정이 1회로 돌려주는 초대 링크가
그 자리다(ADR-0186). 설정하지 않으면 그 요청의 `Host` 헤더로 링크를 만든다. 한
사이트가 SPA 와 `/v1` 을 함께 서빙하면 맞고, 아니면 틀린다. 절대 https 가 아닌
값은 **무시되며** 기동 로그가 한 줄로 그 사실을 말한다 — 조용한 폴백은 없다.
