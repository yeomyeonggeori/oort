...
끝 네 자리만 돌아온다. 엔드포인트는 오늘 **외부 `https://`** 주소여야 한다. 이 서버는
같은 컴퓨터의 주소(`http://127.0.0.1:...`, localhost)로 가는 연결을 기본으로
거절한다. 거절되면 **설정 › AI 연결**이 무슨 일이 일어났는지와 다음에 할 일을
자리에 보여 준다. 로컬 provider를 허용하는 것은 서버 운영자의 셀프호스트
설정이지 클라 스위치가 아니다.
