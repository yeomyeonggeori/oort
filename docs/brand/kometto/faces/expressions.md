...
2. **가장자리 매팅.** 캐릭터 가장자리 2px 띠만 알파를 매긴다. 가장 가까운 걷힌 픽셀을 바탕, 가장 가까운 안쪽 픽셀을 전경으로 보고 투영한 비율이다. 띠의 색은 가장 가까운 안쪽 픽셀 색이다(바탕색 역산은 값이 넘쳐 노란·청록 테를 남겨 버렸다). 남색 테·색 테가 없다.
3. **원호 자름 페이드.** 몸통과 혜성 꼬리는 원본에서 배지 원에 잘려 있다. 자른 경계에서 안쪽으로 크기의 10%(웹 57.6px, 히어로 280px에서 28px) 동안 알파를 smoothstep으로 0까지 내린다. 자른 경계 = 캐릭터와 맞닿은 걷힌 픽셀 중 바깥 바탕색에 가깝고(원판이 아니고) 배지 원 반지름 ±3.5% 안인 것. 거리는 그 경계 픽셀까지의 유클리드 거리다(챔퍼 근사는 페이드 띠에 세로 결을 남겨 버렸다).
   - 근거: 원호로 딱 잘린 단면은 투명 바탕에서 「원판이 사라진 배지」로 읽힌다. 몸이 아래로 녹아 사라지면 바닥 위에 선 흉상으로 읽힌다(히어로 캡처).
   - 페이드는 몸 안쪽 색을 바꾸지 않고 알파만 내린다.
- **가시성 실측(design-review, b9c05412).** 다크 바닥: 후드 대비 11.8:1, 전부 선다. 라이트 바닥: 크림 림 1.10–1.15:1, 말풍선 꼬리 1.10–1.16:1, 후드 1.19–1.25:1. **라이트에서 림과 말풍선 꼬리는 거의 사라지고, 실루엣은 검은 얼굴·하늘색 후드·주황 방울·혜성 꼬리가 만든다.** K6 픽셀을 바꾸지 않는 (c) 결정의 결과다. 라이트에서 림이 보이게 할지(외곽선·그림자 등)는 owner 정본 질문으로 남긴다.
- 바닥 위에 테두리·그림자를 더하지 않는다(위 질문이 결정될 때까지).
