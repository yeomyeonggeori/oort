...
| 무엇 | 경로 | 규격 |
|---|---|---|
| 원본(합성 결과) | `docs/brand/kometto/faces/kometto-{id}-{theme}.png` | 1254×1254 불투명, sRGB. K6 플랫 원본과 같은 캔버스 |
...
| 48px | 목록·배너 같은 보조 자리 | 최소 권장 크기 |
| 72px | 온보딩 모든 화면의 머리(D11) | 기본 |
| 96px · 144px | 빈 화면·큰 카드 | |
| 200px · 280px | 히어로: 폰 200, 데스크탑 280 — 첫 화면·완료·첫 대화만(D11) | 웹 에셋 576px = 192px×3x. 280px@2x는 560px라 576 안이다 |
...
- 배지 원이 캐릭터의 바탕을 들고 다닌다. 크림 원(라이트)과 남색 원(다크)이 각 바닥과 떨어져 보이므로 바닥 위에 테두리·그림자를 더하지 않는다.
- `light`는 혜성 꼬리 끝이 원 밖으로 나온다(K6-flat-light 원본 그대로). 알파가 꼬리 모양을 따른다.
...
- 웹 에셋: 576 RGBA·sRGB, 합성물에서 파생(평균 차 ≤ 3, 16 초과 ≤ 2%), 배지 밖 투명, `idle-dark` = S0 배지
...
- 생성 도구: OpenAI 이미지 생성(gpt-image, codex CLI 경유, ChatGPT 로그인). OpenAI 이용 약관상 출력물의 권리는 사용자에게 있다. 생성 원본 PNG에는 C2PA 출처 청크(`caBX`)가 들어 있다. 커밋한 크롭·합성물은 다시 인코딩해 그 청크가 없다. 생성 원본은 저장소에 넣지 않았다.
