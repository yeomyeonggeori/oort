...
API는 현재 DB role이 정확히 `momo_app`이고, superuser도 `BYPASSRLS`도 아닐 때만 부팅을 계속한다. 조건을 만족하지 않으면 오류를 던져 시작하지 않는다. 「Swift 시절 — Rust 현행 재검증 필요」 [근거: `DatabaseSecurityPosture` 20–30행, `f399e417:server/Sources/MomoServer/DB/DatabaseSecurityPosture.swift`] · 현행 RLS 자세 [`momo-db`](../../server-rust/crates/momo-db/)

감사 로그 조회는 인증된 human workspace admin에게만 허용되며, workspace 범위와 시간·행위 등의 filter를 적용해 REST 응답으로 돌려준다. 「Swift 시절 — Rust 현행 재검증 필요」 [근거: `AuditRoutes` 18–85행, `f399e417:server/Sources/MomoServer/Routes/AuditRoutes.swift`] 첨부 업로드 시작·완료·실패도 각각 audit action으로 기록된다. [근거: `AttachmentRoutes` 110–115행 · 182–192행, `f399e417:server/Sources/MomoServer/Routes/AttachmentRoutes.swift`] · 현행 [`attachments.rs`](../../server-rust/bins/momo-server/src/routes/attachments.rs)

### 자격증명과 provider

Codex/OpenAI OAuth authorization/access/refresh token과 provider API key는 oort의 app·API·worker·DB·diagnostics·local gate evidence 경계 밖이다. 외부 provider verifier는 이 환경변수가 oort 검증 프로세스에 전달되면 실패한다. 「Swift 시절 — Rust 현행 재검증 필요」 [근거: ADR-0004 8–15행, 37–44행](../adr/0004-codex-oauth-hermes-provider-boundary.md#L8-L15), [verifier 183–197행, `f399e417:scripts/verify_external_agent_provider.sh`]

### 첨부

업로드 요청은 archive backend가 만든 upload URL을 클라이언트에 반환하므로 업로드 바이트는 capability URL로 직접 전송된다. oort는 세션 생성 전후에 채널 멤버십을 확인하고 pending attachment·감사 기록을 만든다. [근거: `AttachmentRoutes` 60–90행, 119–125행, `f399e417:server/Sources/MomoServer/Routes/AttachmentRoutes.swift`] · 현행 [`attachments.rs`](../../server-rust/bins/momo-server/src/routes/attachments.rs) 완료 시에는 저장소가 보고한 크기·MIME·file ID를 요청 때 저장한 값과 대조한다. [근거: `AttachmentRoutes` 152–209행, `f399e417:server/Sources/MomoServer/Routes/AttachmentRoutes.swift`]

첨부 content 조회는 같은 workspace의 archived 되지 않은 채널에 속한 active member만 통과한다. [근거: `AttachmentRoutes` 371–397행, `f399e417:server/Sources/MomoServer/Routes/AttachmentRoutes.swift`] S3 backend의 presigned URL 기본 만료는 900초(15분)다. 「Swift 시절 — Rust 현행 재검증 필요」 [근거: `S3ArchiveClient` 83–109행, `f399e417:server/Sources/MomoServer/Drive/S3ArchiveClient.swift`] · 현행 [`momo-drive`](../../server-rust/crates/momo-drive/)
...
현재 Drive 경로는 개인 Drive 연결이 아니라 service account와 운영자가 지정한 하나의 shared drive 경로로 시작한다. 개인 Drive 확장은 ADR에서 예약된 경로다. [근거: ADR-0113 D3](../adr/0113-connector-credential-plugin-boundary.md#L27-L30) Drive backend는 요청에 `driveId`를 넣고 받은 file metadata의 `driveId`도 다시 확인하며, service account token은 `drive.readonly` scope를 사용한다. 「Swift 시절 — Rust 현행 재검증 필요」 [근거: `DriveBackend` 224–256행 · 420–429행, `f399e417:server/Sources/MomoServer/Drive/DriveBackend.swift`] · 현행 [`momo-drive`](../../server-rust/crates/momo-drive/)

Drive plugin migration은 credential과 shared-drive identifier를 DB에 저장하지 않는다고 명시한다. plugin grant에는 `granted_by = member_id` 제약이 있어 타인이 대신 발급하는 grant를 허용하지 않으며, tool 호출은 active delegated member의 active grant를 찾아야 진행된다. 「Swift 시절 — Rust 현행 재검증 필요」 [근거: migration 015 4–6행](../../server/Migrations/015_drive_mcp_plugin.sql#L4-L6), [migration 013 62–72행](../../server/Migrations/013_plugin_registry.sql#L62-L72), [`DriveMCPRoutes` 134–175행, `f399e417:server/Sources/MomoServer/Routes/DriveMCPRoutes.swift`]
...
- 첨부에는 바이러스·멀웨어 검사가 없고, MIME은 형식 검증 및 선언값과 저장소 메타데이터의 일치만 확인한다. MIME sniffing은 하지 않는다. [근거: 조사 기록 12행](../planning/JOURNAL.md#L12), [`AttachmentRoutes` 462–467행 · 158–160행, `f399e417:server/Sources/MomoServer/Routes/AttachmentRoutes.swift`] · 현행 [`attachments.rs`](../../server-rust/bins/momo-server/src/routes/attachments.rs)
...
- 이 문서는 제3자 보안 감사, 침투 테스트, SOC 2 또는 ISO 인증을 주장하지 않는다. 릴리스 게이트의 Enterprise Trust 항목도 threat model·pentest plan·security whitepaper draft를 요구하는 설계 단계로 기록되어 있다. [근거: 스토어 검수 게이트 G-H](../cicd/03-store-readiness-gate.md)
- 구 macOS alpha 채널의 SwiftPM development/Xcode-host build는 unsigned이며, 그 채널은 공증 배포 준비를 주장하지 않는다. Developer ID·공증·staple은 위에서 구분한 **next** 채널의 절차다. [근거: `MACOS_ALPHA_UPDATE_CHANNEL` 8–17행](../NEXT_CHANNEL.md#L8-L17), [`NEXT_CHANNEL` 33–50행](../NEXT_CHANNEL.md#L33-L50)
