...
argv나 출력으로 노출되지 않는다. 초대 bearer code 원문도 SQL/컨테이너 argv에 값으로
...
명시 지정할 수 있다(내부 알파 mDNS 호스트 등). base URL을 얻을 수 없으면 초대를 만들기
전에 fail-closed로 멈춘다. 딥링크는 코드를 담으므로 위 파일과 동일하게 취급하고, 전달
후 터미널 스크롤백에 남지 않도록 주의한다. 형식 정본은 `docs/onboarding-deeplink.md`.
...
> **prod realtime은 아직 열려 있지 않다 — 오케스트레이터 조치 필요.** REST는 위 CORS로 열리지만 **wss는 별개**다. prod compose는 `CENTRIFUGO_CLIENT_ALLOWED_ORIGINS: ${APP_DOMAIN:+https://${APP_DOMAIN}}`로 **단일 파생 오리진**(`https://app.oor7.com`)만 주입하고 `infra/prod/centrifugo.prod.json`에는 `client.allowed_origins`가 아예 없다. 데스크톱 webview는 WS 업그레이드에 `Origin: tauri://localhost`를 실어 보내므로 **prod에서 403으로 거부된다**(Origin 헤더가 없는 네이티브 클라만 무검사 통과). dev/e2e/내부알파(`infra/centrifugo.json`)에는 `tauri://localhost`·`http://tauri.localhost`가 **이미 들어 있다** — prod만 비어 있다. MOMO-398의 "APP_DOMAIN 단일 파생" 계약을 바꾸는 결정이라 DESK-1은 설정 파일을 건드리지 않았다.
