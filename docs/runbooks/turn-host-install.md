...
> **집행 완료(2026-08-19, momo-main — 성재 SSH 위임).** 이 절은 절차 정본이며, 아래 각 걸음의 실측 기록은 #1545 close 코멘트가 원장이다. 요약:
> §6-2 켜기 후 병행 실증은 **불성립으로 실측** — coturn은 `use-auth-secret`이 켜지면 정적 `user=` 계정을 무시한다(reload는 인증 설정을 집지 않는다 — **restart 필요**, 실측). 활성 스트림 0 정지창에서 §6-4까지 연속 집행: 은퇴 표 = 정적 401 / 단명 12/12·lost 0%. §6-3 주입은 compose 배선 #1586+프로덕션 배포(momo-rust:8d0f7d9a)로 완료. microVM 실측(#1588)이 ephemeral 자격만으로 relay↔relay 왕복을 증명해 은퇴 후 경로가 실사용으로 닫혔다.

### 왜 순서가 고정인가

momo-turn은 **프로덕션 실사용 중**이다. `use-auth-secret`을 켜면서 동시에 `user=` 줄을 지우면,
살아 있던 스트림이 죽을 수 있다 — REST 자격의 **만료**는 기존 allocation의 REFRESH에서 재검사되지
않지만(ALLOCATE에서만 — LIVE-5c 실측, PR #1570), 계정 **자체의 삭제**는 매 요청이 통과하는
MESSAGE-INTEGRITY 검증의 키를 없애는 다른 문제이고, 그쪽 거동은 실측된 바 없다. 미실측 리스크를
프로덕션 스트림에 걸지 않기 위해 세 걸음이고, 각 걸음은 되돌릴 수 있다:
...
**TTL은 「새 allocation을 열 수 있는 창」이지 실행 중 스트림의 수명이 아니다 — 실측 정정(LIVE-5c, PR #1570).**
이 절의 이전 판은 TTL을 「한 스트림이 검어지기까지의 시간」으로 적었는데, 실측이 반증했다:
coturn은 REST username의 만료를 **ALLOCATE에서만** 검사하고 이미 존재하는 allocation의
REFRESH에서는 다시 보지 않는다 — TTL 60초 자격이 200초 소크(relay-only)를 버텼고 타이핑 비트
10/10이 전달됐다(마지막 t+180s). producer가 자격을 파이프라인 구성 때 **한 번** 넣고 30초
재검증이 받아오는 새 자격을 버리는 것은 여전히 사실이지만(GStreamer가 살아 있는 ICE 에이전트의
TURN 서버를 교체하지 못한다 — `webrtcbin.ice-agent`는 읽는 순간 에이전트를 파괴, #1438 실측),
그래서 끊기는 스트림은 없다. 잔여 케이스는 하나다: **세션 중 재-ALLOCATE(= ICE restart)** 는
그 순간 유효한 자격이 필요하므로, ICE restart를 실제로 쓰게 되는 날 그 지점에 remint 훅을 넣는다
(주기 교체가 아니라 restart 시 발급 — 재검증이 매번 서빙하는 새 자격이 정확히 그 재료다).
새 관전자는 어느 쪽이든 영향이 없다(새 연결·새 파이프라인·붙는 순간 발급).
