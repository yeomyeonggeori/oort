# [은퇴] NCP 프로덕션(app.oor7.com) Rust 스택 배포 런북

> **⚠️ 이 런북이 가리키는 배포는 더 이상 존재하지 않는다.** 2026-08-26 NCP 완전 철수 — 인스턴스 반납·IP 반환 완료(집행 기록: `docs/planning/JOURNAL.md` 해당 일자, 레포 밖 표면 체크리스트: #1802). 이 문서는 역사 기록으로만 보존하며, 어떤 절차도 실행하지 마라.
>
> 공개 호스트로 여는 현행 절차는 [`docs/SELF_HOST.md` 공개 오리진으로 열기](../SELF_HOST.md#공개-오리진으로-열기)다.
