...
에이전트 플레이북 [`SELF_HOST_AGENT.md`](../SELF_HOST_AGENT.md) §3.2(첫날 백업)·§5(데이터 가져가기)가 이 런북을 가리킨다. **복원 정본은 여기**다.
...
T2는 compose postgres 컨테이너가 없다. dump/restore는 `MIGRATE_DATABASE_URL`(슈퍼유저)만 쓴다. `DATABASE_URL`(momo_app, RLS)로 dump하지 않는다. `--tier t2`는 env `MOMO_SELF_HOST_PLATFORM`의 tier와 같아야 한다 — T1 스택을 URL로 백업하지 않는다. `--platform railway`(별칭 `--railway`)가 `MOMO_SELF_HOST_PLATFORM=railway`를 찍는다. 스탬프 없는 기존 env는 `--tier t2`를 넘기거나 서비스 변수를 설정한다.
