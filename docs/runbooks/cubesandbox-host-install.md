...
| `signalling.validateRoute` | 서버 부재로 **미검증**(stub 경로로만 왕복) | **여전히 미검증** — #1438은 `MOMO_DISPLAY_STUB_VALIDATE=1`로 미디어 도달성만 격리. 실서버 `validate` 결선은 LIVE-1..4 서버 몫(§9, `unverified.serverValidateWiring`) |
