...
//!     fresh `pgvector/pgvector:pg18` DB — all 60 files apply in place, unmodified
...
async fn migration_runner_applies_all_60_and_matches_schema() {
    let _guard = migration_lock().await;
    let pool = superuser_pool().await;
    let mut conn = pool.acquire().await.expect("acquire");

    // discovery = exactly the 60 versioned files, contiguous
    let migs = discover_migrations(&default_migrations_dir()).expect("discover");
    assert_eq!(migs.len(), 60, "expected 60 migrations, got {}", migs.len());
    ...
    let report = run_migrations(&database_url(), &default_migrations_dir(), SeedMode::None)
        .expect("all 60 migrations apply on a fresh pgvector/pg18 DB");
    assert_eq!(
        report.total(),
        60,
        "the runner must consider all 60 files (applying them, or SKIPping the \
    ...
    println!(
        "conformance: 60 migrations applied; outbox_kind={labels:?}; FORCE-RLS tables={forced}"
    );
}
