...
//!
//! Harness contract:
//!   * `DATABASE_URL` connects as a **superuser** (applies the 61 migrations via
...
/// Apply the 61 migrations + bootstrap roles exactly once per test process.
fn ensure_schema_and_roles() {
    ...
    run_migrations(&database_url(), &default_migrations_dir(), SeedMode::None)
        .expect("apply all 61 migrations on a fresh pgvector/pg18 DB");
