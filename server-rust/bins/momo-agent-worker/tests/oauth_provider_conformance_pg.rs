...
//! this suite: **a real loopback HTTP server plays the provider**, serving both
//! `POST /oauth/token` and `POST /v1/chat/completions`.
//!
...
use momo_agent_worker::provider::{ChatProvider, OpenAiCompatProvider};
...
/// A worker with **both** real outbound seams: the shipped HTTP chat client and
/// the shipped HTTP token-endpoint client.
async fn build_worker(config: WorkerConfig) -> AgentWorker {
    let provider: Arc<dyn ChatProvider> = Arc::new(
        OpenAiCompatProvider::new(config.request_timeout).expect("build the real chat client"),
    );
...
fn chat_response(state: &Arc<Mutex<MockState>>, headers: &HashMap<String, String>) -> (u16, Value) {
    ...
    (
        200,
        json!({
            "choices": [{"message": {"content": "구독 OAuth로 답합니다"}}],
