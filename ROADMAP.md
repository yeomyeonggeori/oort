...
- **UX 즉시 체인:** MOMO-383 `#387` workspace-first sidebar/header/menu + ADR-0118 active-member read/owner-admin rename(**PR #389, main `9c1fc7a` merge**) → MOMO-384 `#390` native channel creation sheet + root window-level tooltip(**PR #394 fresh correctness/security/performance+design review Blocker/High 0, worker `status:needs-review`, merge 전**) → MOMO-385 `#391` right member inspector + compact profile/canonical DM(**worker gates·runtime·design PASS, PR handoff 전**) → MOMO-386 `#392` RLS workspace search + result jump.
