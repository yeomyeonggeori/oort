...
- **Local PR gate / multi-session ops**(M1): GitHub Actions는 현재 비용/결제 이슈로 disabled/manual-only이며, PR body local evidence와 worktree branch lock을 하드 운영 규칙으로 둔다. `runtime-relay`는 MOMO-115부터 Docker compose/migrate/server send/outbox/relay/Centrifugo evidence까지 자동화된다. MOMO-186부터 docs/static local gate는 `infra/docker-compose.e2e.yml` config validation으로 e2e stack boundary drift도 잡는다. MOMO-199부터 `scripts/goal_status.sh`는 closed issue/merged PR에 묶인 stale local worktree를 read-only로 audit하고, clean/pushed candidate에만 cleanup command를 출력한다. 정본: `docs/LOCAL_PR_GATE.md`, `docs/MULTI_SESSION_OPS.md`.
