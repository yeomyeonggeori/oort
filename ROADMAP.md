...
| `MOMO-338` | M1 | Hermes adapter bearer single path | human operator login/shared secret을 제거하고 `MOMO_AGENT_TOKEN` 하나로 realtime-first work stream + bounded recovery/callback/message를 처리. `agent:` subscribe는 self-only이며 non-loopback plaintext는 fail-closed |
