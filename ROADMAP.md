...
### 런칭 보조축 — Bring your hosted agent (병렬 · ADR-0162 Proposed)

> **이 축은 v0의 관전·승인·대화 임계경로를 대체하거나 늦추지 않는다.** 이미 호스팅된 외부 에이전트를 oort 팀메이트로 데려오는 진입 장벽을 낮춰 런칭 메시지와 초기 유입을 보강하는 병렬 축이다. 제품 약속은 벤더 중립인 **“Bring your hosted agent”**이고, Grok Bot은 첫 설정 프리셋이자 실증 대상으로 둔다. ADR-0162가 Accepted 되기 전에는 경계 변경 구현을 시작하지 않는다.

| 권장 순서 | 실질 작업 | 기대효과 | 진행 게이트 |
|---|---|---|---|
| **R. 계약 교정** | #1343에서 코드·공식 자료를 다시 대조하고, 원격 MCP·기존 agent gateway·credential·pairing 경계를 ADR과 실행계획에 고정한다. ACP live-chain은 #1345의 별도 감사 레인, managed/self-host catalog는 ADR-0163의 deferred 결정 레인으로 분리한다. | 이미 있는 원장·게이트웨이를 중복 구축하거나 Grok 전용 코어를 만드는 일을 막는다. | ADR-0162 검수 → 성재 Accept/Reject |
| **0. Grok 1회 체험 실측** | 유료 구매 없이 제공되는 개인용 one-time trial이 실제 계정에 노출되는지 확인하고, 노출될 때만 커스텀 MCP·루틴·연결 해제 표면을 실측한다. 앱 설치와 동의는 성재가 직접 승인하며 비밀번호·MFA·토큰을 전달하지 않는다. | 구독비를 먼저 쓰지 않고 Grok 프리셋의 성립 여부와 수동 정리 범위를 확정한다. trial이 없어도 벤더 중립 기반 구현은 계속할 수 있다. | #1344; 미노출이면 Grok 프리셋만 `runtime-unverified`/blocked |
| **1. 범용 연결 기반** | Rust에 에이전트 credential 발급·회전·폐기 API, stateless MCP 진입점, 만료되는 1회 pairing proof를 추가한다. 봇이 proof를 들고 먼저 접속하고 사람이 감지 결과를 확인한다. v0는 `1 Bot=1 connection=1 dedicated agent member=1 routine`으로 격리하고, 별도 active credential proof와 전용 member unpause 뒤에만 협업 권한을 연다. 외부 봇 목록 스크래핑은 하지 않는다. | 어떤 hosted agent든 같은 보안 경계와 최소 권한으로 연결할 수 있고, 한 연결의 해제가 다른 runtime을 멈추지 않으며 Grok 정책 변화가 코어를 흔들지 않는다. | Accepted ADR + 이슈/BUILD_TICKETS/ready 패킷 |
...
| **4. 정리 폐곡선·Grok 증거** | 해제 즉시 로컬 credential을 폐기하고 새 job·write를 막은 뒤 `cleanup_pending`에서 Grok routine과 MCP connector 제거를 안내·확인한다. 공개 삭제 API가 없으면 사용자 확인으로 닫고, 기존 멤버·메시지·작업 이력은 보존한다. 마지막으로 Grok 1개 봇의 pair→work→disconnect를 증거화한다. | 접근권한은 즉시 끊으면서 외부 자동화 잔재를 숨기지 않는다. 검증된 범위만 “Grok Bot도 연결” 메시지로 말할 수 있다. | 자동 삭제 주장 금지; #1344 실측 또는 정직한 수동 cleanup |
