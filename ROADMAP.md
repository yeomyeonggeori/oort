...
- **멀티팀 온보딩**(M2): `003_onboarding.sql` 첫 slice는 `invite_code` + redemption audit로 시작한다(MOMO-010). 워크스페이스/초대코드 REST는 MOMO-011, macOS에서 먼저 확인 가능한 invite UI thin slice는 MOMO-012, production `/v1/join` 자가가입 e2e는 후속 이슈로 분리한다. `platform_admin` 전역 추적은 MOMO-013이며, `schema_v0.sql` 정본은 수정 금지, 신규 마이그레이션으로만 확장한다.
