...
- **UX 즉시 체인:** MOMO-383 `#387` workspace-first sidebar/header/menu + ADR-0118 active-member read/owner-admin rename(**PR #389, main `9c1fc7a` merge**) → MOMO-384 `#390` native channel creation sheet + root window-level tooltip(**worker 구현·실창 검수 완료, review handoff**) → MOMO-385 `#391` member inspector + one-click DM → MOMO-386 `#392` RLS workspace search + result jump.
