...
M6 (CI/CD) ─────────────── 게이트/배포 자동화 ┘
  ...
  └─ 현재: M1 런타임 G-0의 핵심 4개 게이트(MOMO-001~004)는 Docker Desktop으로 검증됨.
     ...
     · MOMO-230에서 credentials가 있는 환경 전용 `external-agent-provider` opt-in gate를 추가해 real external runtime SSE + local momo `@김인턴` 1왕복을 검증한다
     ...
     · MOMO-256에서 내부 알파 agent runtime seed를 `Hermes`/`@hermes`로 정렬하고, local loopback OpenAI-compatible provider를 통해 mention→agent_job→AgentWorker SSE→timeline response를 runtime-agent gate로 검증한다
     · MOMO-257에서 사용자가 local Hermes-compatible provider에서 Codex/OpenAI OAuth나 provider token 설정을 직접 수행하고, momo는 loopback endpoint + Hermes-facing bearer만 받는 setup runbook/wrapper/Command Center evidence를 추가한다
     ...
     · MOMO-262에서 에이전트 초대 흐름을 pairing wizard로 확장해 alias/display name/endpoint/model/scope, manifest copy/export, loopback HTTP guard, provider-owned credential boundary를 앱에서 안내한다
     · MOMO-261에서 승인함/Command Center 의미를 명확히 하고, 사람 typing indicator와 에이전트 working badge를 추가해 로컬 dogfood 중 “지금 무엇이 일어나는지”를 앱 안에서 설명한다
...
이 overlay는 기존 M1~M8 backbone을 깨지 않고, oort의 포지션을 "채팅앱"에서 "context/memory/policy가 있는 agent work OS"로 끌어올리는 제품·운영 축이다.
...
| `MOMO-242` | M1/M7 준비 | External Agent Runtime Smoke | oort agent member가 external runtime/provider와 최소 1왕복을 수행하는 credentialed smoke; provider token은 oort 밖에 두고 readiness/degraded evidence만 redacted 처리 |
...
| `MOMO-257` | M1/M7 준비 | Local Hermes/Codex OAuth provider setup | 사용자가 provider에서 Codex/OpenAI OAuth 또는 provider token을 직접 설정하고, oort는 local loopback endpoint + Hermes-facing bearer만 받아 `@hermes` credentialed smoke를 수행하는 runbook/wrapper/Command Center evidence |
...
| `MOMO-262` | M3/M7 준비 | Agent Pairing Wizard v0 | 멤버 `+`의 에이전트 초대에서 Hermes alias/display name/loopback endpoint/model/scope를 확인하고 pairing manifest/invite code copy/export와 non-loopback HTTP fail-closed guidance를 제공한다. provider OAuth/API key는 oort에 저장하지 않음 |
...
| `MOMO-234` | M1 | Hermes Codex OAuth provider boundary v0 | `docs/adr/0004-codex-oauth-hermes-provider-boundary.md` + external provider verifier evidence/guard; Codex OAuth access/refresh token은 provider-owned이고 oort app/API/DB/local gate에는 저장/전달하지 않음 |
...
| `MOMO-325` | M1 | Hermes Gateway Native Platform Integration v1 | Hermes가 oort를 Slack/Telegram-style platform으로 보고 `agent.job`를 상시 sync하되, final response/usage/audit는 oort REST→Postgres→outbox ledger로 기록; `scripts/momo hermes-gateway-*` + mock gateway harness |
...
MOMO-180은 Paca/OpenHands/Linear/Rovo/GitHub Copilot/Slack/MCP/A2A 흐름을 기준으로 oort의 생태계 방향을 고정한다. MOMO-184는 이를 제품 메시지로 압축해 "Paca를 복제하지 않고, 채널 타임라인을 context/approval/cost/audit execution ledger로 만드는 self-hosted enterprise agent host"를 reusable copy로 고정한다.
...
- 핵심 결정(적용됨): Work=`agent_run` 확장(새 실행 개체 금지) · 실행은 항상 에이전트 호스트(BYOA gateway, oort 서버는 코드 실행 안 함, ADR-0004 유지) · codex sandbox 정책→승인 티어 매핑(349 재사용) · 특화 라우팅 v0=초대된 에이전트 중 capability 배지 명시 선택 · 코드 특화 레퍼런스=Apache-2.0 codex CLI 기반 `codex-workbench` adapter.
...
- **Work Console:** MOMO-375는 transcript/activity drawer까지만 허용한다. `Control+backtick` command input, Codex/Claude/OpenCode process, cwd/repo/worktree, sandbox/approval relay는 ADR-0114 승인 뒤 새 builder로 분리한다. oort 서버는 command process나 upstream credential을 보관하지 않는다.
...
- **Agent Runtime Spec**(M1.5~M3): Hermes/Kim Intern/openclaw를 기준으로 memory/cache/protocol gap을 메우고, oort가 agent host로서 context/capability/execution/ledger 4-plane을 소유한다. Context Packet v0 정본은 `research/11-agent-runtime/04-context-packet-v0.md`, Memory Plane v0 정본은 `research/11-agent-runtime/05-memory-plane-v0.md`, Capability Cache v0 정본은 `research/11-agent-runtime/06-capability-cache-v0.md`, Agent Run Lifecycle v0 정본은 `research/11-agent-runtime/07-agent-run-lifecycle-v0.md`, Approval Pause/Resume v0 정본은 `research/11-agent-runtime/08-approval-pause-resume-runtime.md`, Approval Decision Server Contract v0 정본은 `research/11-agent-runtime/10-approval-decision-server-contract-v0.md`, MOMO-167은 이 contract의 server runtime endpoint/ledger/resume job slice이며, Hermes Adapter Contract v0 정본은 `research/11-agent-runtime/11-hermes-adapter-contract-v0.md`이다. runtime gap/roadmap 정본은 `research/11-agent-runtime/*`.
