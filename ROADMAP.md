...
- **Dogfood shell UX**(M3): MOMO-253/259/263에서 post-login macOS shell을 dogfood 가능한 메신저 chrome으로 정리한다. 원칙은 “대화에 필요한 것은 기본 노출, 개발/진단/승인/업데이트는 숨겨진 surface”다. 작은 창에서는 우측 Command Center/Approvals를 layout-consuming 3rd column이 아니라 overlay drawer로 열고, 넓은 창에서만 inspector column처럼 붙인다. Profile footer는 언어/appearance/downloads/update/session/logout을 모으는 operator surface이며, server/workspace icon은 dogfood v0 local display draft 후 backend persistence로 승격한다.
