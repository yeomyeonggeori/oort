...
- **UX 즉시 체인:** MOMO-383 `#387` workspace-first sidebar/header/menu + ADR-0118 active-member read/owner-admin rename(**correctness review fix·전체 Swift 352 tests·worker swift/macos-ui 완료, #388 뒤 final runtime-db 대기**) → MOMO-384 `#390` native channel creation sheet + window-level tooltip → MOMO-385 `#391` member inspector + one-click DM → MOMO-386 `#392` RLS workspace search + result jump.
