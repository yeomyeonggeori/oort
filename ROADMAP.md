...
- **UX 즉시 체인:** MOMO-383 `#387` workspace-first sidebar/header/menu + ADR-0118 active-member read/owner-admin rename(**connect/bootstrap/cache race·exact subscription cleanup·workspace root RLS/locked join lookup drift guard·production role preflight·accessible recovery/narrow settings projection, 전체 Swift 367 tests·worker full runtime-db/launch macos-ui PASS, momo-main final rereview 대기**) → MOMO-384 `#390` native channel creation sheet + window-level tooltip → MOMO-385 `#391` member inspector + one-click DM → MOMO-386 `#392` RLS workspace search + result jump.
