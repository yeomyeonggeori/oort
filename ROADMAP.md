...
- **UX 즉시 체인:** MOMO-383 `#387` workspace-first sidebar/header/menu + ADR-0118 active-member read/owner-admin rename(**bootstrap/cache race·workspace root RLS/locked join lookup·fresh migration-before-role ACL·no-cache recovery 보강, 전체 Swift 360 tests·worker full runtime-db/launch macos-ui PASS, momo-main final rereview 대기**) → MOMO-384 `#390` native channel creation sheet + window-level tooltip → MOMO-385 `#391` member inspector + one-click DM → MOMO-386 `#392` RLS workspace search + result jump.
