...
/**
 * 승인 확정 문장 앞에 붙는 목적지 한 마디.
 *
 * 확정 화면에서 라디오는 잠긴다(요청이 나가는 중에 목적지를 바꿀 수는 없다).
 * 그래서 **어디로 가는지가 문장에 있어야** 한다 — 잠긴 컨트롤을 다시 읽게 하는
 * 대신, 사람이 지금 누르려는 버튼 바로 위에서 목적지를 말한다.
 */
export function spawnDestinationClause(hostName: string): string {
  return `「${hostName}」에서 실행합니다.`;
}
