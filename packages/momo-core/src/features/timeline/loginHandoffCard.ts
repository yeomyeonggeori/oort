...
/**
 * 이 카드의 딥링크가 **직접 조작을 하러 가는 길인가** (LIVE-5b).
 *
 * ## 이것이 약속이 아닌 이유
 *
 * 카드는 세션 id 만 안다. 그 세션을 **누가 소유하는지는 모른다** — props 에 없고,
 * 있어야 할 이유도 없다(증보 3 D3 이 에이전트에게 허락한 것은 정지·재개·사유
 * 셋뿐이다). 그런데 직접 조작은 소유자만 할 수 있다(D1). 그래서 이 판정이 답하는
 * 것은 「이 사람이 조작할 수 있는가」가 **아니라** 「이 카드가 가리키는 자리가 지금
 * 조작 동선인가」이다.
 *
 * 그 구분이 어휘까지 정한다: 이 값이 참이어도 카드의 낱말은 바뀌지 않는다.
 * 「직접 조작으로 이동」이라고 적어 두고 도착지에 조작 어포던스가 없으면, 그것은
 * 비활성 버튼보다 나쁜 거짓말이다 — 눌리기까지 하니까. 도착지는 소유자를 알고,
 * 어포던스를 세울지 말지는 거기서 정한다(`controlAffordance`).
 *
 * 그래서 이 값이 하는 일은 하나다: 도착한 화면에서 **확인 단계를 미리 무장**한다.
 * 창을 열지는 않는다. 링크가 눌린 것으로 남의 에이전트가 멈추면 안 된다.
 *
 * 참인 조건은 「사람이 필요한 상태」다 — 아직 대기 중이거나(에이전트가 서 있다),
 * 창이 지금 열려 있거나(누군가 이미 잡고 있다). 끝난 개입은 갈 이유가 없다.
 */
export function loginHandoffSeeksControl(
  card: Pick<LoginHandoffCard, "phase" | "control">
): boolean {
  if (card.control !== null && card.control.endedAtMs === null) return true;
  return card.phase === "waiting";
}
