 ...
 * 자가 **안 세는** 글자들 (#1479).
 *
 * 이 어림자의 기본 규칙은 「비ASCII 는 한 배수(1em)」인데, 조판용 서식 문자는
 * 비ASCII 이면서 폭이 0 이다. 규칙을 그대로 두면 `MENTION_AFFORDANCE` 가 실제보다
 * 14~16px 넓게 재이고, 그러면 아래 예산 단정이 재는 것은 문장이 아니라 자의
 * 버그가 된다 — 「한 마디 더 늘리면 넘친다」를 지키는 줄이 한 마디 일찍 운다.
 *
 * 둘 다 이 레포가 실제로 렌더에 쓰는 문자다: U+2060 은 코어의 이 문장이(줄바꿈
 * 기회를 **지운다**), U+200B 는 두 클라의 레이아웃 장치가(기회를 **심는다** —
 * 웹 `TypingLine.tsx` · 폰 `withLatinWordBreaks`).
 */
...
// =============================================================================
// #1479 — 광고 절이 절 **안**에서 끊기지 않게, 낱말을 묶는다.
//
// PR #1477 design-review 실측(폰, AX 동적 타입): iOS 의 `hangul-word` 줄바꿈
// 전략은 `@` 를 독립 낱말로 보므로 `@` 뒤가 어절 경계가 되고, 좁은 글폭에서
// 광고 절이 「보내기, @」 / 「로 부르기」로 끊긴다. 「로」는 목적어를 잃은
// 조사다 — §5.3 7위(의존형태소 절단). #1418 이 웹에서 잡은 것은 절과 절
// **사이**의 이 결함이었고, 이번 것은 절 **안**에서 났다. 절 계약은 절 사이만
// 지키므로 여기는 문장 자신이 지켜야 한다.
//
// 처방은 문장 재구성이 아니라 WORD JOINER(U+2060)다. `@` 는 이 광고가 광고하는
// 그 키라 문장에서 뺄 수 없고, 목적어를 실은 재구성이 두 예산을 넘는다는 것은
// 위 스위트가 계속 재는 사실이다. U+2060 은 폭이 없고(예산 불변) 소리도
// 없으며(서식 문자 — VoiceOver 가 읽지 않고, 이 상자의 이름은 어차피
// `composerFieldLabel` 이 따로 든다) 줄바꿈 기회 하나만 지운다. 전례: 이 레포는
// 이미 zero-width 문자를 레이아웃 장치로 쓴다(웹 `TypingLine` · 폰
// `withLatinWordBreaks` 의 U+200B). 그 둘이 기회를 **심는** 자리라면 여기는
// 기회를 **지우는** 자리다.
// =============================================================================

describe("멘션 광고의 낱말 결합 (#1479)", () => {
  // 이 파일에서 joiner 는 이스케이프(\u2060)로 적는다 — 테스트는 눈으로 읽는
  // 문서이기도 해서다. 정본 소스만 리터럴로 든다: `gate-work-panel.mjs` 가
  // 코어 소스를 정규식으로 **원문 그대로** 읽어 DOM 과 대조하므로, 소스에
  // 이스케이프를 적으면 게이트의 자와 화면이 갈라진다.
  it("@ 와 조사가 WORD JOINER 로 묶여 있다 — 그 사이는 어절 경계가 아니다", () => {
    expect(MENTION_AFFORDANCE).toBe("@\u2060로 부르기");
  });

  it("눈에 보이는 글자는 그대로다 — joiner 를 걷어내면 옛 문장이다", () => {
    // 바뀐 것은 접힘의 자리 하나다. 낱말·조사·순서가 하나라도 바뀌었으면 그것은
    // 문장 재구성이고, 그 결정은 위 예산 스위트를 다시 통과해야 한다.
    expect(MENTION_AFFORDANCE.replace(/\u2060/g, "")).toBe("@로 부르기");
  });

  it("joiner 는 폭이 없다 — 예산의 자가 그것을 세지 않는다", () => {
    // 어림자가 U+2060 을 한 글자(1em)로 세면 이 문장이 실제보다 14~16px 넓게
    // 재이고, 「한 마디 더 늘리면 넘친다」를 지키는 자가 그만큼 일찍 운다.
    expect(estimateWidthPx("\u2060", 16)).toBe(0);
    expect(estimateWidthPx("@\u2060로 부르기", 16)).toBe(
      estimateWidthPx("@로 부르기", 16)
    );
  });
});
