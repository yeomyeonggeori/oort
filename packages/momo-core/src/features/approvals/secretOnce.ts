...
/**
 * 1회 값이 서는 자리의 첫 문장 (ADR-0182 ① · D4).
 *
 * 웹이 AX-4 에서 `ApprovalActions.tsx` 에 처음 적은 문장이고, 폰(AX-7 #2513)이
 * 두 번째 소비자다. 두 클라가 같은 순간에 같은 말을 해야 하므로 규율이 사는 이
 * 모듈로 올린다 — 문장이 두 파일에 있으면 한쪽만 고쳐지는 날이 온다. 웹은 아직
 * 자기 파일의 글자를 들고 있고, 둘이 같은 글자인지는 폰의 대조 시험이 잰다
 * (`clients/mobile/__tests__/actionApprovalCard.test.tsx`). 웹이 이것을 import
 * 하게 되면 그 대조는 할 일을 잃는다.
 *
 * 「이 화면에서만」이 참인 이유가 이 모듈의 머리말이다: 값은 결정 응답에만 있고,
 * 받은 화면이 닫히면 어디에도 남지 않는다.
 */
