...
export function sidebarEmptySectionHint(pointerCanHover: boolean): string {
  ...
  return pointerCanHover
    ? "채널 행을 끌어다 놓거나 우클릭해 이 섹션으로 옮길 수 있습니다."
...
/**
 * 무리의 제목이 「이 섹션」이 아니라 「사이드바」인 이유는 그것이 사실이기
 * 때문이다 - `sectionSort` 는 payload 에 한 칸뿐이라 값도 하나다
 * (`SidebarPrefs.sectionSort` 머리말). 섹션마다 같은 라디오를 세워 두고
 * 「이 섹션의 정렬」이라 부르면, 하나를 바꿀 때 전부 바뀌는 것이 결함으로 읽힌다.
 */
export const SIDEBAR_SORT_GROUP_LABEL = "사이드바 정렬";
...
export const SECTION_MOVE_UP_LABEL = "위로";
export const SECTION_MOVE_DOWN_LABEL = "아래로";
