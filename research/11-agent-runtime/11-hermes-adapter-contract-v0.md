...
oort supports two **official, role-separated execution paths**. The path says who owns the runtime; it does not move governance out of oort.

| Mode | Official product role | Runtime owner | Delivery |
|---|---|---|---|
| AgentWorker OpenAI-compatible SSE (`AGENT_GATEWAY_MODE=worker`) | **managed** agent path | oort | server creates `agent_job`; AgentWorker claims it and calls the provider through `/v1/chat/completions` SSE |
| Hermes platform adapter (`AGENT_GATEWAY_MODE=gateway`) | **BYOA** (bring your own agent) path | user-owned Hermes/provider runtime | server publishes a private `agentwork:` wake-up; adapter re-reads the durable job and returns events/results through oort REST |

Both paths keep oort as the governance host. Postgres remains the source of truth; the server owns the `agent_run` state machine, Context Packet projection, approval records and decisions, budget/usage ledger, audit log, message ordering, and transactional outbox. The runtime may report progress, proposed tool calls, results, and usage evidence, but it cannot commit those guarantees itself.
...
Codex OAuth is outside the oort boundary. If Hermes/Kim Intern uses Codex OAuth,
the provider owns authorization code exchange, access/refresh token storage,
refresh, unlink, rotation, and provider account audit. oort accepts only the
OpenAI-compatible provider endpoint plus `HERMES_API_KEY`; oort app/API/DB,
...
A runtime integration is acceptable only if every row stays under oort server ownership. The two paths may differ in transport, never in the authority that commits a guarantee.
...
| Cost | budget authority and `usage_ledger` reserve/reconcile stay in oort; provider/runtime usage is evidence, not authority | worker parses SSE usage; gateway submits bounded usage with completion |
...
3. oort builds or reads the Context Packet projection and tool grants for the run.
...
7. Text deltas become server-governed `agent.partial`; final text becomes a normal oort message through Postgres/outbox.
...
pass Codex OAuth token env vars to oort processes or verifier scripts.

## 4. Mode B: Hermes Platform Adapter

This is the official **BYOA** path. Hermes treats oort as one messaging platform while oort keeps run governance.
...
- `send(channel, blocks)` writes through REST `POST /messages` only for an explicit oort `run_id`, with `client_msg_id` idempotency. Unbound Hermes lifecycle/setup/command notices are local-log-only; the native gateway final response is committed by `/gateway/complete`.
...
- It must not bypass oort approval/cost/audit decisions for user-visible work.
- It must not forward Codex OAuth tokens into oort REST requests, message props,
...
|---|---|---|
| oort operates the runtime/provider for a hosted or server-side agent | AgentWorker SSE (`worker`) | managed execution; no user-owned gateway process is required |
...
| `platform_adapter_event_mapping.json` | BYOA platform adapter mention event mapping into oort REST invoke/send concepts. |
