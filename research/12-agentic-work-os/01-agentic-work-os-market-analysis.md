...
> 목적: Paca/OpenHands/Linear/Rovo/GitHub Copilot/Slack/MCP/A2A 흐름을 기준으로 oort의 제품 포지션과 생태계 방향을 고정한다.

## 결론

oort는 Slack/Discord/Mattermost의 "대체 채팅앱"으로 포지셔닝하면 불리하다. 이미 성숙한 채팅 UX, 엔터프라이즈 세일즈, 앱 생태계, 모바일 polish를 정면으로 이기기 어렵다. oort의 차별점은 **채널 타임라인이 agent execution ledger가 되는 것**이다.

권장 포지션:

> oort is an enterprise agent host where humans, AI agents, and plugins work in the same channel timeline, while every context packet, tool call, approval, result, cost, and audit record remains governed by the workspace.

한국어 제품 문장:

> oort는 사람, AI 에이전트, 플러그인이 같은 채널에서 일하고, 모든 실행이 승인/비용/감사 가능한 업무 메신저 OS다.
...
oort가 그대로 가져오면 안 되는 부분:

- Paca는 project/task product이므로 board/task extension point가 중심이다.
- oort의 extension point는 channel timeline, message context action, approval card, slash command, member/agent presence, context source badge여야 한다.
- Paca의 MCP plugin v1은 trusted same-process module model을 포함한다. oort는 enterprise trust를 위해 plugin-side MCP exposure도 capability cache, policy version, signed artifact, audit event를 전제로 해야 한다.
...
oort의 차별점은 OpenHands의 agent canvas와도 다르다. OpenHands는 coding agent control plane이고, oort는 일반 업무 채널에서 agent execution을 governed message ledger로 남기는 communication host다.
...
oort는 이들을 대체하기보다 연결해야 한다. Jira/Linear/Paca/GitHub/Google Workspace는 task/document/source-of-work이고, oort는 그 작업을 사람이 승인하고 agent가 실행하는 communication ledger가 된다.
...
oort의 기존 Context Packet, Memory Plane, Capability Cache, approval pause/resume, Agent Run Lifecycle은 이 흐름과 잘 맞다. 다음 단계는 이 스펙들을 "문서상 좋은 생각"에서 server/macOS/runtime contract로 좁혀가는 것이다.

## oort의 차별점

| 비교 대상 | 중심 객체 | 강점 | oort가 이길 지점 |
|---|---|---|---|
...
4. Context belongs to oort, not to the external agent runtime.
...
- Do not let external agent memory bypass oort RLS/context policy.
