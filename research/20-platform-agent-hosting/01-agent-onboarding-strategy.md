# oort 에이전트 온보딩 전략 — "원하는 에이전트를 원클릭으로" 다각 분석 (2026-07-22, Fable — 성재 발제 2차)
...
**oort는 eve처럼 "말" 필요가 없다 — 업계가 수렴 중인 지점(에이전트 명부 + 멘션 가능한 준-멤버 + 거버넌스)을 oort는 이미 하드 불변식("에이전트=member")으로 갖고 있고, 빠진 것은 "문(門)" 두 짝뿐이다.** ①담아오기 문: URL/매니페스트 붙여넣기(+eve/CF 어댑터), ②만들기 문: oort 안 자연어 간편 생성(정의=원장, 실행=기존 이벤트 구동 경로 재사용 — 신규 상주 프로세스 0). 호스팅 레벨 풀 진입은 비노력 유지가 맞되, **"셀프호스트 번들에 eve 런타임 1컨테이너 동봉"이라는 최소형**은 리소스적으로 공짜에 가깝고 전략 가치가 크다.
...
- **커스텀 채널이 1급 API**(`defineChannel` — HTTP/WS 라우트+`send()`+continuationToken+자체 인증): "oort 채널" 작성이 문서 수준에서 지원됨. Postgres world 채택 시 oort의 PG=SoT 문법과 궤 일치.
...
- **두 문이 만나는 곳 = 워크스페이스 에이전트 명부**(Slack Tools>Agents, ChatGPT Agents 사이드바, Copilot Store, Gemini Gallery): 설치 경로 불문 "명부에 오른, @멘션 가능한, 거버넌스 대상 준-멤버". **oort의 '에이전트=member' 불변식이 정확히 이 종착지** — 업계가 이제야 이동 중인 지점을 우리는 0일차부터 갖고 있다.
...
- **격리는 직접 만들지 말 것**: 적대적 코드 실행은 임대(E2B 종량·CF), 자체 상한은 gVisor, 신뢰 도구는 컨테이너+이그레스 통제. oort는 이미 이 결론과 일치(E2B=T3 기질 확정, ADR-0004 이그레스 원칙).

## §2. oort의 3층 제공 모델 — 층별 판정
...
| 원격 셀프 온보딩(URL 붙여넣기) | **A2A agent-card 판독 + 에이전트 명부 등록** | 0130 D4(2단계 예약) — **업계 수렴 확인으로 상향 근거 완성**. Copilot Studio UX(URL→카드 자동 판독→동의→명부) 그대로 oort 관리자 UI에 |
| 웹훅형(n8n/Zapier/Dify) | 인바운드 기성 + outbound 구독 부재 | outbound 이벤트 구독 신설 필요(MOMO-535 후보 유지) |

### T-B. "oort 안 간편 생성" — ★ 성립한다, 상주 프로세스 0으로
핵심 통찰: **oort는 이미 에이전트 생성 API(X-7)와 실행 런타임(AgentWorker+Context Packet)을 갖고 있다.** 빠진 것은 "에이전트마다 다른 인격/도구"의 정의 원장뿐:
- `agent_profile` 원장(가칭): instructions(시스템 프롬프트)·모델 선호·활성 도구(plugin capability 참조)·트리거(멘션/스케줄). **정의=PG 행, 실행=기존 단일 쓰기경로의 agent_run** — eve의 "에이전트=디렉터리"의 oort판은 "에이전트=원장 행+Context Packet".
...
- eve/CF와 실행 인프라 경쟁은 기존 결정대로 비노력(포지션 오류). oort Cloud(T3)는 CLI 세션 호스팅까지.
- **예외 — "동봉 eve" 최소형**: oort 셀프호스트 compose에 eve 런타임 컨테이너 1개(+`channels/momo.ts` 프리셋, Postgres world는 oort PG 별도 DB)를 **옵션 프로파일**로 동봉. 효과: "oort 설치 = 커스텀 에이전트 빌드 환경까지 포함" — 설치자가 자기 에이전트를 코드로 만들면 그대로 oort 멤버. 리소스: 컨테이너 1개(수백 MB), 유휴 CPU≈0 (1-C 실측 범위). 리스크: eve beta API 변동 → **버전 고정+어댑터만 우리가 소유**하면 표면적 최소.
...
5. **MOMO-538 동봉 eve 옵션 프로파일** [소형~중형] — compose `--profile eve` + 버전 고정 + oort 채널 프리셋. *의존: 534.*
