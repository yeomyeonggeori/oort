...
> 규칙: 추론으로 못 얻는 것만 적는다(AGENTS.md §0). Acceptance에 **검증 등급**(`[swift]/[infra]/[sql]/[python]/[xcode]/[ci]/[runtime]/[manual]`)을 반드시 명시.
> 라벨은 `.github/labels.json` 택소노미를 사용하고, `area:*` / `size:*` / 필요시 `priority:*` / `gate:qa`를 추가한다.

## Goal
<!-- 한 문장. "무엇을 done 상태로 만드는가". 예: server에 POST /v1/workspaces 라우트(워크스페이스+owner+초대코드 1개 자동 발급)를 추가하고 swift build green. -->
...
<!-- 등급 정의는 AGENTS.md §2. Swift 이슈는 `swift build` green이 하드 게이트. -->
