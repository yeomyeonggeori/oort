...
auto_classify_script() {
  ...
  case "$1" in
    scripts/migrate.sh|scripts/verify_runtime_role_bootstrap.sh|scripts/verify_rls.sh|scripts/verify_roster.sh|scripts/verify_channel_list.sh|scripts/verify_channel_management.sh|scripts/verify_join.sh|scripts/verify_platform_admin.sh|scripts/verify_approval_decision.sh|scripts/verify_auth_hardening.sh)
...
add_web_commands() {
  ...
  add_note_once not_covered "Real DNS/ACME/TLS on public hosts, invite web join (W-5), and Safari/Firefox coverage (the smoke drives Chromium) remain out of scope for the web v0 gate."
}
