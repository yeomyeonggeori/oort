...
usage() {
  cat <<'EOF'
Usage: scripts/local_gate.sh [--auto] [--profile docs|diagnostics|staging-smoke|host-runtime|backup|local-alpha|internal-alpha|runtime-db|runtime-relay|runtime-live|runtime-agent|external-agent-provider|m3-dbc|web|license|secrets|all]
...
while [ "$#" -gt 0 ]; do
  case "$1" in
    ...
    -h|--help)
      ...
      exit 0
      ;;
    docs|diagnostics|staging-smoke|host-runtime|backup|local-alpha|internal-alpha|runtime-db|runtime-relay|runtime-live|runtime-agent|external-agent-provider|m3-dbc|web|license|secrets|all)
      PROFILE="$1"
      PROFILE_EXPLICIT=1
      shift
      ;;
...
auto_classify_script() {
  ...
  case "$1" in
    ...
    scripts/check_docs_commands.py|scripts/tests/test_docs_commands_gate.sh)
      # #1525. Same shape as the two guards below: it lives in
      # add_static_commands and therefore runs in every profile, so this only
      # names the cheapest lane that re-proves it — and the documents it reads
      # (SELF_HOST.md, the runbooks, AGENTS.md) are docs-lane files anyway.
      AUTO_REASONS+=("$1 -> docs (#1525 docs command drift gate)") ;;
    ...
    scripts/tests/test_cloudflare_recipe.sh)
      AUTO_REASONS+=("$1 -> docs (SH-11d Cloudflare T3 edge recipe)") ;;
...
auto_classify_path() {
  case "$1" in
    ...
    docs/api/openapi.yaml)
      ...
      # inside the web profile (clients/web typecheck/build; rust OpenAPI verifier is static).
      AUTO_NEED_WEB=1; AUTO_REASONS+=("$1 -> web (contract spec; runtime drift gate)") ;;
    clients/web/*)
      # ADR-0183: `--profile web` is clients/web (lint/typecheck/test/build).
      AUTO_NEED_WEB=1; AUTO_REASONS+=("$1 -> web (clients/web)") ;;
    clients/desktop/*)
      AUTO_NEED_ALL=1; AUTO_REASONS+=("$1 -> all (Tauri shell; widen, do not narrow)") ;;
    docs/*|research/*|legal/*|*.md)
      AUTO_REASONS+=("$1 -> docs") ;;
    clients/*)
      # W-S1: clients/macOS·clients/iOS·clients/Core 는 삭제됐다. 남은
      # clients/* 는 위에서 이미 잡히므로(web / desktop), 여기까지
    ...
    infra/aws*)
      AUTO_REASONS+=("$1 -> docs (SH-11c AWS T1 recipe; terraform is an optional tool)") ;;
    infra/cloudflare/*)
      AUTO_REASONS+=("$1 -> docs (SH-11d Cloudflare T3 edge recipe; contract is test_cloudflare_recipe.sh)") ;;
    infra/*)
...
add_static_commands() {
  ...
  add_cmd_once "json syntax" 'jq empty .github/labels.json infra/centrifugo.json docs/api/openapi.undocumented-allowlist.json docs/api/harness-refine-client-msg-id.golden.json && find research/11-agent-runtime/fixtures server/Fixtures -name "*.json" -print0 | xargs -0 jq empty'
  add_cmd_once "openapi contract spec parse" "$GATE_OPENAPI_YAML_PARSE_CMD"
  add_cmd_once "Centrifugo exact credential metadata contract" 'test "$(jq -r ".channel.proxy.subscribe.include_connection_meta" infra/centrifugo.json)" = "true"; grep -Fq "\"include_connection_meta\": true" scripts/local_alpha_runner.sh'
  ...
  # #1525: SELF_HOST.md, the runbooks and AGENTS.md are executed verbatim — AGENTS.md
  # §3 says "copy-paste, 그대로 실행" in so many words. #1472 measured what one
  # wrong command costs there: `cargo fmt --check --manifest-path
  # server-rust/Cargo.toml` exits on "Failed to find targets" against a virtual
  # workspace, three workers ran it and believed the green, and the drift
  # underneath got rediscovered three times. #1472 fixed the executable form and
  # put it in this gate; this step is the other half — the documented form, and
  # every other command in those files, resolved against the tree. It never runs
  # them (they create worktrees, push branches and start Docker), it resolves
  ...
  add_note_once coverage "#1525 docs command drift via scripts/check_docs_commands.py: every command in the GATED_DOCS table (AGENTS.md, docs/RELEASING.md, docs/NEXT_CHANNEL.md, CONTRIBUTING.md, docs/SELF_HOST_AGENT.md, docs/SELF_HOST_AGENT.ko.md, docs/SELF_HOST.md, docs/SELF_HOST.ko.md, docs/SELF_HOST_FIRST_DAY.md, docs/SELF_HOST_FIRST_DAY.ko.md) and every docs/runbooks/*.md must resolve against this tree — the executor exists, is executable and parses (bash -n / py_compile); \`make <target>\` is a target the Makefile defines; \`--profile <p>\` is a profile local_gate.sh's own parser accepts; \`npm [--prefix DIR] run <s>\` is a script that package.json defines; a long flag handed to one of our scripts appears in that script; docker compose -f/--env-file and --package-path/--manifest-path paths exist; and a documented \`cargo fmt --check\` carries --all (the #1472 rule). Runbooks are gated by glob, so a new one arrives covered; the tabled documents are a table with reasons, and a tabled document that disappears is red rather than silently uncovered. Deliberately NOT covered: commands are never executed, so a command that resolves but does the wrong thing is still invisible; unrecognized token shapes are skipped rather than guessed at (a gate that flagged <workspace-id> would be switched off within a week); and \`<!-- docs-cmd-ignore: reason -->\` exempts a single line, at the cost of a written reason. #2124 re-tables the SELF_HOST family (en+ko, including FIRST_DAY and AGENT.ko) after #2142 / ADR-0183 dropped docs/SELF_HOST.md; a tabled document that disappears is still red."
  ...
  add_cmd_once "public-edge Centrifugo internal-only contract" 'scripts/verify_public_edge_centrifugo_contract.sh'
  add_cmd_once "public-edge Centrifugo boundary regression (403/order/hash/redaction RED proofs)" 'scripts/tests/test_public_edge_centrifugo_boundary.sh'
  add_note_once coverage "#1329 Rust/public-edge Centrifugo boundary: infra/rust/Caddyfile must terminate /v1/centrifugo/* with an exclusive 403 before the general /v1 proxy; API and Centrifugo must require the same CENT_PROXY_SECRET source; mutation fixtures prove missing/reordered deny, header drift, public 401/401/400, hash mismatch, current-secret 401, and any raw-secret evidence are RED. H1 fixtures additionally prove the runtime accepts only the exact HTTPS origin derived from the canonical Caddy site; attacker/typo/port/userinfo/path/query/fragment/punycode and ambiguous Caddy inputs produce zero secret reads, Docker execs, curl/network calls, or evidence, production/staging cannot enable the synthetic-only loopback escape, and 3xx is never followed."
  add_note_once not_covered "#1329 public app.oor7.com reload/recreate, real host/API/Centrifugo SHA-256 equality, and old-secret rotation evidence remain runtime-unverified(public host) until the attended public-edge runbook step executes scripts/verify_public_edge_centrifugo_boundary.sh."
  add_cmd_once "Rust image publication contract" 'python3 scripts/tests/test_publish_images_contract.py'
  add_cmd_once "self-host image day-2 tools syntax (#2346)" 'bash -n scripts/tests/test_image_day2_tools.sh'
...
add_local_alpha_commands() {
  ...
  add_note_once coverage "local-alpha is AWS-free: it uses local Docker, the Rust image path, repo-local mock Hermes, and local diagnostics only. It does not call AWS APIs, create cloud resources, trigger release workflows, or require public DNS/TLS."
...
add_web_commands() {
  # ADR-0183 / #2142: this profile is clients/web (the live SPA).
  add_cmd_once "worktree clean" 'if [ "${LOCAL_GATE_ALLOW_DIRTY:-0}" = "1" ]; then echo "LOCAL_GATE_ALLOW_DIRTY=1; dirty state is recorded but not failed"; git status --short; else test -z "$(git status --porcelain)" || { echo "worktree has uncommitted changes"; git status --short; exit 1; }; fi'
...
add_license_commands() {
  ...
  #
  # The SwiftPM license gate retired with the Swift client trees (W-S1 / #1201),
  # so this is now the only dependency license gate in the repo. Audit
  ...
  add_note_once coverage "#1225 dependency license gate: deny.toml is one policy for both cargo workspaces (server-rust 309 crates + clients/desktop/src-tauri 528, 644 unique third-party) and scripts/check_npm_licenses.mjs applies the same allowlist to the canonical npm trees (workspace root incl. packages/momo-core, clients/web, clients/mobile — 1,750 lockfile entries). SPDX expressions are evaluated before any name matching, so a permissive OR branch (node-forge \"BSD-3-Clause OR GPL-2.0\", r-efi \"MIT OR Apache-2.0 OR LGPL-2.1-or-later\") passes while an AND with a copyleft half fails. scripts/tests/test_license_gate.sh proves red on an injected AGPL-3.0 crate, on removing the reviewed MPL-2.0 allowance (desktop only — the backbone has zero MPL), on an unlicensed first-party workspace package, and proves the npm half reads the canonical trees rather than a retired npm tree."
...
case "$PROFILE" in
  docs)
    ...
    add_cmd_once "self-host image day-2 tools (#2346)" 'scripts/tests/test_image_day2_tools.sh'
    ...
    add_cmd_once "pgBackRest/PITR fail-closed contract (#2157)" 'scripts/tests/test_pgbackrest_pitr_contract.sh'
    ...
    add_cmd_once "release manifest drift (#1984)" 'scripts/check_release_manifest.sh'
    add_cmd_once "release manifest contract" 'scripts/tests/test_release_manifest.sh'
    add_note_once coverage "Static docs/CI validation plus SH day-2/doctor/public-edge tests (#2124; harnesses run here, docker missing is RED), oort dispatcher/lib bash -n + shellcheck in the static block (#2124), in-image T2 doctor/backup tools (#2346/#2347: PGDG postgresql-client-18 + python3=3.11.* json; docs profile runs scripts/tests/test_image_day2_tools.sh which docker-builds server-rust/Dockerfile when MOMO_RUST_IMAGE is unset; docker missing is RED, not an optional-tool skip), the pgBackRest/PITR fail-closed contract live run (#2157: scripts/tests/test_pgbackrest_pitr_contract.sh in this arm, docker missing is RED via the same command -v docker / docker info guard as day-2/doctor, not bash -n only; the encrypted closed-loop e2e stays on the backup profile), the Railway platform template contract (#2297: railway.json services/digest pin, --railway key-set equality, Caddyfile.railway caddy adapt + 403 order, public-edge contract on the Caddyfile.railway fixture root), the AWS T1 recipe contract (#2377: prevent_destroy, ports {22,80,443}, Budgets, data-root /data/docker; terraform fmt/validate when terraform is installed, otherwise a visible optional-tool skip line), the Fly T1 recipe contract (SH-11b), the Cloudflare T3 edge recipe contract (SH-11d: loopback ingress, catch-all 404, no quick-tunnel, Mode B --public-origin, approval section), the release-manifest contract harness, and the live scripts/check_release_manifest.sh run against committed releases/latest.json (#1984; no-arg mode, not bash -n only)."
