...
add_static_commands() {
  ...
  add_note_once coverage "#1266 Rust publish/self-host contract: the manual workflow must build server-rust/Dockerfile for native linux/amd64 with the exact source SHA, OCI provenance/SBOM/license metadata and no QEMU; all seven runtime roles must remain reachable from the one image. The behavioral fixture proves local-build keeps the build overlay/--build, published-digest requires the canonical full sha256 ref and omits both, existing env files cannot silently switch modes, and mutable published tags fail before an env file is written."
