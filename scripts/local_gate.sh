...
add_static_commands() {
  ...
  add_cmd_once "local alpha Centrifugo agent proxy contract" 'agent_block="$(awk '\''/"name": "agent"/,/},/'\'' scripts/local_alpha_runner.sh)"; printf "%s\n" "$agent_block"; printf "%s\n" "$agent_block" | grep -F "\"subscribe_proxy_enabled\": true"; printf "%s\n" "$agent_block" | grep -F "\"channel_regex\": \"^ws[0-9A-Fa-f-]{36}\\\\\\\\.[0-9A-Fa-f-]{36}$\""'
...
add_runtime_agent_commands() {
  ...
  add_note_once coverage "MOMO-325/MOMO-337 Hermes gateway path: scripts/verify_hermes_gateway_adapter.sh runs AGENT_GATEWAY_MODE=gateway, mints sha256-only per-agent bearer credentials, verifies scoped realtime/pending/message/callback routes, actor binding, 24h rotation overlap, revoke fail-closed, audit_log.via_token_id, durable final message/usage/job completion, and separately proves the deprecated shared secret works only with MOMO_ALLOW_LEGACY_GATEWAY_SECRET=1."
}
