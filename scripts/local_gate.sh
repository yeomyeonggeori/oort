...
add_runtime_db_commands() {
  ...
  add_note_once coverage "MOMO-511 terminal attach via scripts/verify_terminal_attach.sh: isolated e2e API/relay stack on reserved 27970-27973 ports verifies MomoHost remote PTY binding, exact owner-only ephemeral grant, signed host validation, expiry rejection, immediate host-revoke invalidation, digest-only storage, audit shape, FORCE RLS, and absence of terminal raw bytes or capability values from server/relay ledgers and logs."
