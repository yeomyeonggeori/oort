...
add_external_agent_provider_commands() {
  add_cmd "External agent runtime provider smoke" "scripts/verify_external_agent_provider.sh"
  add_note_once coverage "MOMO-230/MOMO-236/MOMO-242 opt-in external provider gate: when AGENT_PROVIDER_MODE=external-hermes plus HERMES_BASE_URL/HERMES_API_KEY are configured, scripts/verify_external_agent_provider.sh checks OpenAI-compatible SSE directly, boots local Postgres/Centrifugo + MomoServer/OutboxRelay/AgentWorker, verifies /v1/agent-runtime/status redacted availability with no degradedReason, proves Kim Intern is an active invited agent member of #agent-lab, and sends one @김인턴 roundtrip through the external runtime."
