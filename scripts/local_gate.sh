...
auto_classify_script() {
  ...
  case "$1" in
    ...
    scripts/verify_staging_smoke.sh|scripts/verify_internal_hosting_smoke.sh|scripts/verify_prod_install_upgrade.sh|scripts/verify_metrics_observability.sh|scripts/prod_env_preflight.sh|scripts/aws_internal_alpha_preflight.sh)
...
add_static_commands() {
  ...
  add_cmd_once "python syntax" 'PYTHONPYCACHEPREFIX="${TMPDIR:-/tmp}/momo-pycache" python3 -m py_compile adapters/hermes/momo_adapter.py adapters/hermes/provider_chain.py adapters/hermes/adapter.py scripts/mock_hermes.py scripts/mock_push_relay.py scripts/openapi_shape_check.py adapters/hermes/tests/test_momo_adapter_contract.py adapters/hermes/tests/test_provider_chain_contract.py adapters/hermes/tests/smoke_momo_adapter.py scripts/tests/test_agent_seed_policy_contract.py scripts/tests/test_momo354_roster_contract.py && PYTHONPYCACHEPREFIX="${TMPDIR:-/tmp}/momo-pycache" python3 adapters/hermes/tests/test_momo_adapter_contract.py && PYTHONPYCACHEPREFIX="${TMPDIR:-/tmp}/momo-pycache" python3 adapters/hermes/tests/test_provider_chain_contract.py && PYTHONPYCACHEPREFIX="${TMPDIR:-/tmp}/momo-pycache" python3 adapters/hermes/tests/smoke_momo_adapter.py && PYTHONPYCACHEPREFIX="${TMPDIR:-/tmp}/momo-pycache" python3 scripts/tests/test_agent_seed_policy_contract.py && PYTHONPYCACHEPREFIX="${TMPDIR:-/tmp}/momo-pycache" python3 scripts/tests/test_momo354_roster_contract.py'
}
...
add_runtime_db_commands() {
  ...
  add_cmd "Work session idle lifecycle verification (MOMO-648)" "scripts/verify_work_session_idle.sh"
  add_note_once coverage "MOMO-648 idle lifecycle via scripts/verify_work_session_idle.sh: isolated API/relay/notifier/mock-push stack on reserved 28230-28234 ports drives signed-host REST running-to-idle-to-running, a live-host workspace-configured idle timeout to ended(idle_timeout), and an idle host disconnect to orphaned before timeout; it asserts exact lifecycle outbox/audit counts, completion messages, and owner-only id-only momo.work push dispatches."
