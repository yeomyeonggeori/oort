...
add_static_commands() {
  ...
  add_note_once coverage "#1525 docs command drift via scripts/check_docs_commands.py: every command in the GATED_DOCS table (AGENTS.md, docs/RELEASING.md, docs/NEXT_CHANNEL.md, CONTRIBUTING.md, docs/SELF_HOST_AGENT.md) and every docs/runbooks/*.md must resolve against this tree — the executor exists, is executable and parses (bash -n / py_compile); \`make <target>\` is a target the Makefile defines; \`--profile <p>\` is a profile local_gate.sh's own parser accepts; \`npm [--prefix DIR] run <s>\` is a script that package.json defines; a long flag handed to one of our scripts appears in that script; docker compose -f/--env-file and --package-path/--manifest-path paths exist; and a documented \`cargo fmt --check\` carries --all (the #1472 rule). Runbooks are gated by glob, so a new one arrives covered; the tabled documents are a table with reasons, and a tabled document that disappears is red rather than silently uncovered. Deliberately NOT covered: commands are never executed, so a command that resolves but does the wrong thing is still invisible; unrecognized token shapes are skipped rather than guessed at (a gate that flagged <workspace-id> would be switched off within a week); and \`<!-- docs-cmd-ignore: reason -->\` exempts a single line, at the cost of a written reason. AGENTS.md and docs/SELF_HOST.md were dropped from the table in #2142 / ADR-0183; AGENTS.md and live runbooks remain gated."
