...
add_runtime_agent_commands() {
  ...
  add_cmd "AgentWorker persistent DB repeat verification" "scripts/verify_agent_worker.sh"
  ...
  add_note_once coverage "MOMO-325/MOMO-337/MOMO-338 Hermes gateway path: scripts/verify_hermes_gateway_adapter.sh runs AGENT_GATEWAY_MODE=gateway, mints sha256-only per-agent bearer credentials, proves self-only agentwork: job delivery is isolated from observable agent: progress, verifies scoped pending/message/callback routes, actor binding, 24h rotation overlap, revoke fail-closed, audit_log.via_token_id, durable final message/usage/job completion, and separately proves the deprecated shared secret works only with MOMO_ALLOW_LEGACY_GATEWAY_SECRET=1."
}
