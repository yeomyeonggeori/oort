...
add_runtime_db_commands() {
  ...
  add_note_once coverage "MOMO-533 work tool profiles via scripts/verify_work_tool_profile.sh: isolated e2e API stack on preflight-checked reserved 28080-28083 ports verifies the four default seeds, admin CRUD/audit, portable credential/path-free launch templates, signed workd GET projection, custom spawn, disabled/unregistered fail-closed rejection, and FORCE RLS tenant isolation."
