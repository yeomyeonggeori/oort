...
# Swift `server/` 스캔은 ADR-0183 / #2142 에서 빠졌다. 라이브 서빙 표면은
# `clients/web` 이고, 사용자가 받는 서버 문장은 `server-rust/` 에서 나온다.
...
def check_structured():
    ...
    html = open("clients/web/index.html", encoding="utf-8").read()
    title = re.search(r"<title>(.*?)</title>", html, re.S)
    expect("clients/web/index.html <title>", title and title.group(1).strip(), "oort")
