...
write_summary() {
  SUMMARY="$EVIDENCE_DIR/summary.md"
  {
    ...
    echo "## Env"
    echo
    for key in COMPOSE_PROJECT_NAME POSTGRES_DB POSTGRES_PORT DATABASE_URL CENT_PORT CENT_API_URL PORT HOST HERMES_BASE_URL HERMES_API_KEY CENT_API_KEY CENT_PROXY_SECRET JWT_HMAC AGENT_GATEWAY_MODE AGENT_GATEWAY_SECRET MOMO_ALLOW_LEGACY_GATEWAY_SECRET AGENT_CONTEXT_MAX_MESSAGES AGENT_CONTEXT_MAX_CHARS; do
      eval "value=\${$key:-}"
      printf -- "- %s: " "$key"
