...
PY
  ...
  if command -v uuidgen >/dev/null 2>&1; then
    printf '%s-%s-%s\n' "$(uuidgen)" "$(uuidgen)" "$(uuidgen)" | tr '[:upper:]' '[:lower:]'
    return
  fi
  fail "cannot generate local alpha secret; install openssl or python3"
}

write_generated_env() {
  ENV_FILE="$EVIDENCE_DIR/local-alpha.env"
  CENT_TOKEN_HMAC_VALUE=$(generated_secret)
  CENT_API_KEY_VALUE=$(generated_secret)
  CENT_PROXY_SECRET_VALUE=$(generated_secret)
  JWT_HMAC_VALUE=$(generated_secret)
  # #2066 — momo-server refuses to boot without both webhook master keys.
  WEBHOOK_INGRESS_MASTER_KEY_VALUE=$(generated_secret)
  OUTBOUND_WEBHOOK_MASTER_KEY_VALUE=$(generated_secret)
  HERMES_API_KEY_VALUE=$(generated_secret)
  umask 077
  cat >"$ENV_FILE" <<EOF
...
CENT_TOKEN_HMAC=${CENT_TOKEN_HMAC_VALUE}
CENT_API_KEY=${CENT_API_KEY_VALUE}
CENT_PROXY_SECRET=${CENT_PROXY_SECRET_VALUE}
CENT_API_URL=http://localhost:${CENT_PORT:-28000}/api
JWT_HMAC=${JWT_HMAC_VALUE}
WEBHOOK_INGRESS_MASTER_KEY=${WEBHOOK_INGRESS_MASTER_KEY_VALUE}
OUTBOUND_WEBHOOK_MASTER_KEY=${OUTBOUND_WEBHOOK_MASTER_KEY_VALUE}
PORT=${PORT:-28080}
HOST=0.0.0.0
HERMES_PORT=${HERMES_PORT:-28088}
HERMES_BASE_URL=http://localhost:${HERMES_PORT:-28088}/v1
HERMES_API_KEY=${HERMES_API_KEY_VALUE}
AGENT_CONTEXT_MAX_MESSAGES=${AGENT_CONTEXT_MAX_MESSAGES:-30}
AGENT_CONTEXT_MAX_CHARS=${AGENT_CONTEXT_MAX_CHARS:-24000}
...
load_env_file() {
  ...
  MOMO_ALLOW_LEGACY_GATEWAY_SECRET=${MOMO_ALLOW_LEGACY_GATEWAY_SECRET:-0}
  AGENT_CONTEXT_MAX_MESSAGES=${AGENT_CONTEXT_MAX_MESSAGES:-30}
  AGENT_CONTEXT_MAX_CHARS=${AGENT_CONTEXT_MAX_CHARS:-24000}
  ...
  export CENT_API_KEY CENT_TOKEN_HMAC CENT_PROXY_SECRET JWT_HMAC
  export WEBHOOK_INGRESS_MASTER_KEY OUTBOUND_WEBHOOK_MASTER_KEY
  export AGENT_GATEWAY_MODE AGENT_GATEWAY_SECRET MOMO_ALLOW_LEGACY_GATEWAY_SECRET
  export AGENT_CONTEXT_MAX_MESSAGES AGENT_CONTEXT_MAX_CHARS
}
...
write_summary() {
  SUMMARY="$EVIDENCE_DIR/summary.md"
  {
    ...
    echo "## Env"
    echo
    for key in COMPOSE_PROJECT_NAME POSTGRES_DB POSTGRES_PORT DATABASE_URL CENT_PORT CENT_API_URL PORT HOST HERMES_BASE_URL HERMES_API_KEY CENT_API_KEY CENT_PROXY_SECRET JWT_HMAC WEBHOOK_INGRESS_MASTER_KEY OUTBOUND_WEBHOOK_MASTER_KEY AGENT_GATEWAY_MODE AGENT_GATEWAY_SECRET MOMO_ALLOW_LEGACY_GATEWAY_SECRET AGENT_CONTEXT_MAX_MESSAGES AGENT_CONTEXT_MAX_CHARS; do
      eval "value=\${$key:-}"
      printf -- "- %s: " "$key"
...
load_env_file
[ "$CENT_PROXY_SECRET" != "" ] || fail "CENT_PROXY_SECRET is required for local alpha realtime subscribe proxy"
write_local_centrifugo_config
write_stop_script
