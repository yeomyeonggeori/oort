...
is_sensitive_key() {
  case "$1" in
    *PASSWORD*|*HMAC*|*API_KEY*|*TOKEN*|*SECRET*|*CIPHER_PASS*|DATABASE_URL|RELAY_DATABASE_URL|WORKER_DATABASE_URL|MOMO_APP_DATABASE_URL|MIGRATE_DATABASE_URL|OUTBOUND_WEBHOOK_MASTER_KEY|PROVIDER_LINK_MASTER_KEY|CENTRIFUGO_REDIS_ADDRESS|HERMES_BASE_URL)
      return 0
      ;;
    *)
      return 1
...
assert_db_url_user() {
  ...
  [ "$value" != "" ] || return 0
  case "$value" in
    postgres://"$expected":*@postgres:*/*|postgresql://"$expected":*@postgres:*/*)
      pass "$key uses expected database role $expected"
      ;;
    *)
      fail "$key must authenticate as $expected on compose-internal postgres"
      ;;
  esac
}
...
assert_values_differ() {
  local first="$1"
  local second="$2"
  if [ "$(get_var "$first")" = "$(get_var "$second")" ]; then
    fail "$first must not reuse $second"
  else
    pass "$first is independent from $second"
  fi
}
...
if [ "$runtime_mode" = "strict" ]; then
  require_vars \
    ...
    MOMO_IMAGE_TAG MOMO_API_IMAGE MOMO_RELAY_IMAGE MOMO_WORKER_IMAGE MOMO_MIGRATE_IMAGE MOMO_WEB_IMAGE MOMO_LINKSHORT_IMAGE \
    POSTGRES_DB POSTGRES_USER POSTGRES_PASSWORD MIGRATE_DATABASE_URL \
    MOMO_APP_POSTGRES_PASSWORD MOMO_APP_DATABASE_URL RELAY_POSTGRES_PASSWORD RELAY_DATABASE_URL \
    WORKER_POSTGRES_PASSWORD WORKER_DATABASE_URL \
    REDIS_PASSWORD CENTRIFUGO_REDIS_ADDRESS CENT_TOKEN_HMAC CENT_API_KEY CENT_PROXY_SECRET JWT_HMAC OUTBOUND_WEBHOOK_MASTER_KEY PROVIDER_LINK_MASTER_KEY \
  ...
  for key in \
    API_DOMAIN REALTIME_DOMAIN MOMO_CENTRIFUGO_WS_URL ACME_EMAIL MOMO_IMAGE_TAG MOMO_API_IMAGE MOMO_RELAY_IMAGE MOMO_WORKER_IMAGE MOMO_MIGRATE_IMAGE MOMO_WEB_IMAGE MOMO_LINKSHORT_IMAGE \
    PUBLIC_BASE_URL CADDY_EMAIL \
    POSTGRES_PASSWORD MIGRATE_DATABASE_URL MOMO_APP_POSTGRES_PASSWORD MOMO_APP_DATABASE_URL \
    RELAY_POSTGRES_PASSWORD RELAY_DATABASE_URL WORKER_POSTGRES_PASSWORD WORKER_DATABASE_URL \
    REDIS_PASSWORD CENTRIFUGO_REDIS_ADDRESS \
    CENT_TOKEN_HMAC CENT_API_KEY CENT_PROXY_SECRET JWT_HMAC OUTBOUND_WEBHOOK_MASTER_KEY HERMES_BASE_URL HERMES_API_KEY SECRET_SOURCE \
  ...
  assert_db_url_host MIGRATE_DATABASE_URL
  assert_db_url_host MOMO_APP_DATABASE_URL
  assert_db_url_host RELAY_DATABASE_URL
  assert_db_url_host WORKER_DATABASE_URL
  assert_db_url_user MIGRATE_DATABASE_URL "$POSTGRES_USER"
  assert_db_url_user MOMO_APP_DATABASE_URL momo_app
  assert_db_url_user RELAY_DATABASE_URL momo_relay
  assert_db_url_user WORKER_DATABASE_URL momo_worker
  assert_values_differ OUTBOUND_WEBHOOK_MASTER_KEY JWT_HMAC
  assert_values_differ PROVIDER_LINK_MASTER_KEY JWT_HMAC
  assert_values_differ PROVIDER_LINK_MASTER_KEY OUTBOUND_WEBHOOK_MASTER_KEY
  ...
  require_vars \
    ...
    POSTGRES_DB POSTGRES_USER POSTGRES_PASSWORD MIGRATE_DATABASE_URL \
    MOMO_APP_POSTGRES_PASSWORD MOMO_APP_DATABASE_URL DATABASE_URL \
    RELAY_POSTGRES_PASSWORD RELAY_DATABASE_URL WORKER_POSTGRES_PASSWORD WORKER_DATABASE_URL MOMO_BOOTSTRAP_RUNTIME_ROLES \
    REDIS_PASSWORD CENTRIFUGO_REDIS_ADDRESS CENT_TOKEN_HMAC CENT_API_KEY CENT_PROXY_SECRET JWT_HMAC OUTBOUND_WEBHOOK_MASTER_KEY \
    AGENT_PROVIDER_MODE AGENT_MODEL HERMES_BASE_URL HERMES_API_KEY
  ...
  assert_exact OUTBOUND_WEBHOOK_MASTER_KEY change-me-outbound-webhook-master-key
