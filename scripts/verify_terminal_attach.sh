...
API_PORT="${TERMINAL_ATTACH_GATE_API_PORT:-27970}"
CENT_PORT_HOST="${TERMINAL_ATTACH_GATE_CENTRIFUGO_PORT:-27971}"
PG_PORT="${TERMINAL_ATTACH_GATE_POSTGRES_PORT:-27972}"
HERMES_PORT_HOST="${TERMINAL_ATTACH_GATE_HERMES_PORT:-27973}"
  ...
  (SELECT count(*) FROM audit_log a
    WHERE a.action='work.terminal_attach.issued'
      ...
      AND (SELECT array_agg(k ORDER BY k) FROM jsonb_object_keys(a.detail) k)
          = ARRAY['expires_at','issued_at','owner_member_id','schema']::text[])
  || ':' ||
  (SELECT count(*) FROM terminal_attach_capability c
