...
require(components == ["api", "relay", "worker", "migrate", "web"], "matrix must contain five ordered runtime images")
for image in ("momo-server", "momo-outbox-relay", "momo-agent-worker", "momo-migrate", "momo-web"):
    require(f"image: {image}" in workflow, f"workflow missing {image}")
require(workflow.count("dockerfile: infra/prod/docker/swift-service.Dockerfile") == 3, "three Swift services must share the production Dockerfile")
...
for variable in ("MOMO_API_IMAGE", "MOMO_RELAY_IMAGE", "MOMO_WORKER_IMAGE", "MOMO_MIGRATE_IMAGE", "MOMO_WEB_IMAGE"):
