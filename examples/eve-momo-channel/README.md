# eve → momo channel reference

This example lets an eve-hosted agent consume momo's existing gateway BYOA contract. It does not add a server route and never publishes to Centrifugo directly.
...
`continuationToken` is a stable momo workspace/channel/thread key:

```text
momo:<workspace_id>:<channel_id>:<root_or_trigger_message_id>
```

eve namespaces this raw token with the channel file name, so follow-up jobs for the same momo conversation resume the same eve session.

## Install and place the channel

This directory intentionally owns its npm graph; do not add these packages to momo's root.
...
|---|---:|---|
| `MOMO_BASE_URL` | yes | momo API origin, for example `https://momo.example.com` |
| `MOMO_WORKSPACE_ID` | yes | Workspace UUID encoded in the gateway route |
| `MOMO_AGENT_MEMBER_ID` | yes | momo `member(kind=agent)` UUID targeted by pending jobs |
| `MOMO_AGENT_TOKEN` | yes | Scoped per-agent bearer minted by momo; outbound gateway calls only |
...
The momo paths consumed here are:
...
- This is a minimal polling reference, not a production scheduler. Add bounded retry/backoff and lease renewal for turns that can exceed momo's lease duration.
- The route claims one job at a time. Scale-out callers must preserve the lease returned by momo; never synthesize or share it.
...
- Approval cards and approval decisions round-trip in the momo app. This adapter must stop on an approval request; it does not render or approve cards inside eve.
- Provider/model credentials belong to eve. They must not enter momo payloads, logs, state, or this adapter.
